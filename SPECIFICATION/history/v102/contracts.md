...
### Work-item integrity invariants — plugin-agnostic data acquisition

The six work-item integrity invariants — `no-orphan-dependency`, `no-stalled-epic`, `no-duplicate-gap-id`, `no-stale-gap-tied`, `depends_on-ref-wellformedness`, and `unresolved-spec-commitment` — MUST acquire the materialized work-items by invoking the ACTIVE impl-plugin's `list-work-items --filter all --json` thin-transport wrapper, NOT by a direct read of any backend's on-disk store. The wrapper's absolute path is resolved from the `LIVESPEC_IMPL_LIST_WORK_ITEMS` environment variable (an absolute path to the active impl-plugin's `list_work_items.py`). The doctor check runs the wrapper as a subprocess from `cwd=<project_root>`, inheriting the caller's environment — which supplies any backend-specific connection prerequisites (e.g. the beads tenant's `BEADS_DOLT_PASSWORD` / `LIVESPEC_BD_PATH` and the `.beads/` pointer files the wrapper reads from cwd). The wrapper emits a top-level JSON ARRAY of materialized work-item views (per §"Thin-transport skills (3) — required machine query surface"); each view exposes the fields the invariants consume (`id`, `type`, `status`, `origin`, `gap_id`, `depends_on`, and `spec_commitment_hint`). This mechanism is plugin-agnostic by construction: it works identically for `livespec-impl-plaintext` (a JSONL store) and `livespec-impl-beads` (a Dolt-server tenant) and any future backend, because every impl-plugin exposes the same `list-work-items` machine surface.

Graceful skip (no CI regression). When `LIVESPEC_IMPL_LIST_WORK_ITEMS` is unset, OR the wrapper invocation fails to connect (no live provider, no credentials, nonzero exit, or non-array output), each of the six invariants MUST return a `skipped` Finding with a clear message ("no live work-item provider configured" / "work-item store unreachable") — NOT a `fail`. A genuine wrapper CONNECTION failure is treated as `skipped`; only an actual invariant violation fires `fail` (or `warn` for `no-stale-gap-tied`). This preserves the hermetic-CI behavior (where the env var is unset and the invariants already skip) while enabling live enforcement. The `LIVESPEC_IMPL_LIST_WORK_ITEMS` env var (plus the per-tenant backend connection) is exported by the Layer 3 orchestrator's doctor janitor gate (reusing the read-path the orchestrate driver's §"Cross-repo state aggregation" documents), so the six invariants RUN live against the active tenant at the gate; in hermetic CI the env var is unset and they skip.

The invariant LOGIC each of the six sub-sections below describes is unchanged by this acquisition mechanism — only the data-acquisition path is plugin-agnostic. The `no-stale-gap-tied` invariant additionally derives its current-gap-id set from the live `<spec-root>/` content (unchanged).
...
The compat block carries two required fields: `livespec` (a semver range describing supported `livespec` versions, e.g., `>=2.0.0,<3.0.0`) and `pinned` (the specific `livespec` release tag the consumer currently runs against, e.g., `v2.3.0`). Both fields live on each consumer's top-level section in `.livespec.jsonc`, keyed by the consumer name — on the `livespec-impl-beads` key for impl-plugins, on the `livespec-dev-tooling` key for the enforcement-suite sibling library, on the `livespec-runtime` key for the runtime sibling library, and on whatever key names a future sibling consumer registers.
