...
### pi dogfooding compatibility

The `livespec` repository supports maintainer dogfooding from the pi coding agent through the distributed pi Driver plugin `livespec-driver-pi` (the analogue of `livespec-driver-claude` and `livespec-driver-codex`), installed as a project-scoped pi package via the committed `.pi/settings.json` per `contracts.md` §"Plugin distribution". The pi Driver ships THIN pi SKILL.md bindings over CORE's harness-neutral operation prose and wrapper CLIs; it carries no operation behavior of its own.

pi dogfooding is a Driver binding, not a separate LiveSpec product command model. The authoritative command behavior remains the core-owned operation prose under `.claude-plugin/prose/<name>.md` plus the spec-side wrapper contracts under `.claude-plugin/scripts/bin/`. When a pi session receives a request for a `/livespec:*` operation (surfaced in pi as the `livespec-<operation>` skill), the pi Driver binding MUST read the matching `.claude-plugin/prose/<name>.md` file completely before acting; when that prose calls for a CLI, it MUST invoke the configured wrapper under `.claude-plugin/scripts/bin/` with explicit argv. The pi Driver bindings MUST NOT copy operation prose, wrapper files, or built-in templates, and MUST NOT point at `.claude-plugin/skills/*`; core intentionally ships no `.claude-plugin/skills/` tree.

The pi Driver exposes the FULL eight spec-side operations, INCLUDING the mutating ones (`seed`, `propose-change`, `critique`, `revise`, `prune-history`). Mutating pi automation is gated on the pi footgun-guard `tool_call` extension required by `contracts.md` §"Driver-shipped hooks"; until that guard is in place a pi Driver MUST NOT claim mutating operations. All destructive-command controls from the core prose are preserved — in particular `prune-history` remains explicit-user-invocation only.

Under the sibling-repo topology, each orchestrator plugin's repository is responsible for its own pi Driver mapping; implementation-side workflows MUST NOT be promoted into `livespec` by pi compatibility work.
...
- `auto-enable-merge.yml` — auto-enables REBASE auto-merge on PR open. Required so that propose-change PRs in every impl-plugin repo merge with the same cadence as upstream `livespec` PRs (incident 2026-05-26: `livespec-impl-plaintext` PR #26 sat OPEN/CLEAN for 10+ minutes because this file was absent). Rebase-merge resolves each PR against the current `master` tip at merge time and re-runs CI on the rebased result, so no pre-merge branch-update step is required.
  - A plain propose-change-filing pull request — one that adds a file under `proposed_changes/` but nothing under `history/*/proposed_changes/` — always derives a successfully-computed, known-empty stem set, so it is NOT subject to the `spec_pr_merge` conservative fold and keeps the auto-merge-on-open eligibility this bullet documents unaffected; §"Spec pull-request auto-merge requirement" gates only a pull request that actually RATIFIES one or more proposals.
...
Each enumerated file MAY be a Jinja-templated thin pass-through that delegates to a reusable workflow, either at `thewoolleyman/livespec-dev-tooling/.github/workflows/<name>.yml@vX.Y.Z` (per §"Shared code sync — livespec-dev-tooling") when the implementation is fleet-stable enforcement tooling, or at `thewoolleyman/livespec/.github/workflows/reusable-<name>.yml@vX.Y.Z` (per §"Shared CI logic — core-hosted reusable workflows") when it enforces a contract `livespec` itself owns — the reusable-workflow consumption pattern is the canonical sharing mechanism for any workflow whose implementation is uniform across livespec-governed repos. The contract-level requirement is that the file EXISTS in each impl-plugin's `.github/workflows/`; whether the file's body inlines logic or `uses:` a reusable workflow is the template author's choice.
...
Consumers consume `livespec-runtime` via the Python package surface added to `pyproject.toml` either as a runtime dependency under `[project].dependencies` or as a dev dependency under `[dependency-groups].dev`, with `[tool.uv.sources]` declaring `git = "https://github.com/thewoolleyman/livespec-runtime.git"` plus `tag = "vX.Y.Z"`. `livespec` core additionally VENDORS the package under `_vendor/` per `constraints.md` §"Locked vendored libs" — an instance of the source-copied shape §"Shared content provenance" describes — so core carries both surfaces. Invocation MUST take the form `import livespec_runtime.<subpackage>` or `python -m livespec_runtime.<entry>`. PyPI publishing is NOT required in v1; the uv git-source path is sufficient for tag-pinned reproducible builds. There is NO reusable GitHub Actions surface (consumers invoke `livespec-runtime` from their own workflow steps directly, since the call sites are inside skill / hook / wrapper code that the consumer composes itself).
...
5. Resolve the pull request's effective `spec_pr_merge` policy per `spec.md` §"Spec-governance policy settings". An effective `manual` policy — or a pull request whose stem-set derivation step FAILS — leaves the pull request for a human rebase-merge to `master`. An effective `auto-on-green` policy registers rebase auto-merge (equivalent to `gh pr merge --rebase --auto`) after the pull request is opened and before CI completes, subject to the registration-time hard floors defined in `contracts.md` §"Spec pull-request merge-registration mechanics". Both paths merge by rebase only, per the rebase-merge-only rule already codified for `master`.
6. After the merge is CONFIRMED — under either policy — remove the worktree (`git worktree remove <path>`) and delete the local branch. The remote branch deletion is part of the same step: `gh pr merge --delete-branch` in the manual path, while the automatic path MUST NOT assume that invocation and instead deletes the remote branch by whatever means the confirmed merge leaves available.
...
#### Spec pull-request auto-merge requirement

Both `livespec`'s root-repo `.github/workflows/auto-enable-merge.yml` AND the orchestrator-plugin copier template's `templates/orchestrator-plugin/.github/workflows/auto-enable-merge.yml.jinja` MUST implement the following requirement for a pull request whose diff touches a configured spec root: compute the merge-base diff; derive the added non-`-revision` proposed-change stems under `<spec-root>/history/*/proposed_changes/` across every configured spec root; and, ONLY when that derived set is non-empty, invoke `spec_governance.py --project-root "$GITHUB_WORKSPACE" --show-effective --pr-effective-policy --proposal-stem <stem>` (repeating `--proposal-stem` once per derived stem) to obtain the pull-request effective policy, registering auto-merge only on an `auto-on-green` result. Both files MUST satisfy this requirement through the SAME shared implementation, per §"Shared CI logic — core-hosted reusable workflows"; two independent copies of the derivation are PROHIBITED, because a defect fixed in one copy silently persists in the other while both repositories' CI stays green.

A pull request touching no spec root, or one whose derived stem set is successfully computed as EMPTY (it ratifies nothing), MUST fall through unchanged to the non-spec eligibility gates defined in `contracts.md` §"Spec pull-request merge-registration mechanics" — the same path a non-spec pull request takes. A pull request whose stem-set derivation step FAILS, per `spec.md` §"Spec-governance policy settings" `effective_spec_pr_merge`(b) and the dual-source hardening stated there, MUST NOT register for auto-merge.

This requirement applies uniformly to `livespec` itself and to every sibling generated from the orchestrator-plugin template. DERIVE the applicable-repo set from template consumption rather than from a hardcoded count or category. Repo `thewoolleyman/livespec` MAY declare `spec_governance.spec_pr_merge: auto-on-green` once this contract ships; every other repo inherits the safe `manual` default when the key is absent.
...
pi compatibility for repository dogfooding MUST NOT duplicate LiveSpec operation prose, Python wrappers, or built-in specification templates. Core prose under `.claude-plugin/prose/<name>.md` and wrapper CLIs under `.claude-plugin/scripts/bin/` are the shared source of truth; the pi Driver bindings MUST remain thin over those files and MUST NOT copy or restate operation-specific steps, failure handling, output schemas, or wrapper behavior in a way that can drift. The pi Driver MUST NOT point at `.claude-plugin/skills/*` or require that tree to exist.

Core MAY claim pi-native plugin support only once the pi package registration creates the committed `.pi/settings.json` entries AND a separate non-interactive `pi -p` invocation can drive a `/livespec:*` operation through the registered packages without relying on any repo-local adapter or `AGENTS.md` mapping; the skill-command expansion (`/skill:livespec-<operation>`) MUST be verified in the non-interactive path itself, not assumed from its documented interactive behavior. The mere existence of package entries in `.pi/settings.json` does NOT by itself license the claim — pi's trust gate means an untrusted non-interactive invocation silently ignores project packages, so the live drive is the only evidence that counts.

pi dogfooding MUST work without installing or modifying global/system packages beyond the per-user pi CLI installation itself; the package installs are project-scoped. Temporary pi package registrations used for testing MUST be removed after the test unless the user explicitly asks to keep them.

The pi Driver surface MUST preserve every destructive-command control from the core prose. In particular, `prune-history` remains explicit-user-invocation only, and pi MUST NOT infer or auto-activate it from a generic mention of history. Mutating pi operations (`seed`, `propose-change`, `critique`, `revise`, `prune-history`) MUST NOT be exercised unless the pi footgun-guard `tool_call` extension required by `contracts.md` §"Driver-shipped hooks" is in place.
...
When a non-interactive `pi -p` invocation names the `livespec-next` skill

Then the pi Driver binding reads core's `prose/next.md` and invokes the configured next wrapper under `.claude-plugin/scripts/bin/`

And the operation resolves without any repo-local adapter directory or `AGENTS.md` mapping
...
When a non-interactive `pi -p` invocation names a `livespec-<operation>` skill without a per-run approve flag

Then pi ignores the project-local packages and the operation does not resolve

And the correct operator remedy is establishing trust, never copying operation content into the project
