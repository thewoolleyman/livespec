...
Livespec MUST NOT vendor rendering binaries or runtimes for any diagram language: no bundled `plantuml.jar`, no Java-runtime detection, no Graphviz dependency, no Mermaid CLI bundle. The `render_commands` shape declared in `.livespec.jsonc` (per `contracts.md` §"Template manifest wire contract") is the only mechanism by which livespec invokes rendering; the project itself is responsible for ensuring the named binary is on `PATH` at revise time.

Templates that include `diagram_source` and `diagram_rendered` entries MUST state the external renderer requirement in their `README.md` and (when shipped) `CLAUDE.md` with an install recipe per OS. Livespec MUST NOT detect, recommend, or attempt to install the renderer.

The non-vendoring constraint preserves the "Python 3.10 is the sole end-user runtime dependency" invariant in §"End-user runtime dependencies": diagram-supporting templates are an opt-in extension that the project author chooses (and accepts the renderer install burden for), not a livespec-wide dependency expansion.
...
The five work-item structural invariants codified in `contracts.md` §"Doctor cross-boundary invariants" (`gap-tracking-one-to-one`, `no-orphan-dependency`, `no-stale-gap-tied`, `no-duplicate-gap-id`, `no-stalled-epic`) are cross-boundary checks: each queries the active impl-plugin's machine surface (the `list-work-items --json` and `detect-impl-gaps --json` thin-transport skills) rather than livespec-internal state. The cross-boundary nature MUST be reflected in the checks' exit-code derivation: a missing or unresponsive impl-plugin query surface MUST surface as a `fail` Finding (lifting the wrapper exit to non-zero) rather than as a silent skip. The check IDs MUST follow the existing slug convention (lowercase-hyphenated, per §"File naming and invocation"): `gap-tracking-one-to-one`, `no-orphan-dependency`, `no-stale-gap-tied`, `no-duplicate-gap-id`, `no-stalled-epic`.

The doctor static invariant catalog MUST be enumerable from a single named registry inside `livespec.doctor.static` so contributors and tools can list all checks programmatically; the registry contract follows the existing slug↔module-name mapping pattern stated in §"File naming and invocation". Every check ID covered by `contracts.md` §"Doctor cross-boundary invariants" — the five work-item invariants above and `contract-version-compatibility` — MUST appear in this registry.
