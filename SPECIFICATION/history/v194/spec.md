...
`effective_revise_decision_mode` MUST resolve in this order: a cited design-record contradiction; a missing or unreachable design record; any ratification-review blocker; a drift-origin proposal; a valid per-proposal `decision_policy`; the valid global `revise_decision_mode`; and the safe default. Each of the first four branches requires maintainer input before any later value is considered. An absent `decision_policy` inherits the valid global value, while a present malformed, unknown, or wrong-typed override resolves safely to `manual`; missing or malformed global policy likewise resolves to `manual` without raising. `manual` preserves explicit per-proposal human confirmation. `delegated` may own a non-drift decision only when the two exact-byte signals defined above agree and every floor is clear. `consensus` additionally requires evidence from the separately ratified core consensus tier; until that tier and its evidence are available, `consensus` requires maintainer input and arms no unattended decision. In Increment 2 every drift-origin proposal resolves to human regardless of either configured value. `drift_acceptance_mode` does not exist in this increment, so no configuration can delegate drift acceptance.
