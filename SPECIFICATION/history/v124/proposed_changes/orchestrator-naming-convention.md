...
- "implementation plugin" / "orchestrator plugin" CLASS-CONCEPT terminology, ONLY where it is generic class-concept prose that does NOT denote the validated repo-class taxonomy key -> "orchestrator plugin" (consistent with the `### Implementation plugin ecosystem` -> `### Orchestrator plugin ecosystem` H3 retitle).

**KEEP rules applied (NOT changed):**

- The literal `templates/impl-plugin/` PATH token wherever it names the copier directory (renaming the dir is a separate copier-channel migration; the string must still resolve). Only the surrounding repo-name glob nouns were reworded, never the path token.
- The repo-CLASS taxonomy key `impl-plugin` -- the literal value validated by `livespec_dev_tooling/fleet/contract.py` `REPO_CLASSES` and carried as `fleet-manifest.jsonc` `"class"`, plus the `_TEMPLATE_BORN_CLASSES` (copier-template-born) concept that key denotes. Renaming this value spec-only would create spec<->code/data drift and break the family conformance check until a coordinated dev-tooling release + re-vendor; that is a separate cross-repo lockstep slice. KEPT sites: the fleet-manifest repo-class enum list (`core / enforcement-suite / impl-plugin / driver-plugin / library`); "the impl-plugin copier template"/"the impl-plugin scaffold"/"every impl-plugin repo"/"each impl-plugin's `.github/workflows/`" (the template-born class). When a site was genuinely ambiguous between "class taxonomy key" and "repo-naming term", it DEFAULTED to keeping `impl-plugin` (the drift-free choice).
...
**Edit 1 — `SPECIFICATION/non-functional-requirements.md`.** The `### Implementation plugin ecosystem` section is retitled `### Orchestrator plugin ecosystem` and its three paragraphs redefine the convention (full `livespec-orchestrator-<ledger>[-<loop>]` definition naming both axes, the two reference-orchestrator renames, and the explicit `livespec` core-name exemption). The remaining forward-looking `livespec-impl-*` glob / `livespec-impl-<X>` notation / concrete-name references throughout the file — in §"Boundary", §"Shared content provenance", the Codex-dogfooding sections, §"Toolchain pins" (the `bd` entry), §"CI telemetry export", §"Orchestrator contract delegation", §"Shared content sync — copier template", §"Shared code sync — livespec-dev-tooling", §"Prose conventions" (the version-prefix example), §"Workflow discipline — spec-side changes" (the commit-refuse-hook + bootstrap + CI-merge-gate family-wide rules), and the per-orchestrator beads-tenant pointer in the fleet-manifest section — are all renamed per the RENAME rules. The generic class-concept prose ("orchestrator plugin") is reworded where it does not denote the repo-class taxonomy key. Every `templates/impl-plugin/` path token, the repo-CLASS taxonomy key `impl-plugin` (the fleet-manifest enum list + the copier-template-born class nouns), and the `livespec-impl-plaintext` PR #26 citation are KEPT verbatim per the KEEP rules above.
