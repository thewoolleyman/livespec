...
Every Python wrapper under `.claude-plugin/scripts/bin/<sub-command>.py`, including the non-LLM spec-governance control wrapper, MUST accept `--project-root <path>`, defaulting to `Path.cwd()`. Each wrapper adds its own flags above that baseline.
...
| `1` | internal bug — uncaught exception; full traceback on stderr (raw under the implicit supervisor form, structlog-structured under the explicit bug-catcher) |
...
Domain errors flow as `Failure(<LivespecError>)` payloads on the railway; the supervisor `main()` pattern-matches and lifts the exit code from the error's `exit_code` ClassVar. Bugs propagate as raised exceptions and surface per the supervisor bug contract (`non-functional-requirements.md` §"Supervisor discipline"), resulting in exit 1.
...
- **Drift-capture CLI** — a capture interface. The spec-reader CLI and the propose-change CLI are injected as references. Routes impl → spec drift to propose-change. Filing is a machine path; acceptance is human by default and MAY be owned by the consensus tier only under an explicit per-repo opt-in via `spec_governance.drift_acceptance_mode`, per the two-flow doctrine of `spec.md` §"Contract + reference implementations architecture".
...
The `propose-change`, `revise`, and `critique` sub-commands accept `--spec-target <path>` to scope their operation to one specific spec tree. `--spec-target <project-root>/SPECIFICATION/templates/<name>/` targets a sub-spec tree; `--spec-target <project-root>/SPECIFICATION/` targets the main spec; etc. The `doctor` sub-command ALSO accepts `--spec-target <path>`, which selects the tree doctor treats as the MAIN spec root; its sub-spec enumeration remains internal, walking `<spec-target>/templates/<name>/` from whichever root is selected (see §"Per-sub-spec doctor parameterization"). A relative `--spec-target` or `--project-root` is anchored to the working directory, matching `revise` and `propose-change`; both resolve to absolute paths so that per-tree containment computations are well defined.
...
The `doctor` operation prose, through the active Driver binding, MUST offer a per-finding disposition dialogue for every non-`pass` finding whose shared `requires_doctor_disposition_input` predicate is true, regardless of which phase produced it (static phase `fail` or `warn`, or any of the four LLM-driven phase categories — operation-prose objective, template-extension objective, operation-prose subjective, template-extension subjective). The disposition menu for every finding that reaches dialogue MUST present at minimum these five options:
...
3. Directs the user to commit-into-a-feature-branch per the workflow discipline. For `<spec-root>/` paths the narration MAY additionally offer discarding unintentional edits (`git checkout -- <files>`). For `plan/` paths it MUST NOT: an uncommitted `plan/` edit may be write-once research or the write-once epic anchor whose only copy is the working tree — unpublished evidence of a plan — so a discard suggestion against it risks destroying the very artifact the finding exists to protect. The `plan/` narration leads with the commit path and does not present discard as a symmetric option.
...
A `wiring-completeness-cross-repo` check (a repo-tier check: it reads sibling repos' tracked task-runner wiring, never any orchestrator-private state) MUST walk every registered sibling repo (per the `livespec-dev-tooling` and `livespec-runtime` and `livespec-orchestrator-*` registries declared in `non-functional-requirements.md` §"Shared code sync — livespec-dev-tooling" / §"Shared runtime — livespec-runtime" / §"Sibling spec ownership"), resolve the tracked static target inventory reached by its one-line `just check` recipe, compute the canonical-set difference, and fire `fail` on any inventory lacking any canonical slug. The check MAY use a sibling's `local_clone` path when configured or fall back to GitHub queries against the sibling's default-branch justfile and referenced inventory. It MUST NOT infer canonical membership from the recipe body. The invariant covers the adversarial-drift case in which a consumer drops both a canonical slug AND `check-aggregate-completeness` from its inventory in the same change — the in-repo gate cannot catch that combination (the gate is gone before it next runs), but the cross-repo backstop can. The canonical set the check compares against is the `livespec_dev_tooling.canonical_checks` source of truth and the three-layer wiring-completeness invariant defined in `non-functional-requirements.md` §"Shared code sync — livespec-dev-tooling".
...
Install verification. A project is correctly provisioned only when, for EVERY `true`-valued key in its committed `enabledPlugins`, `~/.claude/plugins/installed_plugins.json` holds an entry for that plugin whose `projectPath` equals the project root. A `false` value is an explicit disable and MUST NOT be read as requiring an install record; a checker that enumerates keys without inspecting values reports a false MISSING for every deliberately disabled plugin. Note that this exemption governs DIAGNOSIS, not the verdict: a `false`-valued entry neither requires an install record NOR references its marketplace, so disabling the only enabled plugin of a still-declared marketplace fails the coverage requirement below by design — a governed project with a required plugin switched off is not correctly provisioned, however deliberately it was switched off. Enabled-without-installed, and installed-against-a-different-`projectPath`, are both defective states that any provisioning or currency tooling MUST detect and report loudly; neither may be inferred from a command's exit status. Verification MUST NOT pass vacuously, and vacuity has three levels. (i) EMPTY — the check enumerates enabled plugins, so it succeeds on zero iterations whenever that set is empty. (ii) ALL-FALSE — a `false` value is an explicit disable rather than an enablement, so a non-empty `enabledPlugins` containing no `true`-valued entry is equally vacuous; values MUST be JSON booleans, and any other type is itself defective. (iii) PARTIALLY STRIPPED — an enablement block that has lost some keys while `extraKnownMarketplaces` still declares their marketplaces passes a whole-set check on whichever entry survives. A configuration declaring `extraKnownMarketplaces` MUST therefore fail verification unless EVERY declared marketplace is referenced by at least one `true`-valued entry in `enabledPlugins`. All three are reachable through ordinary operation, as observed on Claude Code v2.1.215: `claude plugin uninstall <plugin> -s project` has been observed to rewrite the project's COMMITTED `.claude/settings.json` and empty `enabledPlugins` rather than merely dropping the install record; `claude plugin disable <plugin> -s project` has been observed to write `false` for that plugin in the same committed file, retaining the key and leaving the install record intact (its `--all` form cannot be combined with `--scope`); and hand edits or merge resolutions produce the partial case. The structural cause is that verification derives its expected set from the same mutable file each of these mutates, so any corruption leaving one `true`-valued entry survives a naive predicate. An unintended uninstall or disable therefore dirties governed config and disarms verification in a single step; the committed settings file MUST be restored from version control before re-verifying — unless the removal is a deliberate de-adoption, in which case the `enabledPlugins` and `extraKnownMarketplaces` blocks MUST be removed together and the removal committed. Reference implementation of the derive-and-install flow ONLY: `livespec_dev_tooling/fleet/ensure_plugins.py`, which derives the marketplace and plugin set from the committed `.claude/settings.json` and issues `claude plugin install ... -s project` followed by `claude plugin update ... -s project` for each. That tool does NOT yet satisfy this invariant: it determines success from each command's exit status and reads no install record, which is exactly the inference this paragraph forbids — so a scoped update that acted on another project's record would be reported as success. Adding the `projectPath` verification AND the non-vacuity guard, with their covering tests, is a committed follow-up owned by the `livespec-dev-tooling` sibling per `non-functional-requirements.md` §"Sibling spec ownership", on the same pattern this contract already applies to the copier-scaffold enforcement check. The non-vacuity half is not incidental: that tool derives its command set from the same committed settings file, so an empty or all-`false` `enabledPlugins` derives zero commands and exits 0 — the identical vacuous pass, in the tool this paragraph cites.
...
Plugin uninstall and disable remain Claude Code platform behaviors outside this contract, and plugin update mechanics are likewise platform-owned; all three are constrained here only by the install-verification invariant above. A scoped update's exit status does not establish which project's record it touched, and an uninstall's or disable's effect on the committed settings file does not exempt that file from the invariant.

### Driver-shipped hooks

The Claude Code Driver plugin (`livespec-driver-claude`) SHIPS an agent-runtime hook bundle at `.claude-plugin/hooks/`: a `hooks.json` registration plus one executable hook entry per hook. The auto-memory redirect and plan-persistence WARN hooks MAY be shell scripts invoked by the harness as `"${CLAUDE_PLUGIN_ROOT}/hooks/<name>.sh"` or Python scripts invoked as `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/<name>.py"`; the cross-Driver no-shadow-ledger hook is a Python script invoked as `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/no_shadow_ledger.py"` so its one neutral body can ship byte-identically in both Drivers' bundles. The bundle is Driver-owned runtime mechanics per the Driver-binding partition (`spec.md` §"Contract + reference implementations architecture"): this section states the required hook surfaces and their behavioral disciplines; the script implementations and their tests live in the Driver repo. The bundle carries four hooks:
...
### Spec-governance input envelopes

Spec-governance invocation envelopes are LLM-layer inputs to operation prose; they MUST NOT be passed through to the Python mutation wrappers. A propose-change envelope MAY carry `mode: interactive | batch` and `spec_target`. When its effective mode is `batch`, it MUST carry non-empty `intent` and `topic`, plus an explicit `relationship: align | modify-to-accommodate | supersede` for every surfaced in-flight item unless `effective_in_flight_alignment` is `default-align` and no conflict exists. A critique envelope MAY carry `mode: interactive | batch` and `spec_target`; effective batch mode MUST carry a non-empty `target`, using the same field name as the operation prose's `Capture critique target` dialogue. Complete batch envelopes are consumed without re-asking the supplied intent, topic, target, or relationship. Incomplete, ambiguous, internally contradictory, or design-record-conflicting envelopes MUST escalate before a mutating wrapper is invoked and MUST leave the spec tree unchanged; interactive mode preserves every unanswered question.

A doctor disposition envelope is an invocation-scoped object map from canonical finding `check_id` to one of `fix-now | capture-as-work-item | propose-change | defer | dismiss`. It has precedence over the global `doctor_dispositions` map only after hard floors. A mapped `fix-now` is executable only when the corrective action is mechanically available under the doctor menu contract. A mapped `capture-as-work-item` MUST still satisfy the active orchestrator's invocation-scoped store-write consent discipline. A mapped `propose-change` threads `proposed_change_hint` and `spec_target` into critique/propose-change: a complete batch authoring path consumes them; otherwise they pre-fill rather than suppress the interactive dialogue.

A proposed-change file MAY carry `decision_policy: manual | delegated | consensus` in YAML front matter. The field applies to that file only and has precedence over the global `spec_governance.revise_decision_mode` value only after every effective-policy hard floor. An absent field inherits the valid global value; a present malformed, wrong-typed, or unknown field resolves to `manual` without raising. The field MUST NOT alter ratification-review evidence requirements or drift-origin authority. This wire surface and its stronger-than-manual delegated composition are grounded in repo `thewoolleyman/livespec`, `plan/spec-side-autonomy/research/brainstorm.md`.

A proposed-change file MAY carry `spec_pr_merge_policy: manual | auto-on-green` in YAML front matter. The field applies to that file only and has precedence over the global `spec_governance.spec_pr_merge` value. An absent field inherits the valid global value; a present malformed, wrong-typed, or unknown field resolves to `manual` without raising.

### Spec-governance control wrapper

The deterministic reference control CLI is `.claude-plugin/scripts/bin/spec_governance.py`. It is a config/control wrapper, not a ninth LLM-driven lifecycle operation. `--project-root <path> --show-effective` emits the committed manifest together with declared and effective values as JSON. `--show-effective` additionally accepts two additive, optional arguments that leave its existing zero-argument behavior unchanged: a repeatable `--proposal-stem <stem>` and a boolean `--pr-effective-policy`. Passing `--pr-effective-policy` (with zero or more `--proposal-stem` values) additionally emits, alongside the existing global effective value, the pull-request effective policy computed per `spec.md` §"Spec-governance policy settings" `effective_spec_pr_merge`(b); omitting `--pr-effective-policy` preserves the current global-only output exactly. `--proposal-stem` supplied WITHOUT `--pr-effective-policy` is a usage error: the CLI MUST reject it and exit 2 via `IOFailure(UsageError)`, mirroring the existing `--skip-pre-check` / `--run-pre-check` mutually-exclusive-argument convention, because a stem list with no fold requested is a caller mistake rather than a case worth silently ignoring. `--project-root <path> --action <action>` performs exactly one allowlisted policy edit, `--project-root <path> --journal-event-json <path>` validates and appends one policy event, and `--check-default-block <path>` verifies that the commented default block in the named file still matches the committed manifest's keys and safe defaults, emitting a structured `spec-governance-default-block-ok` payload on agreement and failing via `UsageError` naming the drifted keys otherwise. The four modes are mutually exclusive; exactly one MUST be supplied. The committed manifest `.claude-plugin/scripts/_vendor/livespec_runtime/api_configurable_keys.json`, owned by the vendored `livespec_runtime` package, is the single declarative source; `.claude-plugin/scripts/livespec/spec_governance/registry.py` re-exports it as a compatibility projection (`ConfigKey` / `CONFIG_KEYS`) preserving the former core import path. A row carries the key, value type, safe default, per-proposal-override support, and allowed values.

The action grammar is:

- `set-propose-change-mode:<interactive-or-batch-or-clear>`;
- `set-critique-mode:<interactive-or-batch-or-clear>`;
- `set-in-flight-alignment:<prompt-or-default-align-or-clear>`;
- `set-doctor-disposition:<doctor-check-id>:<fix-now-or-capture-as-work-item-or-propose-change-or-defer-or-dismiss-or-clear>`;
- `set-revise-decision-mode:global:<manual-or-delegated-or-consensus-or-clear>`;
...
- `set-ratification-review:global:<manual-spawn-or-auto-spawn-or-clear>`;
- `set-ratification-review:proposal:<proposal-stem>:<manual-spawn-or-auto-spawn-or-clear>`;
- `set-ratification-reviewer-model:<model-or-clear>`;
- `set-spec-pr-merge:global:<manual-or-auto-on-green-or-clear>`; and
- `set-spec-pr-merge:proposal:<proposal-stem>:<manual-or-auto-on-green-or-clear>`.

`clear` removes only the selected global value, map entry, proposal override, or model designation so the safe default is inherited. The CLI MUST validate the doctor check-id pattern, proposal stems, model pattern, and every value allowlist; MUST atomically replace only the selected config or front-matter value while preserving unrelated JSONC keys/comments and Markdown body bytes; and MUST NOT invoke a lifecycle operation, spawn a review, disposition a finding, transition git or pull-request state, or mutate spec/history/work-item state. A policy edit is never the state transition it configures.

The append-only journal lives at `<project-root>/tmp/livespec-spec-governance-journal.jsonl`, and the control CLI is its sole core writer. An authoring auto-consumption event names the operation, governing key, effective source, SHA-256 digest of the input envelope's UTF-8 JSON bytes, and outcome without duplicating raw intent. A doctor event carries the finding check id, spec root, optional path/line, SHA-256 digest of the UTF-8 finding message, selected verb, governing key, effective source, and outcome. A ratification event carries the proposal stem/content digest, reviewer identity/model, verdict, effective source, and escalation reason when applicable. A `revise_decision` event carries the proposal stem/content digest, effective mode/source, selected decision, decider model/identity when applicable, ratification-review outcome, final outcome, and escalation reason when applicable; for a drift-origin decision it additionally carries the effective `drift_acceptance_mode`, its effective source, and a SHA-256 digest of the consensus-tier evidence; it MUST NOT carry raw proposal, resulting-file, or consensus-evidence content. An `auto-on-green` merge-registration attempt MUST append an event naming `spec_pr_merge`, the pull-request identity, the derived proposal-stem set, the resolved pull-request effective policy and its effective source, the registration result, the required-gate state, and the final merge evidence when available; it MUST NOT carry raw proposal or resulting-file content, following the same digest-only discipline as every other event. The GitHub pull-request timeline MAY serve as the durable final-evidence leg for this event; the journal is the decision gate — it records which setting governed the registration attempt and refuses to proceed when it cannot be written — rather than its durable archive. Every automated `revise_decision` event MUST append before mutation. Journal failure requires human input and prevents the governed mutation, disposition, or merge registration.

Question enforcement exports one predicate per authoring operation; the predicate is true exactly when human input remains required after hard floors and effective-policy resolution. `requires_doctor_disposition_input` is true for a hard-floor finding, unmapped or invalid check id, unavailable mapped verb, absent downstream consent, journal failure, or failed action, and false only when one valid executable default owns the finding. `requires_revise_decision_input` is true for `manual`, any design-record or review floor, a drift floor UNLESS `effective_drift_acceptance_mode` validly owns that decision on present, fresh and conforming consensus evidence, missing or unavailable exact-byte decision evidence, unavailable consensus evidence, disagreement, or journal failure; it is false only when one valid automated mode owns the exact proposal decision. `awaits_ratification_review` remains true until matching no-blockers evidence exists; auto-spawn transfers ownership of satisfying the predicate to the operation but does not retire the gate before evidence arrives. `awaits_manual_spec_pr_merge` is true exactly when the pull-request effective `spec_pr_merge` policy is `manual` or a registration-time hard floor prevents registration, and false once an eligible pull request is validly registered for merge-when-green — including while its required checks are still pending or red. Revise enforcement and any awareness surface MUST consume `requires_revise_decision_input` rather than re-derive whether the human handoff remains live. Any awareness surface MUST consume the shared predicates rather than re-derive them; core does not normatively name or own an orchestrator awareness surface.

### Spec pull-request merge-registration mechanics

The floors and gates `spec.md` §"Spec-governance policy settings" `effective_spec_pr_merge` and `non-functional-requirements.md` §"Spec pull-request auto-merge requirement" depend on are defined here.

Registration-time hard floors. ALL of the following MUST hold before an `auto-on-green` pull request registers for merge-when-green: the spec change has been ratified through revise; an open, non-draft pull request targets the governed default branch; the pull-request author satisfies the workflow's configured human/release-App identity allowlist; no `do-not-merge` label is present; no unresolved changes-requested review exists at registration time; the repository permits rebase merge; and the spec-governance policy journal is writable.

Completion-time host guarantees. These are owned by branch protection and host merge-queue semantics AFTER registration, never by this policy: the required all-green status check MUST succeed before merge; the policy MUST NOT bypass, weaken, relabel, or replace a required status check; it MUST NOT use squash or merge-commit mode; and it MUST NOT merge a red pull request. A hosting service that cannot provide merge-when-green semantics MUST require human input instead of polling and force-merging.

Non-spec eligibility gates. Every pull request — spec-touching or not — passes through these gates independently of `spec_pr_merge`: draft state, the `do-not-merge` label, the human-author allowlist, and the release-please App exception on a `release-please--` branch. `spec_pr_merge` governs ONLY whether a ratifying spec pull request additionally clears the conservative fold; it neither replaces nor loosens these gates.

### Ratification-review evidence

Every `accept` or `modify` decision in revise input MUST carry reviewer model and identity, a separate/read-only declaration, a UTC-seconds `reviewed_at` timestamp, the literal verdict `NO BLOCKERS`, the proposal stem, and `content_digest`. The digest is the lowercase SHA-256 hex digest of `LP(P) || LP(path_1) || LP(content_1) || ... || LP(path_n) || LP(content_n)`, where `P` is the raw proposed-change file bytes; `resulting_files[]` entries are ordered by ascending bytewise unsigned-byte comparison of the UTF-8 encoding of their exact schema `path` strings; each `path_i` and `content_i` is the UTF-8 encoding of the exact JSON string value; and `LP(x)` prefixes `x` with its byte length as an unsigned 64-bit big-endian integer. No path normalization is applied because the schema's spec-target-relative path convention is canonical.

`bin/revise.py` MUST reject absent, blocking, malformed, or digest-mismatched evidence with the existing retryable validation exit `4` before any write. Evidence is evaluated against the reassembled bytes when the decision is applied, so a fresh conforming review supersedes stale evidence while blockers remain non-waivable. The paired revision record's required `## Ratification Review` section preserves every evidence field durably. For an automated revise decision, the paired revision record MUST additionally preserve the effective decision mode/source and the delegated or consensus evidence needed for durable audit.
