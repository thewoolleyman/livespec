...
The four work-item structural invariants codified in `contracts.md` §"Doctor cross-boundary invariants" (`gap-tracking-one-to-one`, `no-orphan-blocker`, `no-stale-gap-tied`, `no-duplicate-gap-id`) are cross-boundary checks: each queries the active impl-plugin's machine surface (the `list-work-items --json` thin-transport skill, plus an in-process dry-run of `capture-impl-gaps` for snapshot gap-id enumeration) rather than livespec-core-internal state. The cross-boundary nature MUST be reflected in the checks' exit-code derivation: a missing or unresponsive impl-plugin query surface MUST surface as a `fail` Finding (lifting the wrapper exit to non-zero) rather than as a silent skip. The check IDs MUST follow the existing slug convention (lowercase-hyphenated, per §"File naming and invocation"): `gap-tracking-one-to-one`, `no-orphan-blocker`, `no-stale-gap-tied`, `no-duplicate-gap-id`.

The doctor static invariant catalog MUST be enumerable from a single named registry inside `livespec.doctor.static` so contributors and tools can list all checks programmatically; the registry contract follows the existing slug↔module-name mapping pattern stated in §"File naming and invocation". Every check ID covered by `contracts.md` §"Doctor cross-boundary invariants" — the four work-item invariants above and `contract-version-compatibility` — MUST appear in this registry.
