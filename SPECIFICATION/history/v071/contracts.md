...
| `resolve-cross-repo-deps` | (none) | `--filter <ref>`, `--project-root <path>` |
...
Cross-plugin invocation (doctor invoking the active impl-plugin's `list-memos`, the project-local loop driver invoking `livespec:next` and `<impl-plugin>:next`, etc.) MUST go through the skill namespace (e.g., `/livespec-impl-plaintext:list-memos --filter=untriaged --json`), NOT through a direct CLI path. This makes the contract surface discoverable, namespaced, and agent-aware uniformly.
...
The catalogue MUST include the five work-item invariants, the contract-version-compatibility invariant, and the cross-repo-work-item-dep-wellformedness invariant below.
...
### `cross-repo-work-item-dep-wellformedness`

The `cross-repo-work-item-dep-wellformedness` doctor invariant reads each open work-item's `depends_on_external` field from the active impl-plugin's work-items store (via `<impl-plugin>:list-work-items --json`) and enforces four structural derivations against the `cross_repo_targets` block from `.livespec.jsonc` (the block defined by §"Cross-repo work-item dependencies"):

1. **Reference syntax** — every `depends_on_external[]` entry MUST match `<repo>:<work-item-id>`, where `<repo>` is a non-empty string of `[a-z0-9-]` characters and `<work-item-id>` is a non-empty string. Malformed references MUST fire `fail`.
2. **Repo resolvability** — every `<repo>` segment MUST name a key in `.livespec.jsonc`'s `cross_repo_targets` block. Unresolvable references MUST fire `fail`.
3. **Cache presence** — every reference in `depends_on_external` MUST have a corresponding entry in the work-item's `external_dep_cache` object. Missing entries MUST fire `warn` with narration directing the user to run `/livespec:resolve-cross-repo-deps`.
4. **Cache freshness** — every `external_dep_cache[ref].checked_at` value MUST be no older than the configurable staleness threshold (`.livespec.jsonc` key `doctor.cross_repo_cache_staleness_hours`, default `24`). Older entries MUST fire `warn` with the same corrective-action narration.

Doctor MUST NOT mutate the work-item store as part of this invariant — stale or missing cache entries surface as findings whose corrective action is the resolver skill. This preserves doctor's read-only contract per §"Doctor cross-boundary invariants" intro.

This invariant is STRUCTURAL per the binary, contract-level, mechanically-checkable principle codified in this section's intro; it does NOT rank or judge work-item readiness, only the well-formedness of the cross-repo dependency machinery.
...
After install, `livespec` exposes nine slash commands, namespaced under the plugin name: `/livespec:seed`, `/livespec:propose-change`, `/livespec:critique`, `/livespec:revise`, `/livespec:doctor`, `/livespec:prune-history`, `/livespec:help`, `/livespec:next`, `/livespec:resolve-cross-repo-deps`. Renaming any command requires a propose-change cycle.
...
## Cross-repo work-item dependencies

The cross-repo work-item dependency mechanism is parallel-and-complementary to the cross-repo version coordination of §"Cross-repo coordination — pin-and-bump": version pin-and-bump is the coarse-grained release-level coordination; the work-item dependency mechanism is the fine-grained per-work-item coordination for cases when an in-flight work-item in one repo is blocked by a specific in-flight work-item in another repo. The two mechanisms operate at different granularities and are independent.

### `cross_repo_targets` block in `.livespec.jsonc`

Projects participating in cross-repo work-item dependency coordination MUST declare a top-level `cross_repo_targets` block in `.livespec.jsonc`. The block is an object whose keys are short repo names (used as the `<repo>` segment in `depends_on_external` references) and whose values are objects with exactly two required fields:

- `path` — string, filesystem path to the external repo's project root. MAY be absolute or relative to the containing project's root. The path MUST resolve to an existing directory containing a `.livespec.jsonc` file; doctor's `cross-repo-work-item-dep-wellformedness` invariant MUST verify this.
- `plugin` — string, the impl-plugin name (e.g. `livespec-impl-plaintext`) to invoke when querying the external repo's work-items. The `plugin` field MUST always name the impl-plugin (the one exposing `list-work-items`), NEVER the spec-side plugin. Cross-repo work-item lookups always route through impl plugins per §"Implementation-plugin contract — the 9-skill surface".
...
The `cross_repo_targets` block is OPTIONAL at the top level — projects with no cross-repo work-item dependencies omit it entirely. Git-URL-based remote resolution is explicitly deferred to a future propose-change cycle (in scope at this contract: filesystem paths only).
...
### `/livespec:resolve-cross-repo-deps` skill

The `/livespec:resolve-cross-repo-deps` skill is the SOLE place in the system that performs cross-repo reads for work-item dependency resolution. It is a thin-transport skill per §"Thin-transport skill doctrine", backed by a Python wrapper at `.claude-plugin/scripts/bin/resolve_cross_repo_deps.py`.

The skill MUST:

1. Read the active impl-plugin's work-items store via `/<impl-plugin>:list-work-items --json` (existing thin-transport surface; cross-plugin invocation per §"Thin-transport skill doctrine").
2. Aggregate the union of all `depends_on_external` references across all open work-items.
3. For each external reference `<repo>:<work-item-id>`, look up the target repo's `path` and `plugin` from the `cross_repo_targets` block in `.livespec.jsonc`. When `<repo>` is not configured, the skill MUST surface a structured finding (NOT a hard error) and skip that reference.
4. For each resolvable external reference, invoke the target repo's impl-plugin via the cross-plugin namespace (e.g., `/livespec-impl-plaintext:list-work-items --filter=<id> --json --project-root <target-path>`) to retrieve the external work-item's open/closed status.
...
The resolver MUST accept `--project-root <path>` (uniform with all wrappers per §"Wrapper CLI surface") and MAY accept `--filter <work-item-ref>` to scope to a single reference. The resolver MUST NOT mutate any spec-side state; it touches ONLY impl-side work-item stores (local for the write-back, remote-via-skill for the read).

The resolver MAY be invoked: on-demand by the user, as a doctor pre-step (per `spec.md` §"Sub-command lifecycle" pre-step extension), or by the project-local Layer 3 loop driver. The resolver is lifecycle-exempt from doctor's pre-step/post-step LLM phases (it is mechanical resolution, not authored content).
...
The work-item record contract surfaced by `list-work-items`, `capture-work-item`, and `next` MUST support two OPTIONAL fields supporting cross-repo work-item dependency coordination (see §"Cross-repo work-item dependencies" for the broader mechanism):

- `depends_on_external` — array of reference strings. Each reference MUST be syntactically `<repo>:<work-item-id>`, where `<repo>` MUST name a configured cross-repo target in `.livespec.jsonc`'s `cross_repo_targets` block. When the field is absent or empty, the work-item has no declared cross-repo dependencies.
- `external_dep_cache` — object keyed by reference string (from `depends_on_external`); each value MUST be an object with two fields: `status` (string enum, `"open"` or `"closed"`) and `checked_at` (string, ISO 8601 with trailing `Z` per existing front-matter conventions). The cache is populated and refreshed only by the `/livespec:resolve-cross-repo-deps` skill; the work-item store itself MUST NOT mutate this field outside the resolver.

The impl-side `next` ranker MUST honor `external_dep_cache[ref].status == "open"` for any `ref` declared in `depends_on_external` as a hard blocker: work-items with at least one unsatisfied external dep MUST be excluded from the ranked candidates list entirely. The urgency enum (`high`, `medium`, `low`) is unchanged; blocked work-items simply do not appear as candidates. When `external_dep_cache` is missing for a declared `ref`, the ranker MUST treat the dep as unsatisfied (conservative default) and surface a warning that the resolver has not run.
