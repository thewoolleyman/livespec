...
**A function whose name carries a single leading underscore is NOT public for the purposes of this rule, regardless of its presence in `__all__`** — per the private-helper definition in §"Typechecker rule set", which this rule adopts rather than restating. `__all__` membership alone is not sufficient to make a name public here, because consumers legitimately list private helpers in `__all__` to make them importable by their tests.
