...
The deterministic reference control CLI is `.claude-plugin/scripts/bin/spec_governance.py`. It is a config/control wrapper, not a ninth LLM-driven lifecycle operation. `--project-root <path> --show-effective` emits the declarative registry manifest together with declared and effective values as JSON. `--show-effective` additionally accepts two additive, optional arguments that leave its existing zero-argument behavior unchanged: a repeatable `--proposal-stem <stem>` and a boolean `--pr-effective-policy`. Passing `--pr-effective-policy` (with zero or more `--proposal-stem` values) additionally emits, alongside the existing global effective value, the pull-request effective policy computed per `spec.md` §"Spec-governance policy settings" `effective_spec_pr_merge`(b); omitting `--pr-effective-policy` preserves the current global-only output exactly. `--proposal-stem` supplied WITHOUT `--pr-effective-policy` is a usage error: the CLI MUST reject it and exit 2 via `IOFailure(UsageError)`, mirroring the existing `--skip-pre-check` / `--run-pre-check` mutually-exclusive-argument convention, because a stem list with no fold requested is a caller mistake rather than a case worth silently ignoring. `--project-root <path> --action <action>` performs exactly one allowlisted policy edit, `--project-root <path> --journal-event-json <path>` validates and appends one policy event, and `--check-default-block <path>` verifies that the commented default block in the named file still matches the declarative `ConfigKey` registry, emitting a structured `spec-governance-default-block-ok` payload on agreement and failing via `UsageError` naming the drifted keys otherwise. The four modes are mutually exclusive; exactly one MUST be supplied. The single declarative `ConfigKey` registry and committed manifest `.claude-plugin/scripts/livespec/spec_governance/api_configurable_keys.json` are co-authoritative; a row carries the key, value type, safe default, per-proposal-override support, and allowed values.
