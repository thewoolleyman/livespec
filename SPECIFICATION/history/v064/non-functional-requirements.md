...
`.claude/skills/loop/SKILL.md` (the project-local orchestration driver, defined in detail by the `orchestration-layer-and-memo-paradigm` propose-change cycle) is exempt from the drift check because per-project tuning of that skill is expected and load-bearing.
