...
policy-governed spec-governance loop — human-gated by default and at every
doctrine floor — maintains `SPECIFICATION/`; and the
...
The built-in `livespec` template's manifest MUST declare only the six markdown files (`spec.md`, `contracts.md`, `constraints.md`, `non-functional-requirements.md`, `scenarios.md`, `README.md`). **Mermaid is livespec's standard and default diagram format.** Diagram types (sequence, class, state, ER, flowchart/dependency, C4, and the rest) MUST be authored as fenced ` ```mermaid ` blocks inside `kind: markdown` spec files: they require NO manifest entries, NO render command, and NO history special-casing — they are markdown content that GitHub, IDEs, and markdown previewers render natively and that LLMs read natively, and they follow their markdown file's lifecycle. The separate `livespec-with-diagrams` template variant seeds these Mermaid diagram conventions and example fenced blocks into its spec files; it differs from the built-in `livespec` template in that seeded content and in declaring its `spec_files` manifest explicitly (`template_format_version: 2`), which makes a project using it subject to §"Spec-tree path closure" while the built-in `livespec` template's implicit manifest is exempt. The built-in opinion stays narrow; the manifest is the extension point that lets custom templates add markdown files without forking the entire template surface.
...
- **History snapshots** (`<spec-target>/history/vNNN/` materialization during revise) capture the WHOLE spec tree: every file under the spec root EXCEPT the `history/`, `proposed_changes/`, and `templates/` sibling subdirectories is snapshotted byte-for-byte into the new revision directory. Under an explicit manifest the snapshotted set is exactly the manifest's markdown files, since the spec root is closed over them (§"Spec-tree path closure"); an externally-rendered image lives outside the spec root, is not carried by the snapshot, and does not render from an old revision. Fenced Mermaid blocks inside `kind: markdown` files need nothing additional — any `history/vNNN/` snapshot of the markdown renders them natively.
...
- `revise` MUST resolve decision ownership through `effective_revise_decision_mode`. A cited design-record contradiction or a missing/unreachable design record MUST escalate to the maintainer before any configured decision mode is considered. `revise` MUST NOT ratify a resolution that contradicts a cited design record unless the maintainer explicitly acknowledges the contradiction in the revision record's decision (the `## Decision and Rationale` body naming the departed-from record and the deliberate departure).
...
**No ratification with zero independent review.** Before `revise` applies an `accept` or `modify` decision, every proposed-change file MUST receive an independent, read-only adversarial review by a separately spawned agent with the project's designated adversarial-review capability against the exact proposal and resulting-file bytes to be ratified. The review MUST cover replacement-target fidelity, design-record fidelity, drift-sweep completeness, ratification mechanics, cross-repo consistency, claims that expire at ratification, negative assertions about sibling-owned surfaces, and clause lockstep, and its verdict MUST be `NO BLOCKERS`. A blocker, missing or unavailable designated reviewer, stale or malformed review evidence, or disagreement between the review and the decision MUST require maintainer input and MUST NOT be self-waived. A `modify` decision MUST review the final modified content; a `reject` decision MAY proceed without review because it ratifies no content. The livespec fleet designates Fable for this role, grounded by the maintainer's 2026-07-04 declaration in repo `thewoolleyman/livespec`, `plan/spec-side-autonomy/research/brainstorm.md`.

**Delegated revise is stronger than manual revise.** For a non-drift proposal whose effective decision mode is `delegated`, the operation MUST automatically spawn the separate adversarial reviewer above and MUST also obtain an independent delegated decider's acceptance of the exact same final bytes. For that path, the automatic-spawn requirement governs review initiation even when `effective_ratification_review` would otherwise resolve to `manual-spawn`; the evidence floor remains unchanged. Ratification proceeds only when the reviewer returns literal `NO BLOCKERS` and the delegated decider accepts; disagreement, an unavailable participant, stale or mismatched evidence, or journal failure MUST escalate to the maintainer. A `modify` decision binds both signals to the converged final bytes. This composes two independent AI judgments instead of relying on a maintainer to remember the review discipline, so the delegated path is strictly stronger than the manual path it supplements. The governing design record is repo `thewoolleyman/livespec`, `plan/spec-side-autonomy/research/brainstorm.md`.
...
### Spec-governance policy settings

Spec-side dialogue and review automation is configured by independent keys under the core-owned top-level `.livespec.jsonc` block `spec_governance`; there is no master autonomy switch. One declarative manifest row per key MUST drive parsing, type-strict coercion, allowed-value diagnostics, and control-surface rendering. Missing configuration, a missing block or key, malformed JSONC, an unknown value, or a wrong-typed value MUST resolve to the row's safe default without raising.

The manifest rows are:

| Key | Shape | Safe default | Per-proposal override |
|---|---|---|---|
| `propose_change_mode` | `interactive | batch` | `interactive` | no |
| `critique_mode` | `interactive | batch` | `interactive` | no |
| `in_flight_alignment` | `prompt | default-align` | `prompt` | no |
| `doctor_dispositions` | object map from canonical `doctor-*` check id to `fix-now | capture-as-work-item | propose-change | defer | dismiss` | `{}` | no |
...
| `ratification_review` | `manual-spawn | auto-spawn` | `manual-spawn` | `ratification_review_policy` front matter |
| `ratification_reviewer_model` | non-empty string matching `^[A-Za-z0-9._/-]+$` | absent/unconfigured | no |
| `spec_pr_merge` | `manual | auto-on-green` | `manual` | `spec_pr_merge_policy` front matter |

`effective_propose_change_mode` and `effective_critique_mode` MUST resolve an internally contradictory invocation envelope to escalation first, then consider the envelope's invocation-scoped `mode`, the global setting, and the safe default; a resolved `batch` mode with missing required fields MUST escalate. `effective_in_flight_alignment` MUST resolve genuine semantic conflict and every requested partial or full supersession to escalation before considering an envelope relationship, the global setting, or the safe default. `default-align` MAY satisfy a relationship only when the new work is compatible with the surfaced in-flight item and MUST NOT convert a conflict into alignment.

`effective_revise_decision_mode` MUST resolve in this order: a cited design-record contradiction; a missing or unreachable design record; any ratification-review blocker; a drift-origin proposal, which MUST resolve through `effective_drift_acceptance_mode`; a valid per-proposal `decision_policy`; the valid global `revise_decision_mode`; and the safe default. Each of the first three branches requires maintainer input before any later value is considered, and the drift branch requires whatever `effective_drift_acceptance_mode` requires. An absent `decision_policy` inherits the valid global value, while a present malformed, unknown, or wrong-typed override resolves safely to `manual`; missing or malformed global policy likewise resolves to `manual` without raising. `manual` preserves explicit per-proposal human confirmation. `delegated` may own a non-drift decision only when the two exact-byte signals defined above agree and every floor is clear. `consensus` additionally requires evidence from the separately ratified core consensus tier; until that tier and its evidence are available, `consensus` requires maintainer input and arms no unattended decision. A drift-origin proposal MUST resolve through `effective_drift_acceptance_mode` and MUST NOT be routed by `revise_decision_mode` or by a per-proposal `decision_policy` under any value, including `consensus`.
...
`effective_doctor_disposition` MUST resolve a cited design-record contradiction, a missing or unreachable design record, and every other contract-declared never-self-waived class to human input first; it then considers a valid invocation-scoped disposition, a valid global `doctor_dispositions[check_id]` entry, and finally the safe unmapped result requiring human input. Invalid map entries are ignored at entry granularity so valid siblings remain usable. An unavailable mapped action, absent downstream store-write consent, journal failure, or execution failure MUST require human input and MUST NOT fall through silently to a different verb.

`effective_ratification_review` MUST enforce the unconditional independent-review floor and every blocker or reviewer-unavailability condition before considering the per-proposal override, the global setting, and the safe default. `manual-spawn` and `auto-spawn` control only who initiates the required review; neither can remove it or accept blockers. Evidence staleness is evaluated when a decision is applied: stale evidence is invalid, while a fresh conforming review of the reassembled final bytes supersedes it without escalation. A missing, malformed, or unavailable reviewer-model designation requires maintainer input in either mode. Repo `thewoolleyman/livespec` MUST designate `fable` in its root `.livespec.jsonc` when this contract is implemented.

`effective_spec_pr_merge` MUST expose TWO resolutions.

(a) Single-proposal effective policy. For one proposal identified by its stem, resolve in order: a present valid `spec_pr_merge_policy` front-matter value on that proposal's file; else the valid global `spec_governance.spec_pr_merge` value; else the safe default `manual`. A present malformed, wrong-typed, or unknown `spec_pr_merge_policy` value, and a missing or malformed global policy, MUST resolve to `manual` without raising. A stem matching ZERO files under `<spec-root>/history/*/proposed_changes/<stem>.md` MUST resolve to `manual` for that stem. A stem matching MORE THAN ONE file MUST resolve via the same conservative fold as (b): `manual` when ANY matching file's policy resolves `manual`, and `auto-on-green` only when EVERY matching file resolves `auto-on-green`. The multi-match case is real rather than theoretical because the glob is not injective: a stem can recur verbatim across separate history cuts when a proposal is re-filed and re-ratified under the same slug after an earlier rejection or supersession. Both non-happy cases are stated together here so neither is left implicit.

(b) Pull-request effective policy, the conservative fold. Given the explicit, NON-EMPTY set of ratified proposal stems a pull request introduces — supplied by the CALLER, never derived by the control CLI itself — the pull-request effective policy is `auto-on-green` only when every stem's single-proposal effective policy per (a) resolves to `auto-on-green`; any stem resolving to `manual` makes the pull-request effective policy `manual`. An EMPTY stem set passed to this resolution explicitly (the control CLI's `--pr-effective-policy` invoked with zero `--proposal-stem` values) MUST resolve to `manual` as the CLI's own fail-safe floor, and MUST NOT fall back to the global effective value.

(b) governs ONLY a pull request that ratifies one or more proposals. Deriving whether a pull request ratifies any proposal at all, and if so which stems, is the WORKFLOW's responsibility, specified in `non-functional-requirements.md` §"Spec pull-request auto-merge requirement" — a cross-file reference, because that workflow-facing requirement is not defined in this file. The derivation is computed from the pull request's merge-base diff against the non-`-revision` files it ADDS under `<spec-root>/history/*/proposed_changes/` across EVERY configured spec root (the main spec plus any `<spec-root>/templates/<name>/` sub-specs). A workflow that successfully computes this diff and finds ZERO added stems has determined the pull request ratifies NOTHING; such a pull request is NOT subject to `spec_pr_merge` at all. It is not "an empty stem set passed to the fold", it is "no fold invocation", and its eligibility is decided entirely by the registration-time hard floors and non-spec eligibility gates defined in `contracts.md` §"Spec pull-request merge-registration mechanics", unaffected by this key. A workflow whose diff computation itself FAILS (a git or hosting-API error, not merely zero matches) cannot distinguish "ratifies nothing" from "ratifies something unknown", and MUST treat the pull request as ineligible for auto-merge registration exactly as an effective-`manual` policy would; this is the sole fail-closed case, and a successfully-computed empty set is not it.

The derivation carries one required hardening. The workflow MUST compute BOTH the hosting API's pull-request file listing AND a local-git derivation, and the two MUST AGREE. The workflow MUST treat a disagreement between the two derivations, OR an entirely-empty computed pull-request diff (zero changed files in total — not merely zero matched proposed-change stems — which is impossible for a genuine pull request and is therefore a safe, costless signal of a broken or mis-scoped diff computation), as derivation FAILURE, resolving to `manual`. The limit is stated honestly rather than overclaimed: this closes the SOURCE-level failure class — a wrong base ref, a shallow-checkout artifact, or API/git divergence — where the two derivations disagree or one is simply broken. It does NOT close a FILTER-level bug: if the identical faulty pathspec or scoping error is applied to BOTH derivations, both agree and both are wrong, and a pull request that genuinely introduces a `manual`-floored proposal could still yield a false KNOWN-EMPTY. That residual is an implementation-correctness concern for the workflow implementation, not a specification-drafting gap this clause can close by wording alone.

Every policy-driven auto-consumption, automatic doctor disposition, automatic review spawn, review verdict, automated revise decision, and automated spec pull-request merge-registration attempt MUST be journaled through the deterministic control surface defined in `contracts.md` before mutation. Shared predicates — one per authoring operation, `requires_doctor_disposition_input`, `requires_revise_decision_input`, `awaits_ratification_review`, and `awaits_manual_spec_pr_merge` — MUST gate both enforcement and any human-attention advertisement so a caller never re-derives whether input is still required. These policies authorize Spec-Plane work by a supervising upstream caller or attended Driver session only; they confer no spec-side execution or store-write authority on an orchestrator executor. `spec_pr_merge` is the SOLE exception to that authorization scope, and the reconciliation is stated explicitly rather than left as an unaddressed tension: its configured action executes inside an UNATTENDED CI workflow rather than a supervising caller or attended Driver session, and it is safe to execute there precisely BECAUSE it is CI- and branch-protection-bounded workflow automation that only REGISTERS a merge-when-green request — it never mutates spec, history, or work-item state itself, never bypasses, weakens, relabels, or replaces a required status check, and never performs the merge outside host-guaranteed green-gate semantics. Every other spec-governance key's automated action remains scoped to a supervising upstream caller or attended Driver session exactly as stated.
...
- effective revise-decision resolution, automated reviewer/decider dispatch, exact-byte decision-evidence assembly, and the required pre-mutation journal append;
...
- ratification-review dispatch and evidence assembly: after assembling each decision's final `resulting_files[]`, the operation prose MUST dispatch or consume the required independent review against those exact bytes, attach matching evidence, and only then invoke the wrapper;
- the retry-on-exit-4 handshake.

`bin/revise.py` MUST NOT invoke the template prompt, the LLM, the reviewer, or the interactive confirmation flow. It validates review evidence but never spawns an agent. The wrapper's deterministic file-shaping mechanics are codified in `constraints.md` §"Sub-command lifecycle mechanics".
...
- the alignment-intent capture: for each in-flight item surfaced above, the operation prose MUST elicit the user's intended relationship — align (the new proposal conforms to the in-flight design), modify-to-accommodate (the new proposal partially supersedes with explicit rationale), or explicitly supersede (the new proposal replaces the in-flight design; the in-flight branch SHOULD be closed/abandoned upstream) — only when a complete invocation envelope has not already supplied the relationship and `effective_in_flight_alignment` cannot safely resolve it. The captured alignment intent feeds the propose-change template prompt as steering context for the authoring dialogue; it is NOT serialized into the resulting proposed-change findings JSON;
- the retry-on-exit-4 handshake (shared discipline with the other LLM-driven wrappers).

The narration MUST NOT gate the wrapper (`bin/propose_change.py` runs regardless), MUST NOT add any pre-step or post-step doctor check, and MUST NOT block downstream wrapper invocations — its sole purpose is in-flight-design-drift prevention so the user or a complete compatible upstream envelope MAY choose alignment, modification, or supersession before the new proposal lands.
...
- **Spec Plane** (livespec core) — owns `SPECIFICATION/`, the write-once plan records under `plan/<slug>/`, and the `/livespec:*` lifecycle. Concerned with *what* the system should do.
...
**Terminology guard.** The console is NOT a "Driver." "Driver" already names the per-agent-runtime binding (`livespec-driver-claude`, `livespec-driver-codex`, `livespec-driver-pi`) that binds core's spec-side CLIs and prose to ONE tool runtime; the console is the **Control Plane / operator cockpit** — a separate role. Keep the two distinct in all prose and diagrams.
...
- Freeform research prose remains historical evidence, not the authoritative scope register. Before a plan epic may take implementation children, a scoping event MUST cut every known requirement from the research prose into requirement-carrier children under that epic, including requirements deliberately deferred from the current implementation increment. A requirement MUST NOT exist only in prose after that point. Rationale: no gate may presume a fixed seed/research document shape, so the scoping EVENT — not a document schema — is what makes requirements enumerable; design records: repo `thewoolleyman/livespec`, `plan/planning-lane-redesign/research/brainstorm.md` and `plan/planning-lane-redesign/research/maintainer-rulings.md`.
- Deferral MUST be represented in the ledger, not prose. The concrete representation SHOULD be an explicit `deferred` disposition on the requirement-carrier child when the active ledger supports that state; otherwise it MAY be a sanctioned ledger label/state applied only through the same admission valve that owns labels moving into implementation-ready states. Agents and maintainers MUST NOT hand-edit admission labels or bypass that valve to mark deferral. Rationale: the maintainer ruled deferrals are ledger state and must pass through the admission valve; design record: repo `thewoolleyman/livespec`, `plan/planning-lane-redesign/research/maintainer-rulings.md`.
- Handoff persistence MUST provide append-only, per-entry ledger entries, each individually attributed and timestamped; in the Beads reference realization, those entries are plan-epic ledger comments per the `bd` long-prose spike recommendation in repo `thewoolleyman/livespec`, `plan/planning-lane-redesign/research/bd-long-prose-spike.md` for work item `livespec-zsn2xh.1`. The authoritative read path is the ledger entry timeline or comment JSON/timeline read path, not git. Each handoff entry MUST carry only non-derivable content such as rationale, warnings, abandoned attempts, and pointers; derivable state such as child lists, child statuses, PR states, merge state, and readiness MUST be queried fresh from the ledger and git at resume time. This binds the architectural property rather than the literal Beads feature, so any non-Beads realization — e.g. `livespec-orchestrator-git-jsonl` — must conform at SPEC level by realizing an equivalent append-only, per-entry, attributed, timestamped ledger journal. Rationale and design record: the spike citation above.
- The Planning Lane keeps two seams with the Orchestrator Plane: the plan surface writes and reads plan-epic ledger entries for handoffs, reads ledger children to resume work, and routes ripe work into the ledger through the orchestrator's sanctioned capture/admission surfaces. The redesign MUST NOT introduce direct Spec-Plane writes to orchestrator-private storage outside those ledger-entry and capture/admission surfaces. Rationale: the ledger-state ruling, the selected handoff-entry property, and the no-private-storage boundary; design records: repo `thewoolleyman/livespec`, `plan/planning-lane-redesign/research/brainstorm.md`, `plan/planning-lane-redesign/research/maintainer-rulings.md`, and `plan/planning-lane-redesign/research/bd-long-prose-spike.md`.
- A plan epic MUST NOT close or archive while any child requirement or implementation item is undisposed. Independently, archive time MUST include a separate adversarial completeness review that reads the plan's research documents against the epic's children and attests that every requirement has a carrier, including deferred requirements. This independent review is a downstream archive gate; it is not performed by propose-change authoring itself. Rationale: the two-leg archive gate and the requirement-carrier invariant; design records: repo `thewoolleyman/livespec`, `plan/planning-lane-redesign/research/brainstorm.md` and `plan/planning-lane-redesign/research/maintainer-rulings.md`.
...
flowchart TB
    ...
    subgraph op["ORCHESTRATOR-PLANE skills: reference beads-fabro"]
        plan["plan: capture a plan in plan/&lt;slug&gt;/<br/>(research + one epic anchor), scope requirements into epic children,<br/>append handoffs as ledger entries, route matured pieces"]
...
**Revision file format.** Each `<topic>-revision.md` MUST contain, in order: (1) YAML front-matter with the keys `proposal: <stem>.md` (the paired proposed-change filename stem, including any `-N` collision-disambiguation suffix per the filename-stem rule above), `decision: accept | modify | reject`, `revised_at: <UTC ISO-8601 seconds>`, `author_human: <git user.name and user.email per livespec.io.git.get_git_user(), or the literal "unknown" when git is available but either config value is unset>`, and `author_llm: <resolved author id per the unified precedence in §"Author identifier resolution">`; (2) `## Decision and Rationale` — always required; one paragraph explaining the decision; (3) `## Ratification Review` — REQUIRED when `decision` is `accept` or `modify`, immediately after the rationale, carrying the reviewer model and identity, separate/read-only declaration, reviewed-at timestamp, `NO BLOCKERS` verdict, proposal stem, and content digest defined in `contracts.md` §"Sub-command wire contracts"; (4) `## Modifications` — REQUIRED when `decision: modify`; prose-form description of how the proposal was changed before incorporation, with optional short fenced before/after excerpts permitted for hyper-local clarity (line-number-anchored unified diffs are NOT used here — they are fragile across multi-proposal revises); (5) `## Resulting Changes` — REQUIRED when `decision` is `accept` or `modify`; names the specification files modified and lists the sections changed; (6) `## Rejection Notes` — REQUIRED when `decision` is `reject`; explains what would need to change about the proposal for it to be acceptable in a future revision (this preserves the rejection-flow audit trail). Rejected records omit `## Ratification Review`; the conditional sections otherwise retain the order above. For automated skill-tool-authored revisions (e.g., `seed` auto-capture, `out-of-band-edits` auto-backfill), `author_llm` takes the convention value `livespec-seed` / `livespec-doctor`, hardcoded by the wrapper and bypassing the precedence above.
...
The v021 Q3 imperative one-time `tests/heading-coverage.json` population step lands alongside this seed commit; from this revision onward, every revise that adds, changes, or removes a `##` heading MUST update `tests/heading-coverage.json` via the governed propose-change/revise loop's `resulting_files[]` mechanism. When the resulting entry uses `test: "TODO"`, the revise flow MUST first file a covering-test work-item through the governed project's configured work-item capture seam and stamp the returned id into a non-empty `work_item` field beside `reason`. A spec-ahead covering-test item SHOULD depend on the implementing work-item(s) that must land before its test can exist. If the governed project has no configured orchestrator, the author MUST supply an explicit `work_item` reference from the project's external tracker; an unowned TODO entry is never valid.
