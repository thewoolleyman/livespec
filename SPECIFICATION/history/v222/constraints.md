...
Spec-governance batch mode changes only the operation-prose dialogue layer: complete invocation envelopes are consumed by a supervising caller or attended Driver session and MUST NOT turn the Python mutation wrappers interactive or LLM-aware. The deterministic `spec_governance.py` control wrapper owns only validated config edits and journal appends. Safe defaults MUST arm no unattended authoring; an invalid policy, incomplete envelope, ambiguity, or conflict MUST fail toward human attention before mutation. Audit events MUST be structured and SHOULD omit full user intent when an envelope digest plus the resulting proposal artifact supplies sufficient traceability.
...
- **Re-vendoring** of upstream-sourced libs (`returns`, `fastjsonschema`, `structlog`, `typing_extensions`, `livespec_runtime`) MUST go through `just vendor-update <lib>` — the only blessed mutation path. The recipe fetches the upstream ref, copies it under `_vendor/<lib>/`, preserves `LICENSE`, and updates `.vendor.jsonc`'s recorded ref.
...
An author who needs a diagram type Mermaid lacks (per `spec.md` §"Template manifest") renders it with an alternate tool such as PlantUML or Graphviz OUTSIDE livespec and commits the resulting image as an opaque asset OUTSIDE the spec root (per `spec.md` §"Spec-tree path closure"). Livespec MUST NOT detect, recommend, install, or invoke any such renderer; the project owns that tool entirely.
...
A **design-record citation** — the plain-text, repo-qualified pointer to the design record behind a load-bearing semantic definition, required by `spec.md` §"Intent preservation and design-record authority" (repository name plus the record's path within it, plus decision identifiers where the record numbers its decisions) — is an authorized exception to the prohibition on referencing non-`SPECIFICATION/` paths in sibling repos, and equally covers non-`SPECIFICATION/` design-record paths in the citing repo itself (e.g., an archived plan record under `plan/archive/<slug>/`). No allowlist entry is required: design-record citations are meant to be ubiquitous and low-friction, and a per-citation allowlist entry would tax exactly the behavior the mandate requires everywhere.
...
### Spec-governance safety rails

Every effective-policy resolver MUST evaluate its hard floors before invocation overrides, per-proposal overrides, global values, or safe defaults. Missing or malformed policy MUST resolve safely without raising: authoring modes inherit interactive dialogue, `doctor_dispositions` treats each invalid or absent entry as unmapped while retaining valid sibling entries, revise decisions inherit manual confirmation, ratification review inherits manual spawn with an unconfigured reviewer requiring human input, and `spec_pr_merge` inherits `manual`. Safe defaults arm no unattended action.

`spec_pr_merge` carries further rails. An explicitly-empty proposal-stem set passed to the pull-request-level fold MUST resolve to `manual` and MUST NOT fall back to the unfolded global value. A diff-derivation failure — the workflow cannot determine whether the pull request ratifies anything — MUST be treated as ineligible for merge registration; this is distinct from a successfully-computed zero-stem result, which is simply outside this key's governance rather than a failure. The stem-set derivation is a workflow (caller) responsibility and MUST NOT be performed by the deterministic control CLI. A policy edit under this key is never itself the merge-registration or CI-bypass action it configures, exactly as for every other spec-governance key in this file. A hosting service that cannot provide merge-when-green semantics MUST require human input instead of polling and force-merging; `contracts.md` §"Spec pull-request merge-registration mechanics" carries the fuller statement.

The doctor verb allowlist is exactly `fix-now | capture-as-work-item | propose-change | defer | dismiss`. Design-record contradiction or absence is always human-gated. An unavailable action, absent downstream consent, journal failure, or execution failure MUST escalate and MUST NOT silently choose another verb. `dismiss` and `defer` are audited like mutating verbs. Execution of a selected spec-side verb is Spec-Plane work performed by a supervising upstream caller or attended Driver session; no spec-governance policy confers spec-side execution or store-write authority on an orchestrator executor.

The no-independent-review floor is unconditional under every ratification-review mode. The reviewer MUST be separate, read-only, adversarial, and match the project designation; blockers MUST NOT be self-waived. Review evidence MUST bind the canonical content digest defined in `contracts.md`, and a spawn, journal, or evidence-validation failure MUST require maintainer input before mutation. A cited design-record contradiction or missing/unreachable design record remains a mandatory blocker. This floor governs spec-side ratification only; drift-origin proposals retain their separately ratified acceptance authority under `spec.md` §"Contract + reference implementations architecture".

Automated revise decisions MUST remain cumulative with that independent-review floor. `effective_revise_decision_mode` MUST require maintainer input for a cited design-record contradiction, a missing/unreachable design record, or any ratification-review blocker before it considers `decision_policy`, `revise_decision_mode`, or `manual`, and MUST route a drift-origin proposal through `effective_drift_acceptance_mode`. No config value, per-proposal override, reviewer, delegated decider, or consensus result may waive those branches. A drift-origin proposal MUST remain human-decided under every configuration EXCEPT an explicit per-repo `drift_acceptance_mode: consensus` opt-in satisfied by present, fresh and conforming unanimous cross-vendor evidence; `drift_acceptance_mode` MUST NOT accept `delegated`, and no `revise_decision_mode` value may accept drift. `delegated` requires matching exact-byte acceptance from the independent decider plus `NO BLOCKERS` from the separate reviewer; disagreement or missing/stale/mismatched evidence MUST escalate. `consensus` MUST escalate until the separately ratified consensus tier and its evidence are available. Every automated decision MUST append its digest-only journal event before mutation; journal failure MUST require human input. An absent proposal override inherits valid global policy; a present malformed override and missing, malformed, wrong-typed, or unknown global policy MUST resolve to `manual` without raising.
