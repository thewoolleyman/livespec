...
Scenario: Global auto-on-green registers merge-on-green for a single-proposal PR
  Given `spec_governance.spec_pr_merge` resolves to `auto-on-green`
  And the ratifying pull request introduces exactly one proposal stem
  And that proposal carries no `spec_pr_merge_policy` front-matter override
  And every registration-time hard floor holds
  When the workflow derives the stem set and resolves the pull-request effective policy
  Then the pull-request effective policy is `auto-on-green`
  And the workflow registers rebase auto-merge before CI completes
  And the required all-green status check still gates the merge

Scenario: A per-proposal manual override defeats a global auto-on-green setting
  Given `spec_governance.spec_pr_merge` resolves to `auto-on-green`
  And the ratifying pull request introduces exactly one proposal stem
  And that proposal's front matter sets `spec_pr_merge_policy: manual`
  When the workflow resolves the pull-request effective policy
  Then the pull-request effective policy is `manual`
  And no auto-merge registration occurs
  And `awaits_manual_spec_pr_merge` is true

Scenario: One manual proposal folds a multi-proposal PR to manual
  Given `spec_governance.spec_pr_merge` resolves to `auto-on-green`
  And the ratifying pull request introduces three proposal stems
  And two of them resolve to `auto-on-green`
  And the third sets `spec_pr_merge_policy: manual`
  When the workflow resolves the pull-request effective policy
  Then the conservative fold makes the pull-request effective policy `manual`
  And the pull request is left for a human rebase-merge
```
...
```gherkin
Feature: Planning Lane plan store, scoping, ledger handoffs, and archive gate

Scenario: A plan open writes only research plus one epic anchor
  Given a maintainer opens a new plan with slug <slug>
  When the plan store is created at plan/<slug>/
  Then it contains only write-once research inputs under plan/<slug>/research/ and exactly one write-once metadata anchor naming the ledger epic id
  And no handoff.md, supervisor-handoff.md, or mutable status file is created

Scenario: A scoping event precedes implementation children
  Given a plan epic whose research prose names requirements
  When the epic would take its first implementation child
  Then a scoping event MUST already have cut every known requirement into requirement-carrier children under the epic, including deliberately deferred requirements
  And no requirement exists only in prose

Scenario: A deferred requirement is sanctioned ledger state
  Given a requirement deliberately deferred from the current implementation increment
  When its carrier child is dispositioned
  Then the deferral is an explicit deferred disposition where the active ledger supports that state, or a sanctioned ledger label applied only through the admission valve
  And no agent or maintainer hand-edits an admission label to mark it

Scenario: A handoff appends a ledger entry carrying only non-derivable content
  Given a session wrapping up work on a plan
  When it persists its handoff
  Then it appends an individually attributed, timestamped, append-only ledger entry on the plan epic
  And the entry carries only non-derivable content, with derivable state queried fresh from the ledger and git at resume time

Scenario: Migration preserves a pre-existing handoff as write-once evidence
  Given a pre-existing plan with a live handoff.md or supervisor-handoff.md
  When the plan is migrated to the ledger-held shape
  Then the handoff is preserved as a write-once historical-evidence file under plan/<slug>/research/ and is not deleted from the git tip
  And every fleet-spec design-record citation naming the pre-relocation path is updated in the same change or an explicitly linked work-item

Scenario: An archive attempt is blocked by an undisposed child
  Given a plan epic with at least one undisposed child requirement or implementation item
  When close or archive is attempted
  Then the attempt is refused until every child is disposed

Scenario: Archive requires the independent completeness review
  Given a plan epic whose children are all disposed
  When the plan is archived
  Then a separate adversarial completeness review has read the plan's research documents against the epic's children and attested every requirement, including deferred ones, has a carrier
```

## Primary-checkout Playwright calls are refused

```gherkin
Feature: Keep browser artifacts out of governed primary checkouts

Scenario: A Playwright MCP call is refused only at the primary checkout
  Given a livespec-governed Git repository and one of its linked worktrees
  And the primary checkout's resolved Git directory equals its resolved common Git directory
  When Claude Code attempts any `mcp__playwright__*` tool with the primary checkout as `cwd`
  Then the Driver denies the call before MCP invocation
  And no Playwright artifact is created in the primary checkout
  But the same tool call from the linked worktree is allowed
```
...
## Error path — spec pull request cannot auto-merge

```gherkin
Feature: A ratifying spec pull request falls back to human merge

Scenario: A red required gate blocks completion
  Given an eligible pull request is registered for merge-when-green
  And its required all-green status check fails
  When the host evaluates the merge-when-green request
  Then the pull request does not merge
  And the policy neither bypasses nor relabels the required check

Scenario Outline: A registration-time floor blocks registration
  Given `spec_governance.spec_pr_merge` resolves to `auto-on-green`
  And the ratifying pull request has <floor>
  When the workflow attempts merge registration
  Then no auto-merge registration occurs
  And `awaits_manual_spec_pr_merge` is true

  Examples:
    | floor                                       |
    | draft state                                 |
    | a `do-not-merge` label                      |
    | an unresolved changes-requested review      |

Scenario: An invalid policy value resolves to manual
  Given `spec_governance.spec_pr_merge` or a proposal's `spec_pr_merge_policy` is malformed, wrong-typed, or unknown
  When the effective policy resolves
  Then it resolves to `manual` without raising
  And no unattended merge is armed

Scenario Outline: A diff-derivation failure leaves the pull request for human merge
  Given the workflow's stem-set derivation hits <failure>
  When the workflow evaluates auto-merge eligibility
  Then the pull request is ineligible for auto-merge registration
  And the outcome is `manual`

  Examples:
    | failure                                              |
    | a git or hosting-API error                           |
    | disagreement between the two required derivations    |
    | an entirely-empty computed pull-request diff         |

Scenario: A successfully-computed empty stem set is not a derivation failure
  Given the workflow successfully computes the pull request's merge-base diff
  And the diff adds no files under `<spec-root>/history/*/proposed_changes/`
  When the workflow evaluates auto-merge eligibility
  Then the pull request ratifies nothing and is outside this key's governance
  And it keeps the eligibility path the non-spec gates already give it

Scenario: Unsupported hosting requires human input
  Given the hosting service cannot provide merge-when-green semantics
  When the workflow would otherwise register auto-merge
  Then it requires human input
  And it does not poll and force-merge

Scenario: Journal-append failure prevents registration
  Given the spec-governance policy journal cannot be appended
  When the workflow attempts merge registration
  Then registration does not occur
  And the failure requires human input

Scenario: Default configuration arms no unattended merge
  Given no `spec_pr_merge` value is set globally or on any proposal
  When the effective policy resolves
  Then it is `manual`
  And no unattended merge is armed
```
...
```gherkin
Scenario: drift acceptance defaults to human
  Given a repo whose `.livespec.jsonc` declares no `drift_acceptance_mode`
  When a drift-origin proposed change is revised
  Then `effective_drift_acceptance_mode` MUST resolve to `human` and the decision MUST require maintainer input

Scenario: armed consensus accepts only on conforming evidence
  Given a repo declaring `drift_acceptance_mode: consensus`
  And unanimous cross-vendor evidence that is present, fresh and conforming
  When a drift-origin proposed change is revised
  Then the consensus tier MAY own the acceptance
  And `requires_revise_decision_input` MUST be false for that decision
  And the acceptance MUST be journaled with the consensus-evidence digest before any mutation

Scenario: armed consensus escalates on absent or stale evidence
  Given a repo declaring `drift_acceptance_mode: consensus`
  And evidence that is absent, stale or non-conforming
  When a drift-origin proposed change is revised
  Then the decision MUST require maintainer input
  And no unattended decision MUST be armed

Scenario: delegated is refused for drift acceptance
  Given a repo with no `drift_acceptance_mode` declared
  And a `set-drift-acceptance-mode:global:delegated` invocation
  When the control surface resolves it
  Then it MUST be refused with the allowed-value diagnostic
  And the effective value MUST remain `human` without raising

Scenario: revise_decision_mode cannot route drift
  Given a repo declaring `revise_decision_mode: consensus` and no `drift_acceptance_mode`
  When a drift-origin proposed change is revised
  Then the decision MUST resolve through `effective_drift_acceptance_mode` to `human`
  And `revise_decision_mode` MUST NOT route it
```

## Operator-owned Git work retains the declared author

```gherkin
Feature: Operator Git authorship is stable across every production path

Scenario: Interactive, replay, and Fabro work use the operator pair
  Given an opted-in repository declares one operator name and email
  When an ordinary agent commit, a Red→Green replay commit, and a Fabro run each produce operator-owned work
  Then each final commit author MUST equal the exact declared operator pair
  And agent or App attribution MUST remain only trailer, committer, or forge metadata

Scenario: Conflicting or missing identity fails closed
  Given an operator-owned commit encounters a conflicting environment value, explicit author, stale repository or worktree config, or missing required fleet declaration
  When the commit is created or prepared for publication
  Then the commit or, at latest, its publication MUST fail
  And the diagnostic MUST identify the conflicting pair or missing declaration and its corrective action

Scenario: Mechanical authorship is narrow
  Given an exact configured mechanical-author pair is assigned to a release or dependency-pin job
  When that job produces a commit
  Then an exclusively mechanical commit MAY retain that configured pair
  But a mixed or judgment-bearing automation commit MUST use the operator pair

Scenario: Imported third-party authorship is preserved
  Given a genuine third-party commit is imported without operator-owned modification
  When it enters an opted-in repository's published history
  Then the commit MUST retain its original author

Scenario: The all-ref verifier is non-vacuous and reports aliases
  Given fleet conformance selects every published branch and tag for author verification
  When its scan is empty or any selected ref contains a forbidden operator alias
  Then the Verifier MUST refuse an empty scan
  And it MUST report the exact offending commit and author pair from any branch or tag
```
