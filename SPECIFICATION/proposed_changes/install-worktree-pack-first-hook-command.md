...
@@ SPECIFICATION/non-functional-requirements.md — "## Developer-tooling layout", the paragraph beginning "`justfile` is the single source of truth for every dev-tooling invocation.": replace one clause inside that paragraph; the rest of the paragraph is unchanged. @@
-Lefthook pre-commit runs `just lint-autofix-staged` as its first step, which applies
+Lefthook pre-commit runs `just lint-autofix-staged` as its first check-bearing step, immediately after the `00-install-worktree-pack` installer named in `contracts.md` section "Pre-commit step ordering"; it applies
```
