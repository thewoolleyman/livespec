...
Adds a Delegated gate execution clause to the Contracts section "Enforcement-suite invocation", permitting a repository's pre-push gate to execute on a remote executor and constraining that delegation as a property of the delegation — the verdict, the executor, and the pushing host's handling of both — rather than as a requirement that any check target change its internals. Three sub-rules. *The tree*: the gate runs against the exact tree being pushed and the pushing host verifies that identity. *The set*: the checks performed on the executor and on the pushing host together are exactly the set the repository's gate RECIPE selects for that tree WHEN RUN LOCALLY WITHOUT DELEGATION, that selection is a function of the pushed tree alone and never of the environment of either host, every one of them must actually have its verification performed, a check that reports success without verifying has not been performed and moves to the pushing host, and no forge credential beyond a least-privilege read-scoped token for that run — nor any credential from which one can be obtained — may be available to a remote executor. *The verdict*: a verdict exists only when the gate reported its own outcome for this push, every other condition is an absent verdict that refuses the push and is reported as absent rather than as a failure, the push is authorized only when the delegated verdict and every locally-performed check both pass, and no marker may be written on less. Delegation is opt-in and the gate remains runnable on the pushing host by an explicit act. Amends the one bullet in that section that labels the surface as local, and adds three contributor-facing Gherkin scenarios.
...
>
> *The set.* The checks performed for a delegated push, on the executor and on the pushing host together, MUST be exactly the set that the repository's gate recipe selects for that tree when the gate is run on the pushing host without delegation, so that delegation can neither weaken nor widen the gate. That selection MUST be a function of the pushed tree alone; a selection that varies with the environment of the pushing host or of the executor is not a conforming selection. Every check in that set MUST have its verification actually performed: a check that reported success without performing the verification it exists to perform — for any credential reason, whether absent, rejected, or too narrowly scoped — has NOT been performed, and MUST be performed on the pushing host instead. A forge credential beyond a least-privilege, read-scoped token for that gate run MUST NOT be available to a remote executor performing a delegated gate — held by it, present in its environment, mounted into it, or obtainable by it from any service it can reach — and a credential from which such a forge credential can be obtained MUST NOT be available to it on those same terms.
>
> *The verdict.* A verdict EXISTS only when the delegated gate itself reported its own OUTCOME for this push. An executor that could not be reached or authenticated, a transport that reported its own success without carrying the gate's outcome, a process terminated by signal, and any stored or cached result are each an ABSENT verdict; the pushing host's own gate-skip marker is read to decide whether a gate runs at all and is never a verdict. The push is authorized only when the delegated verdict and every check performed on the pushing host both pass, and a marker recording a tree as having passed the gate MUST NOT be written on anything less. An absent verdict MUST refuse the push and MUST be reported as absent rather than as a failing check. The refusal rule applies to the delegation's final outcome.
...
>
> And the push is authorized only when the delegated verdict and that check both pass
...
*A hybrid push.* The record left open what a check the executor cannot perform should do. *The set* answers it: such a check is performed on the pushing host, so most of the gate is delegated and a named remainder is local. The record never described that shape.

*An unconditional credential ceiling.* *The set* forbids any forge credential beyond a least-privilege read-scoped token being available to a remote executor performing a delegated gate — scoped by credential class and availability, not by the purpose the credential was provisioned for, so that a token declared for pod provisioning rather than for a check is equally forbidden. Earlier drafts only forbade WIDENING a credential's scope, and a reviewer showed that a dedicated executor provisioned with an admin-scoped token from the start violated no sentence — which is precisely what an operator would build, because it makes everything green. Closing that needs an unconditional ceiling, and an unconditional ceiling binds executors the fleet has not built yet. The consequence is deliberate and worth seeing plainly: a check that genuinely requires a stronger credential can never be delegated, only performed on the pushing host. The rejected alternative was to let a sufficiently privileged executor perform everything, which trades a containment property for convenience. This ceiling also removes the need to cross-reference §"Self-hosted CI runner host requirements" at all, which an earlier draft did and which imported a sentence sending such checks to hosted capacity — a destination a pre-push gate does not have.

*A parity line, not a no-narrowing line.* The set a delegated gate is held to is the set the repository's own recipe selects for that tree WHEN RUN LOCALLY WITHOUT DELEGATION, so a narrowing that lives in the recipe itself — such as the `hook_gate=1` omission `livespec-dev-tooling`'s pre-push recipe already carries — remains legal, because it narrows the local run and the delegated run IDENTICALLY and delegation therefore changes nothing. A narrowing that applies only on the delegated path does NOT become legal by being committed: a version-controlled executor manifest exporting a skip list narrows only the delegated run, so the delegated set is no longer the set a local run selects, and it is forbidden by the same sentence. The rejected alternative was to forbid every narrowing, which would have outlawed the existing in-recipe carve-out. The line drawn is PARITY WITH THE LOCAL RUN, not whether the narrowing is auditable — an earlier draft of this bullet drew it at auditability, and a reviewer showed that a committed executor-side skip list satisfied that line while defeating the clause's own purpose.
