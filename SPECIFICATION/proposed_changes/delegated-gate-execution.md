...
Adds a Delegated gate execution clause to the Contracts section "Enforcement-suite invocation", permitting a repository's pre-push gate to execute on a remote executor and constraining that delegation as a property of the VERDICT rather than of any individual check target. Three sub-rules. *The tree*: the gate runs against the exact tree being pushed and the pushing host verifies that identity. *The set*: the checks performed on the executor and on the pushing host together are exactly the set the repository's gate RECIPE selects for that tree, every one of them must actually have its verification performed, a check that reports success without verifying has not been performed and moves to the pushing host, and a remote executor may hold no credential beyond a least-privilege read-scoped forge token. *The verdict*: a verdict exists only when the gate reported its own outcome for this push, every other condition is an absent verdict that refuses the push and is reported as absent rather than as a failure, the push is authorized only when the delegated verdict and every locally-performed check both pass, and no marker may be written on less. Amends the one existing bullet that states the pushing-host assumption, and adds three contributor-facing Gherkin scenarios.
...
>
> And the executor is not given a wider credential in order to perform it
>
...
**Two decisions the maintainer is being asked to ratify, not merely to confirm.** Both are choices this proposal makes; neither is a restatement of the design record, and both were surfaced by review rather than by the author.
