...
Adds a Delegated gate execution clause to the Contracts section "Enforcement-suite invocation", permitting a repository's pre-push gate to execute on a remote executor and constraining that delegation as a property of the delegation — the verdict and the executor — rather than of any individual check target's internals. Three sub-rules. *The tree*: the gate runs against the exact tree being pushed and the pushing host verifies that identity. *The set*: the checks performed on the executor and on the pushing host together are exactly the set the repository's gate RECIPE selects for that tree, that selection depends on the tree and the repository's tracked configuration alone and never on the environment the push was invoked from, every one of them must actually have its verification performed, a check that reports success without verifying has not been performed and moves to the pushing host, and no forge credential beyond a least-privilege read-scoped token may be available to a remote executor. *The verdict*: a verdict exists only when the gate reported its own outcome for this push, every other condition is an absent verdict that refuses the push and is reported as absent rather than as a failure, the push is authorized only when the delegated verdict and every locally-performed check both pass, and no marker may be written on less. Delegation is opt-in and the gate remains runnable on the pushing host by an explicit act. Amends the one bullet in that section that labels the surface as local, and adds three contributor-facing Gherkin scenarios.
...
Three edits to `SPECIFICATION/non-functional-requirements.md`. All are architecture rather than mechanism: no executor technology, scheduler, or transport is named, and every requirement is stated about the delegation — the verdict and the executor — rather than about the internals of any check target.
...
>
> *The set.* The checks performed for a delegated push, on the executor and on the pushing host together, MUST be exactly the set the repository's gate recipe selects for that tree, where that selection is determined by the tree and the repository's tracked configuration alone and MUST NOT vary with the environment the push was invoked from, so that delegation can neither weaken nor widen the gate. Every check in that set MUST have its verification actually performed: a check that reported success without performing the verification it exists to perform — for any credential reason, whether absent, rejected, or too narrowly scoped — has NOT been performed, and MUST be performed on the pushing host instead. A forge credential beyond a least-privilege, read-scoped token MUST NOT be available to a remote executor performing a delegated gate.
>
...
*An auditability line, not a no-narrowing line.* The set a delegated gate is held to is whatever the repository's tracked configuration selects for that tree, so a narrowing the repository has COMMITTED (such as the `hook_gate=1` omission `livespec-dev-tooling`'s pre-push recipe already carries) remains legal under delegation, while a narrowing supplied by the invoking shell does not count as part of the selected set at all. The rejected alternative was to forbid every narrowing, which would have outlawed that existing carve-out. Stated plainly because the line drawn is auditability rather than absence of narrowing: a repository that COMMITTED a line narrowing its own pre-push set would satisfy this rule, and the protection is that such a narrowing is reviewable in version control rather than ambient in one pusher's shell.
