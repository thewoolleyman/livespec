...
Adds a Delegated gate execution clause to the Contracts section "Enforcement-suite invocation", permitting a repository's pre-push gate to execute on a remote executor and constraining that delegation as a property of the delegation — the verdict, the executor, and the pushing host's handling of both — rather than as a requirement that any check target change its internals. Three sub-rules. *The tree*: the gate runs against the exact tree being pushed and the pushing host verifies that identity. *The set*: delegation must not cause any check the repository's gate RECIPE would perform for that tree to go unperformed, and a narrowing is conforming only if it applies identically whether or not delegation is in use; every such check must actually have its verification performed on one host or the other, a check that reports success without verifying has not been performed — for any cause, credential causes included — and moves to the pushing host, while a check that reports a SKIP on the same terms on both hosts is not a check that reported success, and no forge credential beyond a least-privilege read-scoped token for that run — nor any credential from which one can be obtained — may be available to a remote executor. *The verdict*: a verdict exists only when the gate reported its own outcome for this push, every other condition is an absent verdict, never a pass, reported as absent rather than as a failure, the push is authorized only when every check the recipe would perform has passed, whether covered by a present delegated verdict or performed on the pushing host, no marker may be written on less, and an absent verdict refuses the push wherever no fallback performs the uncovered checks locally. Delegation is opt-in and the gate remains runnable on the pushing host by an explicit act. Amends the one bullet in that section that labels the surface as local, and adds three contributor-facing Gherkin scenarios.
...
>
> *The set.* Delegation MUST NOT cause any check the repository's gate recipe would perform for that tree to go unperformed. A narrowing of what the gate performs — the executor's run and the pushing host's run taken together — is conforming only if it applies identically whether or not delegation is in use; a narrowing that applies only on the delegated path — however it is expressed, and whether or not it is version-controlled — is not conforming. Every check the recipe would perform MUST have its verification actually performed, on one host or the other. A check that reported success without performing the verification it exists to perform has NOT been performed and MUST be performed on the pushing host instead; this holds for any cause, and specifically for any credential cause, whether the credential is absent, rejected, or too narrowly scoped. A check that reports a SKIP rather than a success, for a condition that holds equally on the pushing host and on the executor, is performed to exactly the extent the recipe would perform it without delegation; it is neither a check that reported success nor a check that delegation caused to go unperformed. A forge credential beyond a least-privilege, read-scoped token for that gate run MUST NOT be available to a remote executor performing a delegated gate — held by it, present in its environment, mounted into it, or obtainable by it from any service it can reach — and a credential from which such a forge credential can be obtained MUST NOT be available to it on those same terms.
>
...
*A hybrid push.* The record's own disposition (research/003 §H, slice S6) was a check that FAILS rather than warns when a credential-needing target runs without one inside the executor — refusal. *The set* departs from that: such a check is performed on the pushing host instead, so most of the gate is delegated and a named remainder is local, and a delegated push is not refused for a check the executor can never hold the credential for under the ceiling below. The record never described that shape.
