...
```
The cross-repo work-item dependency machinery (`livespec_runtime.cross_repo`) is deliberately absent from this set: the orchestrator that uses `livespec_runtime.cross_repo` owns its own vendoring or dependency declaration per its own specification (removal of the `cross_repo/` SUBTREE remains implementation work; the `livespec_runtime` package itself and its `.vendor.jsonc` entry are NOT removed, because core consumes other parts of it at runtime — notably `livespec_runtime.spec_governance`, the loader for the manifest this specification names as the single declarative source).
```
...
**Also observed, not addressed here:** `.vendor.jsonc`'s own header comment and `NOTICES.md` both state vendored-entry counts that no longer match the file (non-spec artifacts, so outside a spec proposal's reach); `_vendor/livespec_runtime/LICENSE` names tag `v0.3.0` while the pin is `v0.19.0`; and the `cross_repo/` subtree removal Edit 2 preserves still has an unstated prerequisite, since core's own `livespec/parse/cross_repo.py` imports it.
