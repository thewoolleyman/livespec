...
**The remaining contradicting clauses were surfaced by TWO independent adversarial reviews of the second draft, which found DISJOINT BLOCKER sets.** One filed the same-paragraph clause (Edit 2) as its blocker; the other filed the `spec.md` sentence (Edit 4), the miscount corrected below, and a vendoring-enumeration collision recorded under "Deliberately out of scope", while separately surfacing the `--show-effective` residual (Edit 3) as a non-blocking observation. The sets were disjoint at BLOCKER grade, not in what each saw. A third round repeated the pattern: both reviews independently found the Edit 5 site and the unfiled-companion claim, and both graded Edit 3's term collision — one non-blocking, one a blocker. The blocker grade was taken. Neither review alone was sufficient, and they graded the same-paragraph clause differently — one a blocker, one non-blocking. That disagreement is recorded rather than resolved silently, because a future reader deserves to know the finding was contested.
...
```
emits the committed manifest together with
```

The obvious shortening — dropping only `registry`, leaving "the declarative manifest" — was REJECTED on review. That exact phrase already names a DIFFERENT artifact in `non-functional-requirements.md`: the fleet-membership manifest `.livespec-fleet-manifest.jsonc`, twice. Shortening this way would have created one-name-two-artifacts ambiguity, the mirror image of the two-names-one-artifact residual Edit 5 exists to remove. `committed manifest` is used instead: it is the term Edits 1 and 2 already establish in this same paragraph, and it occurs nowhere else in the live spec tree.
...
That drift is PRE-EXISTING — this proposal surfaces it rather than creating it — and it turns on a question this proposal has no business settling: whether `livespec_runtime`'s vendored tree is permanent or still slated for removal. That is an architecture call. The maintainer directed on 2026-08-14 that it be filed as its own proposal rather than folded in here, precisely so a terminology correction does not silently decide a packaging question. That filing had NOT happened as of this proposal's last revision (2026-08-14) — it is owed, not done, and this sentence deliberately cites no identifier a future reader would be unable to resolve. It is dated rather than pegged to ratification because a claim about the ratification moment is one the author cannot know when writing it.
