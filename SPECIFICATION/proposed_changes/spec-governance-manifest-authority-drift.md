...
**The remaining contradicting clauses were surfaced by TWO independent adversarial reviews of the second draft, which found DISJOINT BLOCKER sets.** One filed the same-paragraph clause (Edit 2) as its blocker; the other filed the `spec.md` sentence (Edit 4), the miscount corrected below, and a vendoring-enumeration collision recorded under "Deliberately out of scope", while separately surfacing the `--show-effective` residual (Edit 3) as a non-blocking observation. The sets were disjoint at BLOCKER grade, not in what each saw. Neither review alone was sufficient, and they graded the same-paragraph clause differently — one a blocker, one non-blocking. That disagreement is recorded rather than resolved silently, because a future reader deserves to know the finding was contested.
