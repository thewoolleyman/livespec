...
The doctor static phase's `gherkin-blank-line-format` check MUST exempt `minimal`-rooted projects whose `SPECIFICATION.md` does NOT contain any fenced ` ```gherkin ` blocks. The exemption avoids false-positive failures for spec content that has no scenario-style narrative. When `SPECIFICATION.md` does contain gherkin blocks, the check MUST apply normally.
