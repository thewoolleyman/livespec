...
The `livespec` template root MUST contain (a) `template.json` with `template_format_version: 1`, `spec_root: "SPECIFICATION/"`, and the doctor LLM-driven prompt fields, (b) `prompts/<sub-command>.md` for each REQUIRED prompt (`seed`, `propose-change`, `critique`, `revise`), (c) `livespec-nlspec-spec.md` as the canonical NLSpec discipline reference document, and (d) `specification-template/` holding starter content scaffolding for new spec trees. See §"Non-functional-requirements file" below for the new sixth member of the seedable spec-file set.
