...
Every `livespec`-template-rooted spec tree MUST contain the file set `{README.md, spec.md, contracts.md, constraints.md, scenarios.md}` at the spec root. Missing any of these files is a doctor static-phase `template-files-present` failure. Sub-spec trees under `<spec_root>/templates/<name>/` follow the same file-set requirement uniformly per v020 Q1.
