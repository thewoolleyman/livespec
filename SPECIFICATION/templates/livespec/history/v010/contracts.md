...
- `headings_derived_from_intent` — the prompt MUST derive top-level `#` headings within each spec file from intent nouns, NOT from a fixed taxonomy. (Asserts the v020 Q4 starter-content policy; aligns with SPECIFICATION/constraints.md §"Heading taxonomy" which pins intent-derivation to level 1.) **Assertion function**: every entry in `replayed_response.files[]` has a non-empty content body whose first non-blank line begins with `# ` (an H1 header).
