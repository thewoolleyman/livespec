...
plus the rules that have no other home — the session-end plan-anchor path rule
and the overseer / long-running-coordinator discipline.

## Session-end plan-anchor path rule

When a session advanced a **standing plan** — a plan anchored by a ledger epic
and a write-once git metadata file at `plan/<topic>/epic.md` — the session's
closing recap MUST end by printing the exact plan anchor path **verbatim, as the
LAST line of the recap** (nothing after it):
...
path is the runtime-neutral plan anchor: Claude Code, Codex, and a human can
open the same file to find the epic id, while mutable handoff state lives in the
epic's append-only, attributed ledger entries. Slash-command or skill-invocation
syntax is runtime-specific; mention it earlier in the recap only when it is
valid for the current runtime and helpful, never as the last-line plan anchor.

This operationalizes `SPECIFICATION/non-functional-requirements.md` §"Planning
Lane guidance" → "Ledger-held planning state" at the agent-instruction layer by
treating `epic.md` as the portable plan anchor and the plan epic's attributed
ledger-entry timeline as the hand-forward state. If the session advanced the
plan materially, append or update the appropriate ledger entry and preserve any
supporting research under `plan/<topic>/research/` before printing the anchor
path. Legacy `handoff.md` files are historical evidence only after migration,
not the live hand-forward artifact.

## Planning-lane continuation rule

A plan epic's latest hand-forward ledger entry is executable coordination, not
a suggestion to stop. When a resumed plan entry names a next action, execute
that action automatically unless the user explicitly asked only for status,
review, or analysis. This includes chaining to another livespec-orchestrator
operation such as `groom`, `capture-work-item`, `propose-change`, or `drive`
when the ledger entry names it.

Do not stop merely because a plan anchor, research note, or ledger entry was
refreshed, validated, committed, or merged. Keep going until the active
operation is genuinely blocked, the plan closes, the work reaches a
session-rotation boundary, or the user's newest message redirects the session.
For a plan, `groom` stays in
...
When a resumed plan ledger entry names a maintainer-gated action, do not treat
that state as blocked or inactive. Treat it as the next actionable step that
requires an explicit maintainer confirmation before execution.
...
- **Rotate the role before ~50% context — don't hoard it to exhaustion.** The
  coordinator role is fully resumable from the plan anchor
  (`plan/<topic>/epic.md`) plus the plan epic's attributed ledger entries:
  record the non-derivable hand-forward facts in the ledger and hand the role to
  a FRESH session, which resumes lean via the anchor path. There is no reason to
  drive a coordinator to 80%+ and autocompact — that is the concrete failure
...
- **Keep the coordinator lean by construction.** Capture session panes with
  ...
  authoring, migrations, and hand-forward ledger/research writes to sub-agents
  — their context, not the coordinator's. Keep status updates terse: a one-line
  tick per routine event, full detail only at milestones and blockers.
...
- **You are restarted ONLY when YOU declare yourself `ready` — and that is the
  ...
  `read <repo>/plan/<topic>/epic.md, then resume from the plan epic's ledger entries`.
...
- **`plan/<topic>/epic.md` plus the plan epic's ledger entries are what you hand
  forward.** Never leave your resume state in a scratchpad file, a `/tmp` path,
  or this transcript — the next session inherits NONE of it. If your real pending
  work has drifted from the latest ledger entry, append a new attributed ledger
  entry with only non-derivable facts (rationale, warnings, abandoned attempts,
  pointers) and preserve supporting research under `plan/<topic>/research/`.
  Legacy `handoff.md` files, when present, are write-once historical evidence,
  not mutable resume state.
...
- **Never reason your way out of declaring.** A session once did exactly that: it
  ...
  stopped — wedging its track idle at 13%. The right move under the ratified plan
  shape is to append the missing hand-forward ledger entry, verify `epic.md` names
  the plan epic, and declare `ready`. If the resume path looks wrong, FIX THE
  plan anchor or ledger entry; the state file is not the place to register an
  objection.
...
- **`plan`-routed impl** — when a plan matures a piece into ledger
...
**Stays in Claude (NOT factory-dispatched):** the plan itself,
...
- **Tracked-session wrap-up contract (overseer)** — if the overseer daemon pastes
  ...
  `<repo>/tmp/overseer/<topic>/.overseer-state`. `plan/<topic>/epic.md` plus the
  plan epic's attributed ledger entries are the live hand-forward state (never a
  scratchpad); legacy `handoff.md` is historical evidence only after migration. If
  the resume path looks stale, fix the plan anchor or append the missing ledger
  entry; never withhold the declaration to register an objection. Declaring
  nothing does not kill you and does not save you: it gets you reported to a
  human as not responding and wedges your own track. Detail: this file
