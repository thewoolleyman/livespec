...
- `livespec` MAY read git state (read-only `git status` /
  ...
  2. The `revise` and `seed` wrappers, which read
     ...
     - `IOFailure(GitUnavailableError)` when the git binary is
       ...
       (or set `LIVESPEC_REVISER_LLM` and proceed if human
...
- **`LIVESPEC_REVISER_LLM`** (default unset). When set and
  non-empty, the `revise`, `critique`, and `propose-change`
  wrappers use this value for the `reviser_llm` / `<author>`
  field unconditionally, overriding any LLM self-declaration in
  the JSON payload and the `unknown-llm` fallback. See the
  precedence rules in §"critique", §"propose-change → Author
  identifier resolution", and the `reviser_llm` field definition
  in §"Revision file format".
...
- **Author identifier resolution.** The file-level `author` field
  ...
  the following precedence (mirroring the `reviser_llm`
  resolution for `critique` / `revise` / seed auto-capture):
  ...
  is reached. The reserved `livespec-` prefix namespace convention
  (see §"Proposed-change file format") still applies: user-
