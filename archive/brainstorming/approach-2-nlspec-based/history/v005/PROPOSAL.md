...
│   ├── doctor.md
│   └── prune-history.md
├── shared/                            # reusable content referenced by commands/ via @-references;
│                                      # membership is an implementation concern driven by DRY needs
├── scripts/
│   ├── bash-boilerplate.sh            # shared strict-mode + trap boilerplate (sourced by every bash script)
│   ├── dispatch                       # sub-command dispatcher (executable; no .sh extension)
│   └── doctor/
│       ├── run-static                 # orchestrator for doctor's static phase (executable; no .sh extension)
│       └── static/                    # one executable per static check (no .sh extension; see §"doctor" below)
...
│   └── critique-findings.json         # additional schemas added as post-seed propose-changes
...
- Sub-command dispatch: the dispatcher script at `scripts/dispatch`
  tokenizes `$@` (POSIX-style) and emits the resolved sub-command name
  plus remaining positional arguments on stdout. It exits nonzero with
  a diagnostic on unknown sub-command, mismatched args, or missing
  required args. The LLM is instructed on
  `commands/<sub-command>.md` after routing.
...
- Bash scripts bundled with the skill MUST comply with the
  `bash-skill-script-style-requirements.md` companion document
  (destined for `SPECIFICATION/constraints.md`).
- The skill bundle contains runtime-only assets. Developer-time
  tooling (architectural-check scripts, Makefile, `.mise.toml`,
  `.pre-commit-config.yaml`) lives outside the bundle under
  `<livespec-repo-root>/dev-tooling/`. See §"Developer tooling
  layout" below.
...
- `bash >= 3.2` is a hard runtime dependency. The boilerplate
  MUST assert this at script start; if the running bash is too
  old, MUST exit `127` with an actionable install instruction.
  Bash 3.2 is the floor because macOS ships bash 3.2.57 at
  `/bin/bash` on every supported release (Apple has not updated
  bash since 2007 due to GPLv3). The skill's scripts execute on
  the end user's machine, so bash 3.2 compatibility is a core
  portability premise.
...
Every tool the enforcement suite requires — shellcheck, shfmt,
shellharden, shellmetrics, kcov, bats-core, bats-assert,
bats-support, tree-sitter-bash, and bash itself pinned to
`3.2.57` — is managed via `mise`
...
once produces a ready-to-run environment on Linux. Pinning bash
to `3.2.57` ensures livespec developers and CI run the same bash
version macOS end users have, so accidental bash 4.x-only
constructs fail at authoring time rather than silently on user
machines. Developer tooling is NOT installed into user projects;
it is purely livespec-maintainer-facing.
...
- `--skip-pre-check` on any sub-command acts as a per-invocation
  ...
  skip = config OR CLI flag. The skill MUST surface a warning to
  the user via LLM narration. The raw record at the skill's
  structural layer is a JSON finding
  (`status: "skipped"`, `message: "pre-step checks skipped by user
  config or --skip-pre-check"`). The bash layer MUST NOT print the
  warning to stdout (stdout is reserved for the
...
- `prompts/` is a fixed-by-convention path. `prompts/seed.md`,
  ...
  `prompts/critique.md` are all REQUIRED.
...
- Doctor check #2 validates the resolved template directory has
...
Sub-commands MAY internally delegate to another sub-command's
logic (e.g., `critique` delegates file-creation to
`propose-change`'s internal logic). Internal delegation does NOT
re-trigger pre-step or post-step doctor; the outer sub-command's
doctor lifecycle covers the whole invocation.
...
- Creates a new file
  ...
  file format" below).
- `<intent>` is raw freeform text. The skill invokes the active
  template's `prompts/propose-change.md` to reshape it into a
  JSON findings array, then maps each finding to one
  `## Proposal` section.
- External programmatic callers that already have structured
  findings JSON use the `--findings-json <path>` flag instead of
  `<intent>`; the skill validates the JSON against
  `schemas/critique-findings.json` and maps findings directly to
  `## Proposal` sections without invoking the template prompt.
- Within-skill callers (e.g., `critique`) delegate to
  `propose-change`'s internal logic directly, bypassing the CLI
  (see §"livespec skill sub-commands" on internal delegation).
...
- The skill then delegates file-creation to `propose-change`'s
  internal logic (not a sub-command re-invocation) with topic
  `<author>-critique` and the structured findings as input.
  Internal delegation skips the inner pre/post doctor cycle
  since the outer `critique` invocation's doctor lifecycle
  already covers the whole operation.
...
`doctor` runs in two phases. The static phase is implemented as a
family of per-check executables under
`.claude-plugin/skills/livespec/scripts/doctor/static/<slug>`,
orchestrated by `scripts/doctor/run-static`. The LLM-driven phase
runs only after the static phase passes.

#### Static-phase structure

Each static check is a standalone executable at
`scripts/doctor/static/<slug>` (no extension, executable bit,
sourceable guard per the bash style doc). Each check:

- Emits a JSON findings array on stdout (empty array `[]` if
  pass; one or more finding objects if fail).
- Exits `0` (pass) / `3` (fail) / `1` (script-internal error).
- Sources `../../bash-boilerplate.sh`.
- Has a corresponding `.bats` test file under
  `tests/scripts/doctor/static/<slug>.bats`.

The orchestrator `scripts/doctor/run-static`:

- Iterates `scripts/doctor/static/` by glob (lexicographic order).
- Runs each check as a subprocess.
- Aggregates the per-check JSON findings into one
  `{"findings": [...]}` payload on stdout.
- Exits `0` if every child exited `0`, `3` if any exited `3`,
  `1` if any exited `1`. When multiple failure modes mix, the
  rule is: `1` beats `3` beats `0` (internal errors surface first).

#### Static-phase output contract

The orchestrator writes structured JSON to stdout and returns an
exit code per the "Static-phase exit codes" table below. Output
shape:
...
`check_id` takes the form `doctor-<slug>` where `<slug>` matches
the per-check executable's filename. The `doctor-` prefix
namespaces the ID so future check categories don't collide.

Check insert/delete is a non-event: a new check simply adds a new
file under `static/`; a deprecated check is deleted and its slug
disappears. No stable-number invariant to guard.

No human-readable output is produced on stdout; the skill's LLM
narrates the findings to the user from the JSON. Per the bash
skill script style requirements, diagnostic text MAY appear on
stderr.

#### Static-phase checks (exit `3` on any failure)

Each check below is implemented as one executable at
`scripts/doctor/static/<slug>`. The slug is the filename.
...
- **`version-directories-complete`** — Every `history/vNNN/`
...
- **`version-contiguity`** — Version numbers in `history/` are
  contiguous from `pruned_range.end + 1` (if `PRUNED_HISTORY.json`
  exists at the oldest surviving `vN-1` directory) or from `v001`
  upward. Numeric parsing.
- **`revision-to-proposed-change-pairing`** — Every revision file
  in `history/vNNN/proposed_changes/` has a corresponding
  proposed-change file with the same topic in the same directory.
- **`proposed-change-topic-format`** — Every file in the working
  `proposed_changes/` has a name conforming to `<topic>.md`.
- **`out-of-band-edits`** — The diff between committed spec state
  (`git show HEAD:<path>` for each spec file) and the latest
  `history/vN/` copies is empty. Behavior if non-empty:
  * If the project is not a git repo: skip this check with a
    JSON finding marked `status: "skipped"` (no committed state
    to compare).
  * Otherwise: the script auto-backfills a missing version. It
    creates
    ...
    corresponding location. It writes `history/v(N+1)/` with the
    current committed spec content. It then moves the
    proposed-change and revision into
    `history/v(N+1)/proposed_changes/`. Author identifier:
    `livespec-doctor`. Script exits `3` with a finding
    instructing the user to commit the new `history/v(N+1)/`
    files and re-run the original command. WIP in the working
    tree is unaffected by this flow — only committed state
    drives detection.
- **`bcp14-keyword-wellformedness`** — Misspelled uppercase BCP 14
  keywords from a finite list (`MUSNT`, `SHOLD`, `SHAL`, etc.) and
  mixed-case BCP 14 keywords appearing as standalone words
  (`Must`, `Should`, `May`). Sentence-level context-dependent
  checks move to the LLM-driven phase (see below).
...
- **`anchor-reference-resolution`** — All Markdown links in all
  files under `SPECIFICATION/` (spec files, READMEs,
  proposed-change files, revision files) with anchor references
  resolve to existing headings in the referenced files.

#### Static-phase exit codes

- `0`: all checks pass; LLM-driven phase MAY proceed.
- `1`: script-internal failure (bug in `run-static` or any
...
`bash-skill-script-style-requirements.md` contract. Static
checks are pass-or-fail only. There is no warning tier.
...
- Test runner is `bats-core`. Test files use the `.bats`
  extension.
- Assertions MUST use `bats-assert` (with `bats-support` as its
  dependency). Bare `[[ ... ]]` / `[ ... ]` assertions inside
  `@test` blocks are forbidden because they produce opaque
  failure messages; `bats-assert` produces structured diffs that
  surface usefully in the LLM's context.
- Filesystem fixtures live under `tests/fixtures/` (tests MUST
  NOT mutate them). Test-local state lives under
  `BATS_TEST_TMPDIR` (bats-provided).
- Coverage is measured by `kcov`. Pure libraries MUST achieve
  ≥ 95% line coverage; overall coverage across all bundled bash
  files MUST be ≥ 80%. See the companion
  `bash-skill-script-style-requirements.md` for the full
  coverage contract. `kcov` is Linux-only; developers on
  non-Linux hosts invoke the coverage gate through a pinned
  Linux-container wrapper (`make check-coverage` handles both
  paths transparently).
...
├── scripts/                                  (mirrors .claude-plugin/skills/livespec/scripts/)
│   ├── dispatch.bats
│   └── doctor/
│       ├── run-static.bats
│       └── static/                           (one .bats per static-check slug)
│           ├── livespec-jsonc-valid.bats
│           ├── template-exists.bats
│           ├── ...
│           └── anchor-reference-resolution.bats
├── dev-tooling/                              (mirrors <repo-root>/dev-tooling/)
│   └── enforcement/                          (one .bats per enforcement script)
│       ├── check-library-headers.bats
│       ├── check-source-graph.bats
│       └── ...
└── test_*.bats                               (per-specification-file rule-coverage tests)
```

Test organization rules:

- `tests/scripts/` mirrors `.claude-plugin/skills/livespec/scripts/`
  one-to-one, preserving subdirectory structure. `tests/dev-tooling/`
  mirrors `<repo-root>/dev-tooling/` the same way. Adding a script
  anywhere under those trees requires adding the corresponding
  `.bats` file at the mirrored path.
- **Per-script tests** (under `tests/scripts/` and
  `tests/dev-tooling/`) exercise a script's **contract**: exit
  codes, JSON schema of stdout, input validation, error paths,
  individual helper functions.
- **Per-spec-file tests** (`tests/test_spec.bats`,
  `tests/test_contracts.bats`, `tests/test_constraints.bats`,
  `tests/test_scenarios.bats`) exercise **rules stated in a
  specification file** end-to-end, invoking whatever script(s)
  implement the rule. These are the tests referenced by
  `heading-coverage.json`; per-script tests are not required to
...
- Each test MUST execute the skill against fixtures in a
  ...
  `SPECIFICATION/`.
- A meta-test, `test_meta_section_drift_prevention.bats`, MUST
...
- **Coverage registry**: `tests/heading-coverage.json` maps
  ...
  ```json
  [
    { "heading": "Proposed-change file format", "test": "test_propose_change.bats::test_multi_proposal_write" }
  ]
  ```
...
implementation concern, not specified here. Full bash- and
bats-specific constraints (complexity thresholds, purity
classification, enforcement-suite scripts under
`dev-tooling/enforcement/`, etc.) are in
`bash-skill-script-style-requirements.md`, destined for
...
├── dev-tooling/
│   ├── bash-boilerplate.sh                    (symlink to .claude-plugin/skills/livespec/scripts/bash-boilerplate.sh)
│   └── enforcement/                           (architectural-check executables)
...
├── Makefile                                   (enforcement-suite entry points; see below)
├── .mise.toml                                 (pins bash=3.2.57 and every enforcement tool)
└── .pre-commit-config.yaml                    (calls Makefile targets against staged files)
```

Key conventions:

- The enforcement suite is **invocation-surface-agnostic**: every
  check is a `make check-*` target, invoked by pre-commit hooks,
  pre-push hooks, CI, or manually at the shell. Pre-commit,
  pre-push, CI, and manual invocation are consumers of the same
  targets, not separate owners of the checks.
- The enforcement suite runs on **Linux**. Developers on non-Linux
  hosts use pinned Linux-container wrappers (for kcov and any
  future Linux-only check); `make check-coverage` dispatches
  transparently.
- Bash itself is pinned to `3.2.57` in `.mise.toml` (see
  §"Dependencies" above) so developers and CI run the same bash
  version macOS end users have, catching any accidental bash 4.x
  construct at authoring time.
- Enforcement-suite check scripts under `dev-tooling/enforcement/`
  are standalone executables (no `.sh` extension; executable bit;
  sourceable guard) and MUST themselves comply with every rule in
  `bash-skill-script-style-requirements.md`.
- Full enforcement-suite contract (canonical Makefile target list,
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   files, shared files, scripts — including `bash-boilerplate.sh`,
   `dispatch`, `doctor/run-static`, and the per-check executables
   under `doctor/static/` — schemas, and the built-in
...
6. The full `doctor` static-check suite (every check listed in
   §"Static-phase checks") is implemented as per-check
   executables under `scripts/doctor/static/<slug>`, orchestrated
   by `scripts/doctor/run-static`, with documented exit-code
   semantics (0/1/3) and the JSON output contract using
   `doctor-<slug>` check_ids.
...
10. The test suite covers every sub-command, both doctor phases,
    ...
    `test_meta_section_drift_prevention.bats` meta-test. Test
    trees mirror the implementation trees one-to-one.
11. `jq` presence check is implemented; the skill aborts at
    start with an OS-appropriate message if absent. Bash 3.2+
    version assertion is implemented in `bash-boilerplate.sh`;
    the skill exits 127 on bash too old.
12. Developer tooling is in place under `<repo-root>/dev-tooling/`:
    enforcement check executables, Makefile entry points,
    committed `.mise.toml` pinning `bash=3.2.57` plus every
    enforcement tool, and a `.pre-commit-config.yaml` calling
    the Makefile targets.
13. `livespec` dogfoods itself: `<project-root>/SPECIFICATION/`
...
14. The skill bundle includes its own `help` text covering every
    sub-command.
...
2. Implement the plugin skeleton: `plugin.json`, `SKILL.md`,
   per-command files, `scripts/bash-boilerplate.sh`,
   `scripts/dispatch`, `scripts/doctor/run-static` plus the
   per-check executables under `scripts/doctor/static/`, the `jq`
   and bash-version presence checks, and the minimum subset of the
...
5. Land the v1 Definition of Done items as the spec evolves.
   ...
   `prior-art.md`, `bash-skill-script-style-requirements.md`,
   etc.) into appropriate appendices or references within the
...
- Authoring of the specific enforcement-check scripts under
  `<repo-root>/dev-tooling/enforcement/`, the Makefile entry
  points that invoke them, the committed `.mise.toml`, and the
