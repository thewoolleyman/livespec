...
│   │   ├── returns_pyright_plugin/      # dry-python/returns pyright plugin (BSD-2) — v018 Q4
...
│   │   └── typing_extensions/           # (PSF-2.0) — minimal shim exporting override + assert_never (v013 M1)
...
- **Vendored pure-Python libraries** bundled with the skill, each
  ...
  - `dry-python/returns` (BSD-2) — ROP primitives (`Result`,
    ...
    `Fold.collect`).
  - `dry-python/returns` pyright plugin (BSD-2; v018 Q4) —
    vendored alongside the library at
    `.claude-plugin/scripts/_vendor/returns_pyright_plugin/`
    with its upstream LICENSE preserved. Pyright resolves the
    plugin via `[tool.pyright]`'s `pluginPaths =
    ["_vendor/returns_pyright_plugin"]` entry in
    `pyproject.toml`. Rationale: `Result` and `IOResult` are
    used pervasively (every public function returns one or the
    other per §"Architecture-level constraints"); without the
    plugin, strict-mode inference of the two-track composition
    forces routine `# type: ignore` usage, contradicting the
    style doc's "no `# type: ignore` without narrow
    justification" rule and the "Strongest-possible guardrails
    for agent-authored Python" principle. The plugin is a
    livespec-wide load-bearing guardrail, not an optional tool.
    Closes the `returns-pyright-plugin-disposition` deferred
    item in v018.
  - `fastjsonschema` (MIT) — JSON Schema Draft-7 validator.
  - `structlog` (BSD-2 / MIT dual) — structured JSON logging.
  - `jsoncomment` (MIT) — JSONC (JSON-with-comments) parser; used
    ...
    other JSONC input.
  - `typing_extensions` (PSF-2.0) — **vendored as a minimal shim**
    at `_vendor/typing_extensions/__init__.py` (v013 M1). The
    shim exports exactly `override` (no-op decorator per PEP 698)
    and `assert_never` (raises `AssertionError` at runtime per
    PEP 702-ish). Module-named `typing_extensions` so pyright
    recognizes the import path for `reportImplicitOverride` (L2)
    and the `Never`-narrowing of `assert_never` (L7). Preserves
    attribution to upstream at
    `https://github.com/python/typing_extensions`; the shim's
    `LICENSE` file is a verbatim copy of the upstream PSF-2.0
    license. Future livespec revisions MAY widen the shim to
    re-export additional `typing_extensions` symbols (each
    addition is a one-line edit); re-vendoring the full upstream
    is a future option tracked as a scope-widening decision, not
    a v013 default.
...
- **Initial-vendoring exception (one-time, v018 Q3).** The first
  ...
  `fastjsonschema`, `structlog`, `jsoncomment`, and the v018 Q4
  addition `returns_pyright_plugin`) is a one-time MANUAL
  procedure, distinct from the blessed `just vendor-update` path
  above:
  ...
  file by hand and copies the upstream LICENSE." The
  circularity the initial-vendoring exception resolves:
  `just vendor-update <lib>` invokes Python through
  `livespec.parse.jsonc` to read/write `.vendor.jsonc`, and
  `livespec.parse.jsonc` imports the vendored `jsoncomment`;
  the recipe cannot run before `jsoncomment` is already
  vendored.
...
`import-linter` — is managed via `mise`
...
The v012-added dev tools (mise-pinned, NOT vendored in the bundle):
...
- **`hypothesis`** (HypothesisWorks/hypothesis, MPL-2.0) — property-
  ...
  for mise-pinned dev-time deps; the vendored-libs license policy
...
These are mise-pinned because they are test-time / dev-tool-only
deps and follow the same packaging convention as `pytest`,
`pytest-cov`, `pytest-icdiff`. They are NOT imported by
`.claude-plugin/scripts/livespec/**` at user-runtime, so bundle vendoring would be
mis-applied.

Note: `typing_extensions` is NOT in this mise-pinned list. It is
...
Sub-spec tree shape (structurally identical to a main spec tree
per the template's own conventions):
...
└── SPECIFICATION/                          (main spec, described above)
    ...
    └── templates/                          (v018 Q1 addition)
        ...
        │   ├── README.md                   (optional; follows the livespec-template convention)
        ...
        │   └── history/                    (per-sub-spec versioned snapshots)
...
- **Pre-seed template selection (v014 N1; pre-seed template
  resolution mechanism codified in v017 Q2).** When
  ...
  a custom template path. The LLM then resolves the
  chosen template's directory by invoking
  `bin/resolve_template.py --project-root . --template
  <chosen>` via Bash (per the v017 Q2 addition to
  §"Template resolution contract"); the wrapper's stdout is
  the absolute template directory path. The LLM uses that
  path to Read `<path>/prompts/seed.md` and proceeds with the
  normal seed template-prompt dispatch. The chosen value is
  ALSO passed into the wrapper via the seed-input payload's
  required top-level `template` field (see payload schema
  below); the wrapper uses that value to bootstrap
  `.livespec.jsonc` (per v016 P2). When `.livespec.jsonc` IS
  present, the wrapper reuses its existing `template` value
  (no dialogue is needed; SKILL.md prose MAY skip the prompt
  entirely and proceed with the normal `bin/resolve_template.py
  --project-root .` resolution that walks upward for the
  existing config). The pre-seed dialogue is the only
  SKILL.md-prose special case for handling an absent
  `.livespec.jsonc`; every non-seed sub-command's SKILL.md
  invokes `bin/resolve_template.py` without `--template` and
  gets exit 3 on a missing config (which is the correct
  behavior because non-seed sub-commands cannot usefully run
  without a seeded project).
...
- The LLM (per `seed/SKILL.md`) invokes the active template's
  ...
  **Sub-specs payload (v018 Q1).** The `sub_specs` field is a
  list (possibly empty) of `SubSpecPayload` entries, one per
  built-in template's sub-specification tree per
  §"SPECIFICATION directory structure — Template sub-
  specifications". Each entry carries the sub-spec's
  `template_name` (matching the template's directory name under
  `.claude-plugin/specification-templates/<name>/` — `"livespec"`
  or `"minimal"` for v1 built-ins) AND a `files[]` array with
  the sub-spec's spec-file content at
  `SPECIFICATION/templates/<template_name>/<spec-file>`. When
  the active main-spec template is `livespec` (multi-file), the
  seed prompt emits sub-spec payloads for BOTH built-in
  templates so the seeded repo carries full sub-spec trees
  ready for Phase 7 agent-generation. When the active main-spec
  template is `minimal` (single-file), the seed prompt MAY emit
  an empty `sub_specs: []` (sub-specs are a main-spec-side
  concept; a minimal-rooted repo doesn't typically ship its own
  built-in template sub-specs).  Custom-template authors MAY
  choose whether their seed prompts emit sub-spec payloads per
...
- `bin/seed.py --seed-json <path>` is the sole wrapper entry
  ...
  5. **(v018 Q1)** For each sub-spec tree, create
     ...
     Each sub-spec tree follows its OWN template's convention
     for whether a versioned `README.md` is captured: the
     `livespec` sub-spec ships a sub-spec-root `README.md`
     + a per-version README snapshot; the `minimal` sub-spec
     ships neither (mirroring the minimal template's
     no-README convention). The skill-owned
...
- Uses the **static registry** at
  ...
  composition.
- **Iterates over every spec tree (v018 Q1)**. At startup the
  orchestrator enumerates `(spec_root, template_name)` pairs: the
  main spec tree first (template-name `main`), then each
  sub-spec tree under
...
These fields preserve the fallback reason for bootstrap-critical
...
- Coverage is measured by **`coverage.py`** via `pytest-cov`.
  ...
  exclusions), and `<repo-root>/dev-tooling/**`. Vendored
  third-party libraries under `.claude-plugin/scripts/_vendor/` are excluded
  from the coverage measurement (they are pinned and audited
  via the discipline in §"Dependencies — Runtime"; no
  automated audit script). See the companion
  `python-skill-script-style-requirements.md` for the full
  coverage contract (100% line+branch; pragma escape hatch
  capped per file; no tier split). Note: v012 L13 mutation
  testing is a SEPARATE release-gate measurement; the line+
...
├── .mise.toml                                 (pins python3 >=3.10, just, lefthook, pyright, ruff, pytest, pytest-cov, pytest-icdiff, hypothesis, hypothesis-jsonschema, mutmut, import-linter — typing_extensions is vendored per v013 M1, NOT mise-pinned)
...
├── pyproject.toml                             (ruff + pyright + pytest + coverage + import-linter config; no build system; Import-Linter minimum configuration per v013 M7, narrowed to two contracts per v017 Q3)
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   + fastjsonschema + structlog + jsoncomment — and the
...
12. Developer tooling is in place under `<repo-root>/`:
    ...
    committed `.mise.toml` pinning every dev tool (including
    v012 additions: `hypothesis`, `hypothesis-jsonschema`,
    `mutmut`, `import-linter`; `typing_extensions` is vendored
    per v013 M1, NOT mise-pinned),
    ...
    only via `check-no-raise-outside-io`; **per v018 Q4,
    `[tool.pyright]` declares `pluginPaths =
    ["_vendor/returns_pyright_plugin"]`** pointing at the
    sixth vendored lib so pyright recognizes the dry-python/
    returns plugin for strict-mode `Result` / `IOResult`
    inference — closes
    `returns-pyright-plugin-disposition`; pyright is the
    chosen typechecker — closes
    `basedpyright-vs-pyright`),
    and `NOTICES.md` listing vendored libraries (extended with
    `typing_extensions` PSF-2.0 per v013 M1; mise-pinned dev
    tools are NOT in NOTICES.md).
...
4. Implement remaining sub-commands (`propose-change`,
   ...
   cycles against the seeded spec trees. Each cycle targets a
   specific tree via `--spec-target <path>` (default: main
   spec root).
...
**Bootstrap exception (v018 Q2).** The bootstrap ordering above
(steps 1-4, ending with the first `livespec seed` invocation in
step 3) lands imperatively. The governed
propose-change → revise loop becomes operable starting at
step 4 — after seed has produced the working
