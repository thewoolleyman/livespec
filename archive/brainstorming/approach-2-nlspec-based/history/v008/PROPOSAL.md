...
3. **Inputs.** CLI flags the sub-command accepts and what each
   ...
   under `scripts/livespec/schemas/` is named and any required
   validation step is called out.
4. **Steps.** An ordered list of LLM-driven steps. Each step is
   ...
   - Validate output against a schema (name the
     `livespec.validate.<name>.validate` entry point).
...
- The LLM (per `seed/SKILL.md`) invokes the active template's
  ...
  ```
  The LLM validates this output against the schema via
  `livespec.validate.seed_input.validate` (factory shape), writes
  the validated JSON to a temp file, then invokes `bin/seed.py
  --seed-json <tempfile>`.
- `bin/seed.py --seed-json <path>` is the sole wrapper entry
  ...
  and auto-captures the seed itself as a proposed-change.
...
- The LLM validates the JSON against the schema using
  `fastjsonschema` via
  `livespec.validate.proposal_findings.validate` (factory shape);
  re-invokes the prompt on malformed output (up to 3 retries)
  with error context; aborts on repeated failure.
...
- **`bin/revise.py` ONLY accepts `--revise-json <path>`**; it
  ...
  ```
  The LLM validates this against the schema via
  `livespec.validate.revise_input.validate` (factory shape),
  writes it to a temp file, then invokes
...
- **`out-of-band-edits`** — Compares committed active spec state
  ...
  * **Pre-backfill guard — uncommitted prior backfill present.**
    ...
    1. A `SPECIFICATION/history/v(N+1)/` directory (whether
  ...
  * **Comparison (both sides HEAD-committed).** Otherwise, the
    check diffs `git show HEAD:SPECIFICATION/<spec-file>` against
    `git show HEAD:SPECIFICATION/history/vN/<spec-file>` for each
  ...
  * **Backfill on drift.** If the diff is non-empty, the check
    ...
    corresponding location. It writes `SPECIFICATION/history/v(N+1)/`
    ...
    `livespec-doctor`. Check exits `3` with a finding instructing
    the user to commit the new `history/v(N+1)/` + backfill
    proposed-change files and re-run the original command.
