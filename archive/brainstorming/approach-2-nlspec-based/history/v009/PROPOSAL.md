...
6. **Failure handling.** Exit-code-to-narration mapping for the
   wrapper:
   - Exit `0` → proceed to the post-wrapper step.
   - Exit `1` → internal bug; surface the error and abort.
   - Exit `2` → usage error; restate the expected invocation and
     abort.
   - Exit `3` → precondition/input failure; surface the findings
     from the wrapper's stderr structlog line(s) and abort. The
     user is directed to the corrective action the finding
     describes.
...
- **`help` and `doctor` have neither pre-step nor post-step**
  wrapper-side static. `help` does no spec work; `doctor` IS the
  static phase.
...
- `bin/doctor_static.py` does NOT accept `--skip-pre-check`; it
  IS the static phase and has no pre/post wrap. Passing the flag
  to `doctor_static.py` results in an argparse usage error
  (exit 2). The skill prose for `doctor` does not forward this
  flag.
...
1. Run the LLM-driven phase per `commands/doctor.md` (skill
   prose).
2. Honor `--skip-subjective-checks` / `post_step_skip_subjective_checks`
   for the LLM-driven phase. **These flags are LLM-layer only**;
   they are never passed to Python wrappers, and Python wrappers
   never see them.
3. Findings from the LLM-driven phase are surfaced to the user
   per the rules in §"doctor → LLM-driven phase".
...
- `--skip-subjective-checks` is an **LLM-layer flag only**. It is
  never passed to a Python wrapper. The user passes it in dialogue;
  the LLM honors it when deciding whether to run the subjective
  portion of the post-step LLM-driven phase. Effective skip = config
  OR LLM-layer flag.
...
- **`LIVESPEC_REVISER_LLM`** (default unset). When set and
  ...
  overriding any LLM self-declaration in the JSON payload and
  the `unknown-llm` fallback. See the precedence rules in
  §"critique" and the `reviser_llm` field definition in §"Revision
  file format".
...
- `template.json` is REQUIRED and MUST contain at minimum
  `{"template_format_version": 1}`. A template MAY additionally
  declare `"spec_root": "<relative-path>"` (default
  `"SPECIFICATION/"`) naming the directory under the repo root
  that contains the spec files, `proposed_changes/`, and
  `history/`. Doctor static checks read `spec_root` from the
  resolved template's `template.json` via `DoctorContext.spec_root`
  and parameterize all their path references against it; a custom
  template that places spec files directly at the repo root sets
  `"spec_root": "./"`. No other fields are defined in v1.
...
- The LLM (per `seed/SKILL.md`) invokes the active template's
  ...
  failure, it exits 3 with structured findings on stderr and the
...
- **Auto-generated
  ...
  `created_at`, `reviser_llm: livespec-seed`, `reviser_human:
...
- **`bin/propose_change.py` ONLY accepts `--findings-json <path>`**;
  ...
  `<intent>` path is driven by the LLM per `propose-change/
  SKILL.md`: the LLM invokes the active template's
  `prompts/propose-change.md`, captures the output, writes the
  JSON to a temp file, then invokes `bin/propose_change.py
  --findings-json <tempfile> <topic>`. The wrapper validates
  the payload against the schema internally; on validation
  failure, it exits 3 with structured findings on stderr, and
  the LLM MUST re-invoke the template prompt with the error
  context, up to 3 retries.
...
- `<author>` defaults to a string identifying the current LLM
  ...
  2. Otherwise, the LLM self-declares its model identifier in
     ...
     `bin/critique.py` (the template prompt instructs the LLM
     to populate it with its best-effort model ID or
     `"unknown-llm"` as fallback).
  3. Otherwise, use the literal `"unknown-llm"`.
  A warning is surfaced whenever the fallback is reached via (3).
...
- The LLM writes the JSON to a tempfile and invokes
  `bin/critique.py --findings-json <tempfile> <author>`. The
  wrapper validates the payload against the schema internally;
  on validation failure, it exits 3 with structured findings on
  stderr, and the LLM MUST re-invoke the template prompt with
  the error context, up to 3 retries.
- `bin/critique.py` delegates internally to `propose_change`'s
  Python logic with topic `<author>-critique` and the structured
  findings as input. Internal delegation skips the inner pre/post
  doctor cycle since the outer `critique` invocation's wrapper
  ROP chain already covers the whole operation.
...
- **`bin/revise.py` ONLY accepts `--revise-json <path>`**; it
  never invokes the template prompt, the LLM, or the interactive
  confirmation dialogue. The LLM-driven per-proposal confirmation
  flow is entirely skill-prose-side (per `revise/SKILL.md`). Once
  every decision is confirmed, the LLM assembles a JSON payload
  conforming to `scripts/livespec/schemas/revise_input.schema.json`:
  ```json
  {
    "reviser_llm": "<LLM self-declaration or 'unknown-llm'>",
    "decisions": [
      {
        "proposal_topic": "foo",
  ...
  ```
  The optional `reviser_llm` field is the LLM's best-effort
  self-identification, used per the precedence defined in
  §"Configuration: `.livespec.jsonc` — Environment variables"
  (env var `LIVESPEC_REVISER_LLM` wins if set; else this field;
  else `"unknown-llm"`). The LLM writes the payload to a temp
  file and invokes `bin/revise.py --revise-json <tempfile>`.
  The wrapper validates the payload against the schema
  internally; on validation failure, it exits 3 with structured
  findings on stderr, and the LLM MUST re-assemble (or
  re-prompt) and retry, up to 3 retries.
...
- On `IOFailure(err)`: emit a structured-error JSON line on stderr
  via `structlog`, then exit `err.exit_code` (typically `1` for
  `LivespecError`).
...
`doctor/SKILL.md`, not a script. It has no exit codes. Its
...
`--skip-subjective-checks` LLM-layer flag):
...
1. YAML front-matter (file-level):
   ...
   **Author-identifier namespace convention:** identifiers with
   ...
   `livespec-doctor`) are **reserved** for automated
   skill-tool authorship. Human authors and LLMs MUST NOT use
   this prefix; they use any non-`livespec-`-prefixed identifier
   (e.g., `thewoolleyman`, `claude-opus-4-7-1m`,
   `unknown-llm`). The reservation is pattern-validated by the
...
1. YAML front-matter:
   ...
   reviser_human: <git user.name and user.email from `git config`, or "unknown">
   reviser_llm: <resolved LLM id (env var / payload / "unknown-llm"), or reserved livespec-<tool>>
   ---
   ```

   `reviser_human` is populated by the `revise` / `seed` wrappers
   via `livespec.io.git.get_git_user()` (see §"Git"); the
   fallback is the literal `"unknown"` when git is unavailable
   or the config values are unset.

   `reviser_llm` is populated per precedence:
   (1) env var `LIVESPEC_REVISER_LLM` if set and non-empty;
   (2) the LLM's self-declared `reviser_llm` field in the
   wrapper-input JSON payload if present and non-empty;
   (3) the literal `"unknown-llm"`. For automated skill-tool
   authored revisions (e.g., `seed` auto-capture,
   `out-of-band-edits` auto-backfill), `reviser_llm` takes a
   reserved-prefix value (`livespec-seed`, `livespec-doctor`)
   set directly by the wrapper, bypassing the precedence above.

   **Author-identifier namespace convention** applies here
   identically to §"Proposed-change file format": `livespec-`
   prefix reserved for skill-tool authorship; all other
   authorship uses non-reserved identifiers.
...
- **Vendored libraries are version-pinned, not hash-audited.**
  `.vendor.toml` records the exact upstream ref for each vendored
...
6. The full `doctor` static-check suite (every check listed in
   ...
   (`Fold.collect`), with documented exit-code semantics (0/1/3),
   the supervisor's findings-to-exit-code derivation, and the JSON
   output contract using `doctor-<slug>` check_ids.
7. The `doctor` LLM-driven phase's objective and subjective
   check categories are implemented at the skill layer
   (`doctor/SKILL.md`) with the LLM-layer
   `--skip-subjective-checks` skip mechanism for subjective.
