...
│       ├── validate/                    # pure validators (Result-returning, factory shape); 1:1 paired with schemas/ (v013 M6)
│       │   ├── doctor_findings.py
...
- Coverage is measured by **`coverage.py`** via `pytest-cov`.
  ...
  third-party libraries under `scripts/_vendor/` are excluded
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   walking schema ↔ dataclass ↔ validator), `context.py` (with
   `DoctorContext.spec_root`), `types.py` (canonical NewType
...
2. Implement the plugin skeleton: `plugin.json`, per-sub-command
   ...
   `.claude-plugin/scripts/livespec/`, vendored libraries under `scripts/_vendor/`,
...
5. **Process every entry in `deferred-items.md`** by filing
   ...
   the restricted-YAML parser (`front-matter-parser`), and the
