...
v033 D2, plus the rest of `just check`) becomes binding at
the moment lefthook is installed into `.git/hooks/`. Per
...
│       ├── red_output_in_commit.py            (v032 D4 / v033 D4: every feature/bugfix commit body MUST contain a `## Red output` heading + fenced pytest output block; hard gate from v033-codification onward, not informational; lefthook pre-commit only, NOT in `just check`)
