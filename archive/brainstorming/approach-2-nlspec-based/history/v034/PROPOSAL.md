...
mechanical replay verification. The pre-commit hook
...
### Baseline-grandfathered violations (v034 D6)

A file at `<repo-root>/phase-5-deferred-violations.toml`
enumerates currently-failing violations that are
grandfathered during the v034 D7 drain phase. The local
pre-commit hook runs the FULL canonical-target list (not a
thinned aggregate). Each check loads the baseline and skips
violations whose `(target, file, rule, location)` tuple
matches an entry. NEW violations (not in the baseline) fail
the hook unconditionally. Each drain cycle that fixes a
violation ALSO removes the matching baseline entry in the
same commit.
...
1. **Created at v034 codification.** The replay-hook
   activation commit also writes the initial baseline file
   enumerating every violation that fails the full
   canonical-target run at that moment.
2. **Drained per fix.** Each drain-cycle commit removes the
   `[[violation]]` block(s) it resolves. The hook verifies
   removal: a baseline entry SHOULD be removed when the
   underlying violation is resolved (CHECK passes after
   the fix); a removal without resolution causes the next
   commit to fail because the now-unmasked violation is
   detected.
3. **Deleted when empty.** When the file contains zero
   `[[violation]]` blocks, the cycle that removed the
   final entry ALSO deletes the file. The absence of the
   file means full-scope-no-grandfather is in effect; the
   local pre-commit aggregate runs the full canonical list
   without any carve-outs.

**CI semantics.** CI does NOT load the baseline. The full
canonical target list runs under fail-fast: false; failing
targets fail their CI job. Baseline grandfathering is a
local-development affordance only — once a fix lands and
the baseline entry is removed, CI sees the unmasked-then-
masked transition just as a single passing target.

**Why mechanical-not-prose.** The v033 thinned aggregate
encoded "what's deferred" in a 20-line justfile comment.
This was unstructured (not parseable by tooling), drift-
prone (cycles re-adding targets to the aggregate had to be
matched against the comment by hand), and opaque to the
hook (the hook ran the thinned list; what was deferred was
invisible inside check execution). The TOML baseline
inverts these: structured (parseable), drift-resistant (the
hook checks the file as it runs each check), and
transparent (each violation has a `note` field naming the
fix plan).
