...
- Coverage is measured by **`coverage.py`** via `pytest-cov`.
  ...
  coverage contract (100% line+branch; no line-level pragma
  escape hatch). Note: v012 L13 mutation testing is a
...
installed into `.git/hooks/` — which is the Phase 5 sub-step
that promotes `just bootstrap` from its Phase-1 placeholder
echo to the real recipe (`lefthook install &&
ln -sfn ../.claude-plugin/skills .claude/skills`).
Pre-Phase-5 commits are grandfathered (they precede the test
suite). From the Phase-5 lefthook-install commit onward, no
commit lands without passing the per-commit gate; the Phase
6 first self-application seed is the first working commit
subject to the discipline at full strength.
