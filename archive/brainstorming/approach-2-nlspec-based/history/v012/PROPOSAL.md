...
- **`hypothesis`** (HypothesisWorks/hypothesis, MPL-2.0) — property-
  ...
  `python-skill-script-script-style-requirements.md` §"Property-
...
`livespec/**` at user-runtime, so bundle vendoring would be
mis-applied.
...
├── .mise.toml                                 (pins python3 >=3.10, just, lefthook, pyright, ruff, pytest, pytest-cov, pytest-icdiff, hypothesis, hypothesis-jsonschema, mutmut, import-linter)
├── .vendor.jsonc                               (records exact upstream version pinned for each vendored library)
├── pyproject.toml                             (ruff + pyright + pytest + coverage + import-linter config; no build system)
...
10. The test suite covers every sub-command, both doctor phases,
    ...
    + branch coverage across `scripts/livespec/**`,
    `scripts/bin/**`, and `dev-tooling/**`. Per v012 L12,
    ...
    `check-pbt-coverage-pure-modules`). Per v012 L13, the
    release-tag CI workflow runs `just check-mutation` against
    `livespec/parse/` and `livespec/validate/` with a ≥80%
    mutation kill-rate threshold (NOT in `just check`; NOT
    per-commit).
...
12. Developer tooling is in place under `<repo-root>/`:
    ...
    contracts per L15a),
    and `NOTICES.md` listing vendored libraries (unchanged
    in v012; mise-pinned dev tools are NOT in NOTICES.md).
