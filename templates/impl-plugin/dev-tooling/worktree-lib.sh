...
# every clone. The commit-refuse gate (refuse-primary-commit.sh) uses the
# exact same test so the gate and the lifecycle helpers always agree on what
# "primary" means.
#
...
# linked worktree. Mirrors refuse-primary-commit.sh exactly.
worktree_is_primary() {
