...
> **Coordinator self-discipline lives in `.ai/agent-disciplines.md`** — read its
> overseer / long-running-coordinator discipline section alongside this skill. In
> short: rotate the overseer role before ~50% context (refresh the handoff and
> hand off to a FRESH session; don't hoard the role to autocompact), never park
> ready work behind a "my context is heavy" excuse (track work runs in each
> session's OWN context), and keep yourself lean by `tail`-capturing panes and
> delegating heavy authoring to sub-agents.
