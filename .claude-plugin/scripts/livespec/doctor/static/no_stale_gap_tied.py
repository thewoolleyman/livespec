...
Cross-boundary mechanism:
  ...
  Finding (never `fail`). See `_work_items_provider.py`.

  The current-gap-id set is derived by applying the same MUST /
  MUST NOT / SHOULD / SHOULD NOT enumeration + stable-hash gap-id
  derivation that `livespec-impl-plaintext`'s `detect-impl-gaps`
  applies, against the live `<spec-root>/` content. The derivation
  MUST stay in lockstep with the impl-plugin's; a future refinement
  will replace the duplicated logic with a subprocess invocation of
  `<impl-plugin>:detect-impl-gaps --json` (or extract the shared
  detector into `livespec_runtime`).
"""
...
_SLUG_PREFIX: str = "no-stale-gap-tied"
...
def run(*, ctx: DoctorContext) -> IOResult[Finding, LivespecError]:
    ...
    wrapper (per `ctx.work_items_provider`), enumerates the live
    spec's current gap-id set, and warns when an open gap-tied
    work-item's gap_id is absent from that set. A connection-class
    failure or an unset provider yields a `skipped` Finding.
    """
