...
Per the v2 manifest mechanism in SPECIFICATION/spec.md §"Template
manifest" and contracts.md §"Template manifest wire contract":
v2 templates additionally declare `spec_files` — a mapping from
spec-target-relative path strings to per-file declarations. Each
declaration carries a `kind` discriminator (markdown |
diagram_source | diagram_rendered); diagram_rendered entries
additionally carry `derived_from` pointing at the diagram_source
