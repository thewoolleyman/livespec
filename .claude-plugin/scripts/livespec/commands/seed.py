...
# Skill-owned README content — frozen verbatim per PROPOSAL.md
# §"Specification model" lines 999-1024. Per PROPOSAL.md lines
# 992-994, these READMEs are skill-owned: hard-coded inside the
# skill (this Python package), written by `seed` only, NOT
# regenerated on every `revise`. Per PROPOSAL.md lines 1065-1068,
# the content is template-agnostic (same for `livespec` and
# `minimal` templates; only the `<spec-root>/` base differs).

_PROPOSED_CHANGES_README = (
    "# Proposed Changes\n"
    "\n"
...
def _write_skill_owned_readmes(*, spec_root_dir: Path) -> None:
    """Write `<spec-root>/proposed_changes/README.md` + `<spec-root>/history/README.md`.

    Per PROPOSAL.md lines 992-994, both READMEs are skill-owned
    (hard-coded inside this package), written only at seed time,
    and never regenerated by `revise`. Content is the literal
    frozen text from PROPOSAL.md lines 999-1024 (template-
    agnostic per lines 1065-1068).

    The `<spec-root>/proposed_changes/` directory is created here
    explicitly because no other seed step creates it (the
    `<spec-root>/history/v001/proposed_changes/` directory is a
    distinct, nested directory that holds the auto-captured seed
    pair, not the working in-flight proposals).

    The `<spec-root>/history/` directory is implicitly created by
    `mkdir(parents=True)` for `history/v001/`, so this helper
    only needs to write the README there.
    """
    proposed_changes_dir = spec_root_dir / "proposed_changes"
    proposed_changes_dir.mkdir(parents=True, exist_ok=True)
    write_text(
        path=proposed_changes_dir / "README.md",
        content=_PROPOSED_CHANGES_README,
    )
    history_dir = spec_root_dir / "history"
    history_dir.mkdir(parents=True, exist_ok=True)
    write_text(path=history_dir / "README.md", content=_HISTORY_README)
...
def main() -> int:
    ...
    template_value = cast("str", payload["template"])
    write_text(
        path=cwd / ".livespec.jsonc",
        content=json.dumps({"template": template_value}) + "\n",
    )
    _write_skill_owned_readmes(spec_root_dir=cwd / spec_root)
