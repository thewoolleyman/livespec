...
Common diagram types (sequence, class, state, ER, flowchart,
light C4) are authored as fenced `mermaid` blocks directly
inside the markdown spec files — no manifest entry, no render
command, no history special-casing. Replace the starter
flowchart below with your project's actual component
relationships:
