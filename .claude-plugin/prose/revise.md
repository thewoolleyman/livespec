...
Harness-neutral driving prose for the `revise` operation, per
...
operation; the revise CLI named in `.livespec.jsonc` (core reference
implementation: `bin/revise.py`) is the contract half. Drivers bind
...
JSON payload is assembled and the CLI is invoked. Pre-step
...
- `--revise-json <path>` (required). Absolute or
  ...
  the CLI resolves it per the four-step precedence below.
- `--author <id>` (optional). Uniform `--author` flag with
  ...
  populated independently by the CLI via
  ...
  fallback (4) is reached for `author_llm`, this prose
  MUST surface a warning to the user ("Running with
...
- `--spec-target <path>` (optional). Defaults to the main
  ...
  there. The CLI validates the target structure
...
- `--skip-stale-branch-check` (optional). Skips the Step 3.5
  ...
  used, this prose MUST surface a warning narration
...
- `--post-step-doctor` (default-off; this prose MUST pass it
  ...
  snapshot lands the CLI invokes the doctor-static CLI as
  a subprocess; any `status: "fail"` finding lifts the CLI
  ...
  test-scoping; the production invocation includes it.
...
phases but are NEVER passed to the revise CLI
(per):
...
invocation, this prose forwards the choice to the
relevant post-step phase and never to the revise CLI.
Both-flags-in-the-same-pair is a usage error; surface and
abort the affected post-step phase. See the doctor prose
(`prose/doctor.md`) for the full LLM-driven-phase contract.
...
2. **Read the revise prompt.** Read
   ...
   dispatch (run the resolution CLI, then read the prompt
   file) and works uniformly for built-in and custom
   templates.

3. **Enumerate pending proposed-change files.** List
   `<spec-target>/proposed_changes/*.md`. Skip the
   operation-owned `README.md` and any file ending in
   `-revision.md` (those are revision pairings from prior
   cycles, not in-flight proposals; the CLI's pre-step
   ...
   STOP — the CLI's pre-step will exit 3 with the
   ...
   **Narrate stale-pending-proposal count + oldest
   ...
   `--spec-target` first. Per SPECIFICATION/contracts.md
   §"`revise` skill-prose responsibilities", this
   narration MUST NOT gate the CLI, MUST NOT add
   any pre-step or post-step doctor check, and MUST
   NOT block downstream CLI invocations.
...
3.5. **Stale-branch precondition check.** Run the
   ...
   **Narration rule (silent override forbidden).** Whenever
   the precondition is skipped, this prose MUST
   ...
   this prose + the flag pair + the config key
...
4. **Capture optional steering intent.** Ask the user
   ...
   *new intent* rather than decision-steering (best-effort
   ...
   to run the propose-change operation first; on ambiguity,
...
5. **Per-proposal decision dialogue.** For each
   ...
   - **Prompt the user for confirmation.** Present each
     ...
     confirm / override. This is the only operation
   ...
   - The user MAY at any per-proposal turn toggle
     ...
     (whole-revise scope) and the dialogue auto-accepts
   ...
   When assessing each `## Proposal` section, check it against
   the three placement splits below. A proposal that violates a
   split is malformed; prefer `modify` (re-shaping the content
   to the right form/tree) over `accept`, or `reject` and direct
   the user to re-file the content where it belongs.

   (i) **Behavior ⇒ Gherkin scenario (+ clause).** Load-bearing
   behavior — an observable input→output, a state transition, an
   error path, or an invariant the implementation must honor —
   MUST be stated as a BCP14 `MUST`/`SHOULD` clause AND have a
   `## Scenario` in `scenarios.md` (Given/When/Then). Prose may
   only AUGMENT the clause+scenario, never CARRY the behavior
   alone. A behavior-introducing proposal's `resulting_files[]`
   MUST include the new/updated scenario (and, where the project
   links clauses to scenarios, the `tests/heading-coverage.json`
   link), co-edited atomically per `spec.md` §"Self-application".
   Behavioral prose with no scenario is malformed.

   (ii) **Functional vs non-functional placement.** Apply the
   §Boundary litmus (`non-functional-requirements.md`
   §"Boundary"): "does a project merely GOVERNED by livespec
   inherit this, or is it livespec's own infrastructure?"
   User-observable intent/behavior → `spec.md`; user-observable
   wire contracts → `contracts.md`; constraints whose violation
   an end user could observe → `constraints.md`; contributor-only
   concerns → `non-functional-requirements.md`. Resolve the
   constraints.md ↔ non-functional-requirements.md call by
   user-observability, not topic.

   (iii) **Cross-repo placement (multi-repo projects).**
   Realization mechanism — how a specific orchestrator / impl
   plugin works (its Dispatcher, ledger, sandbox flow) — belongs
   in THAT repo's `SPECIFICATION/`, never core. Repo-agnostic
   guidance any consumer inherits belongs in core. A
   core-targeted proposal describing a specific tool's internals
   MUST be redirected to that tool's repo.
...
8. **Resolve effective flags.** Determine whether the user
   ...
   two LLM-layer flag pairs. Compose the CLI argv with
...
9. **Invoke the revise CLI.** Run the revise CLI named in
   ...
   freshly-cut `vNNN/` snapshot. Capture the exit code. The
   CLI validates the payload internally, performs
...
11. **Retry-on-exit-4.** On CLI exit code `4` (schema
    ...
    the CLI. The exact retry count is intentionally
...
13. **Post-step `capture-impl-gaps` invocation.** AFTER the
    CLI exit 0 (Step 9), AFTER the CLI's post-step
    doctor static phase has completed inside that CLI
    ...
    `## Post-CLI`, this prose MUST invoke the
    active impl plugin's `capture-impl-gaps` front-end
    against this revise pass's `--spec-target` per
    SPECIFICATION/spec.md §"Sub-command lifecycle".
    ...
    plugin MUST advertise a `capture-impl-gaps` front-end
    (per SPECIFICATION/contracts.md §"Interactive dialogue
    ownership (orchestrator-side)"). If the plugin does NOT
    advertise it, silently skip this step — not every impl
    ...
    (e) **Invoke the front-end.** Invoke the active impl
    plugin's `capture-impl-gaps` front-end with
    `--since-version <prior-vN> --spec-target <spec-target>
    --project-root <project-root>`. The invoked front-end
    owns its own per-gap consent dialogue; this prose
    MUST NOT gate or wrap that dialogue and MUST NOT
    re-prompt the user once `capture-impl-gaps` is invoked.
...
chain in the doctor prose §"Skip control"):
...
silent-skip narration in the doctor prose §"Skip
control"): this prose MUST surface a warning via LLM
...
in-flight proposals; the operation-owned
`proposed_changes/README.md` persists).

The LLM-driven post-step phase then runs per the doctor
prose (`prose/doctor.md`), honoring the two doctor-LLM flag
pairs from Inputs. After the LLM-driven post-step phase
completes, Step 13 invokes the active impl plugin's
`capture-impl-gaps` front-end (when advertised), honoring
the third LLM-layer flag pair and the
...
- Exit `0` → success. The new `vN` exists at the path
  described in `## Post-CLI`. Proceed to the
  post-step LLM-driven phase per the doctor prose
  (`prose/doctor.md`). This also covers intentional
  `--help` output (user asked for help, not an error).
