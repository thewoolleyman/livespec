...
Harness-neutral driving prose for the `critique` operation, per
...
operation; the critique CLI named in `.livespec.jsonc` (core reference
implementation: `bin/critique.py`) is the contract half. Drivers bind
...
runs after the CLI exits, followed by the LLM-driven
post-step phase.

## When to run

- The user invokes the critique operation, says "critique the
...
- If the user asks to *process* findings rather than surface
  them, route to the revise operation instead — critique only
  emits findings as a proposed-change file; it does not
  accept or reject them.

## Inputs

The critique CLI accepts the following flags in v1:
...
- `--findings-json <path>` (required). Absolute or
  ...
  The schema shape is identical to propose-change's input —
  same `findings[]` array with `name`, `target_spec_files`,
  `summary`, `motivation`, and `proposed_changes` per finding.
  The semantic difference is content: critique findings
  describe ambiguities, contradictions, or missing rules in
  the current spec, not new behavior.
- `--author <id>` (optional). Uniform `--author` flag with
  ...
  this prose MUST surface a warning to the user
...
LLM-driven phase but are NEVER passed to the critique CLI
(per):
...
this prose forwards the choice to the post-step LLM-driven
phase and never to the critique CLI. Both-flags-in-the-
same-pair is a usage error; surface and abort the LLM-driven
phase. See the doctor prose (`prose/doctor.md`) for the full
...
2. **Read the critique prompt.** Read
   ...
   the two-step template-prompt dispatch (run the resolution
...
7. **Invoke the critique CLI.** Run the critique CLI named in
   ...
   / v017 Q1 reserve-suffix canonicalization composes the two
   and preserves the `-critique` suffix at the 64-char
   filename cap. The resulting file is
   `<spec-target>/proposed_changes/<canonical-author>-critique.md`
...
9. **Retry-on-exit-4.** On CLI exit code `4` (schema
...
10. **Narrate fallback-author warning.** If the resolved
    ...
    to the user. The CLI does not gate on this; the
...
- Delegated to `propose_change.run()` with topic hint
  ...
  since critique's outer CLI ROP chain already covers
  the whole operation.
- Written
  `<spec-target>/proposed_changes/<canonical-author>-critique.md`
...
the standard CLI-side lifecycle. Then the LLM-driven
post-step phase runs per the doctor prose
(`prose/doctor.md`), honoring the two LLM-layer flag pairs
from Inputs. Critique does NOT run revise; the user reviews
...
- Exit `0` → success. The proposed-change file exists at
  the path described in `## Post-CLI`. Proceed to the
