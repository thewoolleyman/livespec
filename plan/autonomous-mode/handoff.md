...
## SESSION UPDATE — 2026-07-19 (cont. 24): B3 IMPL + B5 (spec propose → revise v028 → impl) ALL DONE + MERGED to console master (tip `1bfdb41d`) + TWO-REPO LIVE-VERIFIED — **the full B1–B5 cockpit-UX behavior set is now landed**. Also root-caused + fixed the tmux E2E harness's non-deterministic pane width (dedicated per-test socket). NEXT = B6/B7 docs + B8 release capstone (other sessions), backfill real-TUI E2E, Stage-2 LAST.

### RESUME — READ FIRST
- **Console master tip `1bfdb41d`.** **B1 / B2 / B3 / B4 / B5 are ALL DONE (spec + impl, merged)** — the full B1–B5 cockpit-UX behavior set is landed. B5 impl merged `1bfdb41d` (PR #289): the 3 baked-in doc sentences removed, Scenario 21 tmux E2E green, 100% coverage, two-repo live-verified. **Remaining cockpit program = B6/B7 docs + B8 release capstone (OTHER sessions' worktrees — coordinate, don't duplicate), backfill real-TUI E2E for existing scenarios, then Stage-2 LAST (maintainer-gated).**
...
- **NEXT = B6/B7 docs + B8 release capstone** (held by OTHER sessions' worktrees `docs-console-tui-usage` / `console-release-pipeline` — coordinate, do NOT duplicate), plus backfill real-TUI tmux E2E for existing scenarios (5/9/11/13), then **Stage-2 LAST** (the maintainer-gated MVP acceptance — drive real fleet items end-to-end via the live TUI). Detail in the cont.23 / cont.22 sections.
...
### B5 impl — DONE (merged `1bfdb41d`, PR #289)
Removed the 3 baked-in doc sentences from the summary-view projections in `crates/console-application/src/lib.rs`: "Spec lifecycle status is projected from LiveSpec adapter observations." / "Revise-required events stay visible in the Spec view until resolved." / "The event log is the canonical source for projections." (the complete set — the only doc prose in any pane body). `summary_detail_lines` (console-tui) now renders a title-only line for a row with no operational detail. The 3 KEPT help surfaces (Status-line hints, modal Help overlay, Settings per-row inline help) untouched. Scenario 21 tmux E2E `tmux_tui_e2e_panes_operational_content_only_scenario_21` sweeps every view for the removed fragments (`is projected from` / `stay visible` / `canonical source`); `tests/heading-coverage.json` Scenario 21 `test` link filled (gap-xiziukv6 stays linked). **100% coverage; `just check-e2e-tmux` 7/7 green; all 14 CI checks green; two-repo live-verify** (console `attention:15` / orch `attention:47`).
