...
| id | tenant | what | next |
|---|---|---|---|
| `livespec-127o` | core | README rewrite (spec-driven): add a README-contract to `SPECIFICATION/` (purpose, audience = zero-knowledge newcomer, required structure, reference-not-duplicate), then author the README to conform. Supersedes the incrementally-grown README (PR #723). | ⏸ **DEFERRED (Session 15, maintainer)** — removed from this thread's scope; proposal withdrawn (recoverable via git history); `127o` + slices stay `backlog`. See handoff §"Session 15". |
| `livespec-m0xu` | core | rename `templates/impl-plugin/` → `templates/orchestrator-plugin/` (template PATH only; the repo-class/role term deliberately kept). | ✅ **DONE (Session 11)** — groomed PATH-only → Slice 1 spec (history/v155, PRs #780/#781) + Slice 2 code (PR #782); m0xu + `livespec-pzrvzm` CLOSED; sibling copier update verified (exit 0). Surfaced+filed+fixed the factory App `workflows`-permission gap (`livespec-2ef0.1`, closed). See handoff §"Session 11". |
