...
| 7 — restore and verify up | **DONE** — unattended proving reboots 2026-09-04 17:14Z and 2026-09-06 01:59Z; CI on all ten repos; churn-slot cap restored to 64 for the soak (livespec-dev-tooling PR #1751), then MEASURED back to 32 the same morning when the soak's second hour saturated the CPU at 35–46 running jobs (PR #1777); every scale set's `maxRunners` bounded to `max(2 × nominalQuota, 6)` after the first backlog at 32 showed the control plane tracked backlog depth (PR #1805); the cap and its derivation are owned by epic `livespec-ifwnqj` (ratified: livespec v220 `ci-host-backlog-object-bound`, livespec-dev-tooling v057 `scale-set-ceiling-bounded-to-fair-share`) | child `livespec-e2vcqf` (closed 2026-09-06; close rationale corrected 09:50Z), spec history v218 and v220, epic `livespec-ifwnqj` comments of 2026-09-06 |

Archived 2026-09-06 05:13Z after both plan archive gates passed on a first-hour soak reading (completeness review `plan-completeness-review-poweredge-raid-array-maintenance-20260906T0511Z`); UN-ARCHIVED the same day by maintainer decision when the soak's second hour reversed that reading, and RE-ARCHIVED on the corrected record above after a fresh independent completeness review (`plan-completeness-review-poweredge-raid-array-maintenance-20260906T0949Z` on epic `livespec-g52yrb`; the epic's 2026-09-06T09:44:56Z corrective handoff is the read-first entry).
