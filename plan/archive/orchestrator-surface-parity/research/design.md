...
- **P4** [git-jsonl spec, propose-change → revise]: supersede OR3 v017 reduced-scope (git-jsonl is now a full peer). Reconcile the current git-jsonl skill-count DRIFT in the same pass (reassigned here 2026-07-08 from the `needs-attention-hardening` carry-over list, epic livespec-yes5 — a surface/count matter, not a needs-attention behavior matter): `README.md:20` cites a contracts section "the 10-skill surface" while `README.md:74` says "Seven skills" and `SPECIFICATION/contracts.md:19` is "## The seven-skill surface" — the README contradicts itself and cites a non-matching section title. The P4 supersede rewrites the git-jsonl skill-set spec to the full-peer 12, so it reconciles these counts as a side effect; P2's mechanical skill-set-consistency check then prevents the drift from recurring.
