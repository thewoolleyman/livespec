...
## Supervisor restart state — 2026-08-13T20:37Z

This supervisor has published its binder as the append-only, attributed ledger
handoff entry `019ffcd7-fd50-7a56-af37-72cd122de6b3` on plan epic
`livespec-s43svm`. Read that entry together with `.ai/supervisor-protocol.md`.
Do not create or update `plan/fleet-ci-runner-pool/supervisor-handoff.md`; this
file is the legacy session-respawn handoff and the ledger entry is the durable
supervisor binder.

The worker session `fleet-ci-runner-pool` is actively monitoring core PR
[#2276](https://github.com/thewoolleyman/livespec/pull/2276), which bumps
`livespec-runtime` to v0.19.0 and synchronizes the core-vendored API manifest.
At the measurement above, it is OPEN, CI is in progress, and GitHub reports
`mergeStateStatus: DIRTY` while checks run. The worker owns that PR through its
terminal forge state. Do not interrupt it with a second task.

After that obligation reaches a terminal forge state, the worker owns one
follow-up only: compare `.ai/supervisor-protocol.md` with the current
`livespec-overseer` `supervise-plan` generator requirements, especially the
required `## Supervisor scratch discipline` role section. If a change is
needed, the worker must use its own dedicated worktree -> reviewed PR ->
rebase-merge -> cleanup path; if not, it must record the verified no-change
finding on epic `livespec-s43svm`. Never edit tracked files in the primary
checkout.

Supervisor preconditions were measured before the ledger entry: worker
`fleet-ci-runner-pool` and supervisor `fleet-ci-runner-pool-supervisor` are
distinct live Codex panes; the worker cwd resolves inside `/data/projects/livespec`;
and the plan exists at `plan/fleet-ci-runner-pool/`. The worker's Codex process
is launched by `bun`, so live-agent proof must inspect full process arguments,
not only the `comm` field.
