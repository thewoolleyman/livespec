...
>
> **Status of implementation — DONE 2026-07-19 (maintainer chose "-n workers only"; supervisor --slots left at 6).** The lane-aware `-n` shipped to all 4 xdist Python repos: **livespec #1372**, **livespec-dev-tooling #456**, **livespec-orchestrator-git-jsonl #330** (all MERGED), **livespec-runtime #263** (auto-merge on green). Design: `ci.yml` sets `LIVESPEC_CI_LANE=${{ contains(vars.CI_RUNNER_LABELS,'ubuntu') && 'hosted' || 'local' }}` (RUNNER_ENVIRONMENT does NOT reach container jobs), and the justfile's `test_nprocs := if LIVESPEC_CI_LANE=='hosted' {"auto"} else {LIVESPEC_TEST_PARALLELISM or 25%-of-nproc}` drives both coverage recipes. **PROVEN on livespec's master run: `check-coverage` logged `created: 4/4 workers` (`-n auto` → the 4-core hosted runner).** Point 3 (concurrency) DONE — the 6 dangling PRs were CLOSED (nothing was ever merged; master already all-completes). Point 1 (supervisor) unchanged. **Console (Rust): NO change** — its static `jobs=4`/`test-threads=4` already = all cores on the 4-core hosted runner (auto) and ≈22% on the 18-core VPS; dynamic lane-awareness is a follow-up for when a dedicated Rust host is provisioned (nothing to validate against now). **Per-host knob for "multiple other hosts":** a dedicated self-hosted box reports `self-hosted` → 25% by default, so set `LIVESPEC_TEST_PARALLELISM` (e.g. `auto`/high) in THAT host's runner env — wire it into runner provisioning. NB: the cont.5 fan-out on 2026-07-19 was interrupted by an account weekly-usage-limit that killed the 3 helper agents mid-run; the main session finished all 3 repos by hand and put them on auto-merge.
>
