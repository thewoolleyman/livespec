...
> **KNOWN FOLLOW-UPS (non-blocking):** (a) **console cold builds** — its earlier cutover DELETED `actions/cache`, so on hosted it builds cold; tolerable (each matrix job on its OWN hosted runner → NO cross-job contention, unlike the 883s self-hosted-shared figure), but wall-clock is slower than the ~430s warm baseline. Restoring a **container-aware** cargo/target cache (correct `CARGO_HOME`/HOME under the baked image — NOT a copy of the pre-cutover ubuntu-latest cache config, whose paths differ) is the recommended optimization. (b) The orchestrator baked-image cutover + gh-hermeticity fix (prior item 3) remain separately deferred + maintainer-gated. (c) `livespec-dev-tooling-xb7` (ci.yml `container: image:` tag not auto-bumped) still open.
>
