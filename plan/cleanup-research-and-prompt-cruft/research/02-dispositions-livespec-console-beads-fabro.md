...
| Judgment call | CONFIRMED verdict |
|---|---|
| `plan/impl-dispatch/` as the D2 conversion of `prompts/impl-obligations-handoff.md` | **YES** — archive the prompts/ file to `archive/prompts/`, create NO new thread; the parked thread + ledger own every open obligation. |
| Stale "BLOCKED on github-app-auth" park note in `plan/impl-dispatch/handoff.md` | **Out of this epic's scope** — refresh at that thread's own next resume (recorded here so it isn't lost). |
