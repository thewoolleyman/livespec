...
> **Amendment 2026-09-04 (authoritative for the window).** Two corrections to
> the text below, both recorded on the plan epic `livespec-g52yrb` (handoff
> 2026-09-03T23:19Z and the scope correction 2026-09-03T23:24Z):
>
...
>    off → seat the 4 SATA drives + the StarTech PEX8M2E2 (low-profile
>    bracket; eyeball Riser 2) carrying the 2× WD SN8100 4 TB → boot the
...
>    `SKIP_BOOTLOADER`) → one LVM VG per NVMe drive, PVs addressed by
>    `/dev/disk/by-id` only → boot from the array → retier via the fstab
>    bind mounts (containerd churn → NVMe A, runner work volumes → NVMe B,
>    warm cache STAYS on the array) → `nvme smart-log` under the first
>    sustained CI load. The stand-in LVs are left empty in place as spare
>    capacity. Decision 6 in the table is therefore void: NVMe is never a
>    boot device, even temporarily.
...
**Superseded by the amendment above:** the hardware install still happens in
the one power-off window, but no NVMe clone is made. Kept for the reasoning
trail.
