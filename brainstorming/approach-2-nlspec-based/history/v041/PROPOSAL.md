...
2. **Green.** Write the *minimum* implementation that turns
   ...
   test and observe it pass. Per v039 D4, before staging the
   Green amend run `just check-coverage-incremental` (per
   v039 D3) on the impacted impl + test pair to verify
   per-file 100% line+branch coverage proactively; the
   incremental tool finishes in seconds where the full
   `check-coverage` aggregate runs the entire suite under
   parallel coverage and takes minutes. Once the impacted
   pair is green and at 100% coverage, the Red-Green pair
   commits together as the feature/bugfix unit of value;
   pre-commit's `check-coverage` re-validates against the
   full tree as the load-bearing gate.
...
### Red-time branch enumeration + proactive coverage (v039 D4)

The v034 D2-D3 amend pattern locks the test file's SHA-256
at the Red commit; the Green amend can only stage
implementation, not modify the test. Any impl branch the
test does not exercise — including defensive branches
(`try`/`except` arms, `isinstance` narrowings, `sys.path`
guards, `if __name__ == "__main__":` entry points, fallback
clauses, exhaustiveness sentinels) — surfaces as a per-file
100% coverage gate failure at Green-amend time. Recovering
requires backing up to the Red commit, extending the test
to drive the missing branches, and re-running the Red→Green
amend pair. Each round-trip burns one full pre-commit
aggregate (~5+ minutes pre-v039 D2; reduced post-v039 D2
but still non-trivial), so coverage gaps are a Red-time
discipline failure, not a Green-time discovery.

**Red-time enumeration discipline.** Before committing the
Red moment, the executor MUST enumerate every branch the
planned implementation will introduce and write tests for
each. The enumeration covers, at minimum:

- Each public-behavior branch (the obvious ones the test
  was written for).
- Each defensive branch the impl will need to satisfy
  static-analysis or runtime contract: `try`/`except`
  clauses (one test per `except` arm; the universal
  `case _: assert_never(...)` exhaustiveness sentinel is
  exempt because it is structurally unreachable by
  contract), `isinstance` narrowings (one test per arm),
  optional-arg fallbacks (one test per default-vs-supplied
  branch), `sys.path` guards (one test per branch when the
  module is wrapper-shaped), `if __name__ == "__main__":`
  entry points (one test invoking the module as a script,
  asserting the entry point routes correctly).

**Proactive coverage discipline.** After authoring the
Green impl draft and before staging, the executor MUST run
`just check-coverage-incremental` (v039 D3) on the
impacted impl + test pair to surface coverage gaps in
seconds, NOT discover them via the Green-amend
`check-coverage` gate after a 5+-minute aggregate. The
incremental tool exists specifically to make proactive
coverage cheap enough that skipping it is never the
ergonomic choice. The pre-commit gate remains the
load-bearing safety net (`check-coverage` over the full
tree); the incremental tool is the fast-feedback layer that
makes the gate's failure-mode rare.

**Mechanical reinforcement.** Q1 (proactive coverage runs)
and Q2 (Red-time branch enumeration) are not directly
mechanically enforceable — there is no hook that can verify
"the executor ran the incremental tool" or "the executor
thought through every defensive branch." Both are made
mechanically cheap to follow by v039 D3's incremental tool;
both are mechanically caught post-hoc by the existing
`check-coverage` gate at Green-amend time. The discipline
is the gate-shifting move: catch the gap at the
incremental-tool layer (seconds), not the
full-aggregate layer (minutes).
...
The instant a flake is observed (in CI, in pre-commit aggregate
runs, during local development, or via any other channel), work
halts on whatever was in progress and the flake is conclusively
resolved.

**Conclusive resolution** means one of:

1. **Fix deterministically.** Identify the root cause (timing
   assumption, shared state, ordering dependency, nondeterministic
   input space, hypothesis deadline, fixture pollution, etc.) and
   apply a deterministic remedy: pin the seed, eliminate the slow
   path, use `monkeypatch.chdir(tmp_path)` for cwd-fallback
   branches, set `@settings(deadline=None)` only when timing has
   no semantic relevance to the test, isolate fixtures, refactor
   away from the shared state — whatever the cause demands. The
   fix MUST be verified by repeated runs (typically ≥5 consecutive
   `just check-coverage` invocations).
2. **Delete.** If the test cannot be made deterministic (e.g., it
   asserts behavior that is itself nondeterministic, or the cost
   of pinning is greater than the test's value), delete it. A
   deleted flaky test is strictly preferable to a kept flaky
   test: a kept flake erodes the suite's signal value over time
   by training contributors to retry-and-ignore failed builds.

**Forbidden alternatives:**

- Marking the test `@pytest.mark.flaky` (or any retry plugin) to
  auto-retry on failure. Retry-on-failure tolerates the flake;
  this rule does not.
- Recording the flake as a "non-blocking follow-up note" in
  `bootstrap/STATUS.md` or any other deferred-items log. Flakes
  are ALWAYS blocking; deferral is not a valid disposition.
- Filing the flake as a low-severity open-issues entry
  (`non-blocking-pre-phase-6` / `non-blocking-post-phase-6`).
  The open-issues severity tiers do not apply to flakes — flakes
  are auto-blocking like PROPOSAL.md drift.
- "It's flaky in CI but passes locally" or "it's only flaky
  ...
  the flake to investigate is part of the fix, not a gate on
  whether to fix.

**Mechanical enforcement.** The existing pre-commit and
pre-push aggregate (`just check-coverage` running pytest under
`-n auto` per v039 D2) IS the flake-detection layer — any
observed pytest failure under that aggregate (whether the
failure reproduces deterministically or appears flaky) blocks
the commit / push. There is no separate "flake check"; the
rule is: the aggregate MUST be green every time.

**Originating instance.** v040 codifies this rule after a single
observed flake of `tests/livespec/validate/test_proposal_findings.py::test_validate_proposal_findings_round_trips_name_text`
under v039 D2's new `-n auto` xdist conditions (one occurrence
during v039 D3 verification, passed on retry). The flake was
deterministically resolved at commit `aaaaa82` by lifting the
schema load out of the test body into a module-level constant,
eliminating per-example file I/O and the associated 200ms
hypothesis-deadline pressure under xdist worker contention. The
v040 rule generalizes the precedent.
...
**Activation.** The hard-constraint pre-commit gate runs
`just check-pre-commit` — a Red-mode-aware aggregate (per
v036 D1, broadened in v037 D1) that detects the v034 D3
Red commit shape (exactly one test file added or modified
under `tests/` AND zero implementation files added or
modified under `.claude-plugin/scripts/livespec/**`,
`.claude-plugin/scripts/bin/**`, or
`dev-tooling/checks/**`) and skips `check-coverage` in
that case (deferring its verification to the commit-msg
replay hook), running the full `just check` aggregate
(including 100% line+branch **per-file** coverage per
v033 D2 and all canonical-target checks) in every other
case (Green amend, `test:`/`chore:`/etc., non-Red
`feat:`/`fix:`). Pre-push and CI invoke `just check`
directly (no Red-mode classifier; full suite always);
manual invocation of `just check` from a developer shell
is also unconditional. Per v039 D1, `check-tests` is
dropped from the canonical aggregate (the test suite is
exercised end-to-end as a side effect of `check-coverage`,
which now also runs the entire suite under
parallel-instrumented pytest); the Red-mode skip semantic
correspondingly collapses to skipping only `check-coverage`.

The pre-commit gate becomes binding at the moment lefthook
is installed into `.git/hooks/`. Per v033 D5a this happens
at the v033-codification-commit boundary (one commit +
the four guardrail-script commits authoring D1-D4 + the
`just bootstrap` promotion + the `lefthook install`
invocation). The earlier framing ("Phase-5-exit lefthook
activation") is closed at v033; moving the activation
forward eliminates the discipline-gap window during which
v032's first redo authored 56 cycles of impl-first-style
work without commit-time mirror-pairing or per-file-
coverage enforcement. Pre-v033-codification-commit
commits are grandfathered (they precede the new
guardrails). From the v033-codification commit onward, no
commit lands without passing the per-commit gate; the
Phase 6 first self-application seed is the first working
commit subject to the discipline at full specification-
driven strength.
...
1. **Red mode (initial commit).** Triggered when the staged
   ...
   **Coexistence with the pre-commit gate (v036 D1, v037 D1, v039 D1).** The
   Red commit's failing test would otherwise be blocked by
   lefthook pre-commit's `check-coverage` invocation. Per v036 D1
   (broadened in v037 D1), lefthook pre-commit invokes
   `just check-pre-commit` (not `just check`); that aggregate
   detects Red-mode shape via `git diff --cached --name-only
   --diff-filter=AM` and skips `check-coverage` when the
   shape matches (one test file added or modified under
   `tests/`, zero impl files added or modified). Per v039 D1
   the Red-mode skip semantic targets `check-coverage` only;
   `check-tests` is no longer in the canonical aggregate (the
   suite is exercised by `check-coverage` itself under v039 D2's
   parallel-instrumented pytest). The commit-msg replay hook
   (this hook) runs pytest on the staged test file, verifying
   the Red moment. Net effect: each Red commit's test is run
   exactly once at commit time (by the replay hook), and
   pre-commit's other canonical-target checks run
   unconditionally.
...
│       ├── branch_protection_alignment.py     (v041 D1: compares ci.yml's matrix.target list against master branch protection's required_status_checks.contexts; required-but-missing-from-ci.yml fails, ci.yml-but-not-required warns; gracefully exits 0 when `gh` is unavailable locally)
│       ├── master_ci_green.py                 (v041 D1: fetches the most recent master CI run via `gh run list` and fails on `failure`/`cancelled`/`timed_out`/`action_required`/`stale`/`startup_failure` conclusions; pending statuses non-blocking; gracefully exits 0 when `gh` is unavailable locally)
...
list (every `just check-*` target plus `check-coverage`
and the e2e + prompts targets when active; per v039 D1
`check-tests` is no longer enumerated separately —
`check-coverage` exercises the suite as a side effect of
its parallel-instrumented pytest invocation). Steps inside each matrix job:
...
Three rules govern the relationship between the CI workflow
at `.github/workflows/ci.yml`, the master-branch protection's
required-checks list, and the master CI conclusion at any
commit:

1. **No permanent-red CI jobs.** No job in
   `.github/workflows/*.yml` MAY be configured to run while
   documented as failing-and-deferred. A job MUST be either
   (a) repaired in the same commit that breaks it, or (b)
   removed from the workflow until repaired. "Deferred until
   Phase X" is not a permitted CI state — the comment in
   `justfile`'s aggregate stating a target is deferred and
   the matrix line that runs the same target as red MUST
   NOT coexist. When a deferred job is later repaired, it
   re-joins the matrix in the same commit that fixes its
   underlying issue.

2. **CI matrix and branch-protection required-checks list
   MUST agree.** The set of required checks on master
   branch protection MUST equal the set of jobs in
   `.github/workflows/ci.yml`'s `matrix.target`, modulo
   workflows explicitly marked optional (e.g., the
   `e2e-real.yml` real-Claude harness, which is a
   manually-triggered release-gate workflow). Drift between
   the two — a required check that is not in the matrix,
   or a matrix entry that is not required — produces the
   v039 D1 / `check-tests` failure mode where GitHub blocks
   merges on a missing-not-failing required check.

3. **Master CI is green at every commit.** No commit MAY
   land on a phase-N branch when master CI is red on its
   most recent run. "Master red" includes failed required
   checks AND failed non-required-but-running jobs — rule
   1 already removes deferred jobs, so any remaining job is
   load-bearing.

**Mechanical enforcement (Guard Layer 1).** Two
`dev-tooling/checks/` scripts implement rules 2 and 3:

- `branch_protection_alignment.py` reads ci.yml's
  `matrix.target` list, fetches master branch protection's
  `required_status_checks.contexts` via `gh api`, and fails
  on any required-but-missing-from-ci.yml entry. The
  ci.yml-but-not-required entries emit informational
  warnings (some workflows are intentionally optional).
- `master_ci_green.py` fetches the most recent master CI
  run via `gh run list --branch master --limit 1 --workflow
  CI` and fails on conclusion `failure`, `cancelled`,
  `timed_out`, `action_required`, `stale`, or
  `startup_failure`. Pending statuses (`queued`,
  `in_progress`, `waiting`, `pending`, `requested`) are
  treated as non-blocking — CI may not have caught up to a
  fresh master push yet.

Both checks gracefully exit 0 with a structured warning
when `gh` is unavailable or unauthenticated locally so
per-commit pre-commit runs are not blocked; CI with
`GH_TOKEN` exercises the full enforcement path. Both are
wired into the `just check` aggregate as
`check-branch-protection-alignment` and
`check-master-ci-green`.

Rule 1's enforcement is partially mechanical (Guard Layer 1
detects deferred-and-failing jobs in CI via
`master_ci_green.py`) and partially editorial (the
`justfile` aggregate's deferral comments + the ci.yml
matrix MUST agree on whether a job runs).

**Skill-side enforcement (Guard Layers 2-3).** The
throwaway bootstrap skill at
`bootstrap/.claude-plugin/skills/bootstrap/SKILL.md`
encodes session-start sanity checks (step 1 sub-step 6:
branch ↔ STATUS phase alignment + master CI green +
active-PR mergeability) and a 4-step phase-advance
transaction (§5d: verify prior-phase PR merged + verify
master CI green + cut fresh `phase-(N+1)-*` branch from
master HEAD + land STATUS commit on new branch). The
skill-side enforcement disappears at Phase 11 cleanup; the
production livespec plugin doesn't have a phase-advance
lifecycle, so the equivalents aren't needed there.

**Future SPECIFICATION migration.** Phase 8 item 7
(`static-check-semantics`) will eventually carry a
propose-change cycle migrating the v041 D1 contracts into
`SPECIFICATION/constraints.md`, at which point this
PROPOSAL section becomes reference-only per the
post-Phase-6 dogfooding contract. Until then, this section
is the authoritative source for the contracts.
