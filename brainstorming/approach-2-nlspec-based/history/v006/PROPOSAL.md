...
├── commands/                          # one file per sub-command (skill-layer prose)
│   ├── help.md
│   ├── seed.md
│   ├── propose-change.md
│   ├── critique.md
...
├── scripts/
│   ├── bin/                           # Python shebang-wrapper executables (≤5 lines each)
│   │   ├── seed.py
│   │   ├── propose_change.py
│   │   ├── critique.py
│   │   ├── revise.py
...
│       │   └── static/                # one module per static check
...
- **Sub-command dispatch.** Each sub-command has its own executable
  at `scripts/bin/<cmd>.py`. `SKILL.md` names each wrapper by path;
  the LLM invokes it directly via the bash tool. No top-level
  dispatcher script. The wrapper is a thin shebang pass-through to
  `livespec.commands.<cmd>.main()`.
...
- **CLAUDE.md coverage:** every directory under `scripts/` MUST
...
- **`python3` >= 3.10** is the sole runtime dependency imposed on
  end-user machines. The skill MUST check `sys.version_info` at
  startup and MUST exit `127` with an actionable install instruction
  if older (e.g., "Install Python 3.10+ via `brew install python@3.10`
...
- **No other runtime dependencies.** No `jq`. No PyPI install step.
  ...
  `.claude-plugin/skills/livespec/scripts/_vendor/<lib>/`.
- **No bash** is invoked anywhere in the shipped bundle.
- **Vendored pure-Python libraries** bundled with the skill:
...
once produces a ready-to-run environment. Developer tooling is NOT
installed into user projects; it is purely livespec-maintainer-facing.
...
from `scripts/_vendor/fastjsonschema/`.
...
- `--skip-pre-check` on any sub-command acts as a per-invocation
  ...
  config or --skip-pre-check"`). The Python layer MUST NOT print
  the warning to stdout (stdout is reserved for the
...
`.claude-plugin/skills/livespec/scripts/livespec/schemas/`.
Authoring the full schemas and prompt input/output details is an
implementation task filed as a post-seed `propose-change`.
...
- External programmatic callers that already have structured
  ...
  `scripts/livespec/schemas/critique_findings.schema.json` and
  maps findings directly to `## Proposal` sections without invoking
  the template prompt.
...
- Generates a critique of the current spec using the active
  ...
  `.claude-plugin/skills/livespec/scripts/livespec/schemas/critique_findings.schema.json`).
- The skill validates the JSON against the schema using
  `fastjsonschema`; re-invokes the prompt on malformed output
  (up to 3 retries) with error context; aborts on repeated failure.
...
- **Static phase** is implemented in Python. The orchestrator is
  at `.claude-plugin/skills/livespec/scripts/livespec/doctor/
  run_static.py` and invoked via the shebang wrapper at
  `scripts/bin/doctor_static.py`. Each static check is a Python
  module at `scripts/livespec/doctor/static/<module>.py`.
- **LLM-driven phase** is skill behavior (prose in
  `commands/doctor.md`), not Python. It has no exit codes.
...
- Runs entirely via ROP (`flow` / `bind` / `.map` / `.lash`),
  short-circuiting on first stage failure, producing a `fail`
  Finding for known-domain failures via `.lash`.

The orchestrator `scripts/livespec/doctor/run_static.py`:

- Discovers check modules by glob over
  `scripts/livespec/doctor/static/` (lexicographic order).
...
The slug↔module-name mapping converts hyphens to underscores
(slug `out-of-band-edits` ↔ module `out_of_band_edits.py`).
User-facing `check_id` values in JSON findings keep hyphens:
...
module under `static/`; a deprecated check is deleted and its slug
...
`scripts/livespec/doctor/static/<module>.py`. The slug is the
hyphenated form of the module filename.
...
- **`out-of-band-edits`** — The diff between committed spec state
  ...
  * If the project is not a git repo: skip this check with a
    ...
    to compare).
  * Otherwise: the check auto-backfills a missing version. It
    creates
    ...
    `livespec-doctor`. Check exits `3` with a finding
...
`commands/doctor.md`, not a script. It has no exit codes. Its
findings are split into two categories; both run by default.
...
- Coverage is measured by **`coverage.py`** via `pytest-cov`.
  ...
  under `.claude-plugin/skills/livespec/scripts/livespec/**` and
  `<repo-root>/dev-tooling/**`. Vendored third-party libraries
  under `scripts/_vendor/` are excluded from the coverage
  measurement (audited for drift via `check-vendor-audit`
  instead). See the companion
...
│   └── test_wrappers.py                      (meta-test: all wrappers match the shebang-wrapper contract)
├── livespec/                                 (mirrors .claude-plugin/skills/livespec/scripts/livespec/)
│   ├── commands/
│   │   ├── test_seed.py
...
- `tests/livespec/` mirrors `.claude-plugin/skills/livespec/scripts/livespec/`
...
├── .vendor.toml                               (records upstream version for each vendored library)
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   markdown files, shared files, and the Python package tree under
   `scripts/livespec/` — including `commands/`, `doctor/run_static.py`
   and `doctor/static/<slug>.py` per-check modules, `io/`, `parse/`,
   `validate/`, `schemas/`, `context.py`, `errors.py`, plus the
   `bin/*.py` shebang-wrapper executables and `_vendor/` with
   dry-python/returns + fastjsonschema + structlog — and the
   built-in `specification-templates/livespec/` template) is
...
4. `.livespec.jsonc` schema validation is implemented via
   ...
   `scripts/livespec/schemas/livespec_config.schema.json`.
...
6. The full `doctor` static-check suite (every check listed in
   ...
   single ROP chain (`Fold.collect`), with documented exit-code
...
7. The `doctor` LLM-driven phase's objective and subjective
   ...
   (`commands/doctor.md`) with the skip-mechanism for subjective.
...
10. The test suite covers every sub-command, both doctor phases,
    ...
    `dev-tooling/**`.
11. `python3 >= 3.10` presence check is implemented at import
    time in `scripts/livespec/__init__.py`; the skill exits 127
...
12. Developer tooling is in place under `<repo-root>/`:
    ...
    `lefthook.yml` delegating to just targets,
    `.github/workflows/ci.yml` delegating to just targets,
    committed `.mise.toml` pinning every dev tool, `.vendor.toml`
    recording vendored library upstream versions,
    `pyproject.toml` configuring ruff + pyright + pytest +
    coverage, and `NOTICES.md` listing vendored libraries.
13. Every directory under `.claude-plugin/skills/livespec/scripts/`
    and under `<repo-root>/tests/` and `<repo-root>/dev-tooling/`
    contains a `CLAUDE.md` describing directory-local constraints,
    enforced by the `check-claude-md-coverage` target.
...
15. The skill bundle includes its own `help` text covering every
    sub-command.
...
2. Implement the plugin skeleton: `plugin.json`, `SKILL.md`,
   per-command markdown files, the Python package tree under
   `scripts/livespec/`, vendored libraries under `scripts/_vendor/`,
   `bin/*.py` shebang wrappers, doctor's static-check modules,
   the Python-version presence check, and the minimum subset of the
...
5. Land the v1 Definition of Done items as the spec evolves.
   ...
   `prior-art.md`, `python-skill-script-style-requirements.md`,
...
- Full authoring of `python-skill-script-style-requirements.md`
  into `SPECIFICATION/constraints.md` at seed time.
...
- Authoring of the specific enforcement-check scripts under
  `<repo-root>/dev-tooling/checks/`, the `justfile` recipes that
  invoke them, the committed `.mise.toml`, the `lefthook.yml`
  config, and the `.github/workflows/ci.yml` workflow.
