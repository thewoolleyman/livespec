...
│   ├── doctor-static                  # doctor's static phase (executable; no .sh extension)
│   └── checks/                        # architectural-check scripts invoked by both git pre-commit hooks and CI
...
- `livespec` MAY read git state (read-only `git status` /
  ...
  single documented reader in v1 is doctor check #9 (out-of-band
  edit detection), which uses `git show HEAD:` to compare committed
  spec state to history.
...
at `.claude-plugin/skills/livespec/scripts/doctor-static`. The
LLM-driven phase runs only after the static phase passes.

#### Static-phase output contract

The script writes structured JSON to stdout and returns an exit
code per the "Static-phase exit codes" table below.
Output shape:
...
2. The active template (built-in or at the configured path)
   ...
   `template_format_version` and is supported by livespec. On
   any failure, the static finding MUST name the offending
   field, its value, and the path to `.livespec.jsonc`.
...
9. **Out-of-band edit detection.** The diff between committed
   spec state (`git show HEAD:<path>` for each spec file) and
   the latest `history/vN/` copies is empty. Behavior if
   non-empty:
   - If the project is not a git repo: skip this check with a
     JSON finding marked `status: "skipped"` (no committed
     state to compare).
   - Otherwise: the script auto-backfills a missing version.
     ...
     `history/v(N+1)/` with the current committed spec
     content. It then moves the proposed-change and revision
     into `history/v(N+1)/proposed_changes/`. Author
     identifier: `livespec-doctor`. Script exits 3 with a
     finding instructing the user to commit the new
     `history/v(N+1)/` files and re-run the original command.
     WIP in the working tree is unaffected by this flow —
     only committed state drives detection.
...
- `1`: script-internal failure (bug in `doctor-static`); LLM-
  driven phase MUST NOT run; the invoking sub-command MUST
  abort.
...
- Coverage is measured by `kcov`. Pure libraries MUST achieve
  100% line coverage; overall coverage across all bundled bash
...
│   ├── dispatch.bats
│   └── doctor-static.bats
└── test_*.bats                        (per-specification-file tests, one file per spec file)
```
...
- Tests MUST be split into separate files per specification file:
  `test_spec.bats`, `test_contracts.bats`,
...
classification, architectural-check scripts under
`scripts/checks/`, etc.) are in
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   `dispatch`, `doctor-static`, and `checks/` — schemas, and the
...
6. The full `doctor` static-check suite (12 checks above) is
   implemented with documented exit-code semantics (0/1/3) and
...
10. The test suite covers every sub-command, both doctor phases,
    ...
    `test_meta_section_drift_prevention.bats` meta-test.
...
- **Writing to git.** `livespec` MUST NOT invoke `git commit`,
  ...
  `git show HEAD:` is the narrow exception for doctor check #9.
...
2. Implement the plugin skeleton: `plugin.json`, `SKILL.md`,
   ...
   `scripts/dispatch`, `scripts/doctor-static` implementing the
   static checks, the `jq` dependency check, and the minimum
   subset of the `livespec` template needed to consume
   PROPOSAL.md as seed input.
