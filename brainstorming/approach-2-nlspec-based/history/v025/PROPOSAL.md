...
- **Initial-vendoring exception (one-time, v018 Q3).** The first
  ...
  `fastjsonschema`, `structlog`, `jsoncomment`) is a one-time
  MANUAL procedure, distinct from the blessed `just vendor-update`
  path above. (The v018 Q4 addition `returns_pyright_plugin` was
  removed from this list in v025 — see decision D1 of
  `history/v025/proposed_changes/critique-fix-v024-revision.md`.):
