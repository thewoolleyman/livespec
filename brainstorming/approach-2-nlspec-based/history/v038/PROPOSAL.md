...
`check-coverage` in that case (deferring their
verification to the commit-msg replay hook), running the
full `just check` aggregate (including 100% line+branch
**per-file** coverage per v033 D2 and all canonical-
target checks) in every other case (Green amend,
`test:`/`chore:`/etc., non-Red `feat:`/`fix:`). Pre-push
and CI invoke `just check` directly (no Red-mode
classifier; full suite always); manual invocation of
`just check` from a developer shell is also unconditional.
...
1. **Red mode (initial commit).** Triggered when the staged
   ...
   **Coexistence with the pre-commit gate (v036 D1, v037 D1).** The
   Red commit's failing test would otherwise be blocked by
   lefthook pre-commit's `check-tests` / `check-coverage`
   invocations. Per v036 D1 (broadened in v037 D1),
   lefthook pre-commit invokes `just check-pre-commit`
   (not `just check`); that aggregate detects Red-mode
   shape via `git diff --cached --name-only --diff-filter=AM`
   and skips `check-tests` + `check-coverage` when the
   shape matches (one test file added or modified under
   `tests/`, zero impl files added or modified). The
   commit-msg replay hook (this hook) runs pytest on the
   staged test file, verifying the Red moment. Net effect:
   each Red commit's test is run exactly once at commit
   time (by the replay hook), and pre-commit's other
