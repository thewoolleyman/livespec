...
4. **Steps.** An ordered list of LLM-driven steps. Each step is
   ...
   - **Retry-on-exit-4:** on wrapper exit code `4` (schema
     ...
     and re-assemble the JSON payload. **Up to 3 attempts total
     (1 initial invocation plus up to 2 retries)** per PROPOSAL.md
     §"Templates — Skill↔template communication layer"; abort
     on the third failing attempt with a visible user message.
     Exit code `3` (precondition/doctor-static failure) is NOT
...
6. **Failure handling.** Exit-code-to-narration mapping for the
   ...
   - Exit `4` → schema-validation failure on LLM-provided JSON
     ...
     to 2 retries (3 attempts total; see step 4 "Retry-on-exit-4"
     above).
...
with error context. After 3 attempts total (1 initial invocation
plus up to 2 retries) fail, the skill aborts the sub-command with
an error and preserves partial state for investigation.
...
2. The canonical shape for the v014 N9 end-to-end integration
   ...
   deterministically (format is implementer choice per
   `deferred-items.md`'s `template-prompt-authoring` and
   `end-to-end-integration-test` joint resolution; the real LLM
   treats the comments as inert markdown).
...
- The LLM (per `seed/SKILL.md`) invokes the active template's
  ...
  up to 2 retries (3 attempts total) per PROPOSAL.md §"Templates —
  Skill↔template communication layer."
...
- **Post-step doctor-static failure recovery (v014 N7).** If
  seed's post-step doctor-static emits fail Findings (exit 3),
  the specification and history files have already been
  written to disk (seed's sub-command logic completed before
  post-step). To correct the issues WITHOUT re-seeding (seed's
  idempotency refusal blocks re-seed), the user follows this
  recovery path:
  ...
  3. Run `/livespec:revise --skip-pre-check` to process the
     ...
     makes the spec pass its own doctor-static.)
...
- **`bin/propose_change.py` accepts `--findings-json <path>`
  ...
  template prompt with the error context, up to 2 retries
  (3 attempts total).
...
- **Author identifier → filename slug transformation (v014 N5).**
  ...
  component (the `<resolved-author>-critique.md` topic suffix in
  `critique`, or any other filename-derived use in the future),
  ...
  transformation applies uniformly across `propose-change`,
  `critique`, and `revise`. Full semantics (edge cases,
  deterministic behavior for empty results, collision with
  already-slugged topic values) are codified in
...
- The LLM writes the JSON to a tempfile and invokes
  ...
  prompt with the error context, up to 2 retries (3 attempts
  total).
...
- `bin/critique.py` delegates internally to `propose_change`'s
  ...
  operation.
- **Collision disambiguation (v014 N6).** If a file with topic
  `<resolved-author-slug>-critique.md` already exists, the skill
  MUST auto-disambiguate using the same monotonic-counter-
  from-`2` convention documented in §"propose-change":
  `<resolved-author-slug>-critique-2.md`,
  `<resolved-author-slug>-critique-3.md`, and so on. No user
  prompt. The `<resolved-author-slug>` form follows the v014 N5
  author-slug transformation.
...
│   └── test_*.py                             (pytest suite; shared between mock and real harness modes; env var LIVESPEC_E2E_HARNESS=mock|real selects executable)
...
- A meta-test, `test_meta_section_drift_prevention.py`, MUST
  ...
  test case in the coverage registry. The meta-test SKIPS any
  `##` heading whose text begins with the literal `Scenario:`
  prefix (scenario blocks are exercised end-to-end by the
  per-spec-file rule test for the scenario-carrying spec file;
  per-scenario registry entries are not required).
- **Coverage registry**: `tests/heading-coverage.json` maps
  ...
  [
    ...
    { "spec_file": "spec.md", "heading": "Foo rule", "test": "TODO", "reason": "rule added in v015; implementation and test pending" }
  ]
  ```
  ...
  (`<path>::<function>`), OR the literal `"TODO"` when the test
  is not yet authored — in which case the entry MUST also carry
  a non-empty `reason` field explaining why (e.g., `"rule
  added in v015; implementation and test pending"`).
...
the `minimal` template. Two invocation tiers share the same
pytest suite:
...
`LIVESPEC_E2E_HARNESS=mock|real` selects the executable.
...
1. **Retry-on-exit-4**: an intentionally schema-invalid LLM
   payload triggers the wrapper's exit-4 retry loop; the
   LLM (or mock) re-composes a valid payload within the
   3-attempts-total budget. Assertion: sub-command succeeds
   on retry.
...
affect natural-language prompt interpretation). The exact
format (HTML comment syntax, key=value, JSON-in-comment,
etc.) is implementer choice settled at the intersection of
`deferred-items.md`'s `template-prompt-authoring` (prompt
author picks) and `end-to-end-integration-test` (mock parser
honors); both deferred entries MUST agree on a single
format. This is an instance of the architecture-vs-mechanism
discipline (v009 I0): v014 codifies the CONTRACT ("parseable
directives identifying wrapper invocations") but not the
mechanism.
...
├── dev-tooling/
│   └── checks/                                (enforcement-check Python scripts; purity + import_graph + import-surface portion of no_raise_outside_io are NOT here — replaced by Import-Linter declarative contracts in pyproject.toml per v012 L15a)
│       ├── file_lloc.py
│       ├── private_calls.py
│       ├── global_writes.py
│       ├── supervisor_discipline.py
│       ├── no_raise_outside_io.py             (raise-site portion only per v012 L15a)
...
├── pyproject.toml                             (ruff + pyright + pytest + coverage + import-linter config; no build system; Import-Linter minimum configuration per v013 M7)
...
3. **Both built-in templates are fully implemented** (v014 N1):
   ...
   - `minimal` (single-file): `template.json` declaring
     ...
     at the intersection of `template-prompt-authoring` and
     `end-to-end-integration-test` deferred entries), plus the
     starter `specification-template/SPECIFICATION.md`.
...
10. The test suite covers every sub-command, both doctor phases,
    ...
    **Per v014 N9, the end-to-end harness-level integration test
    ...
    the shared pytest suite. CI triggers for the real target
...
12. Developer tooling is in place under `<repo-root>/`:
    ...
    contracts per L15a + v013 M7 minimum-configuration
    example),
...
5. **Process every entry in `deferred-items.md`** by filing
   ...
   `basedpyright-vs-pyright` evaluation (added in v012 L14),
   the restricted-YAML parser (`front-matter-parser`), the
   companion-doc migrations, the v014 N9
   `end-to-end-integration-test` entry (fixture + mock
   executable + pytest suite + CI workflows), and the
