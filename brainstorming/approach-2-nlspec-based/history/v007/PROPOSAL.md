...
│       │   ├── fastjsonschema_facade.py # typed wrapper over fastjsonschema (Any confined here)
...
+ sub-command logic + post-step doctor static) is composed as a
single ROP chain inside the Python wrapper. The post-step
LLM-driven phase, where applicable, runs from skill prose **after**
the wrapper exits.

### Wrapper-side: deterministic ROP chain
...
run_static(ctx)            # pre-step
   ...
   |> emit_findings_and_exit
```

- Failures at any stage short-circuit the chain via ROP semantics.
- The pre-step skip flag (`--skip-pre-check` / config
  `pre_step_skip_static_checks: true`) is parsed by the wrapper
  and elides the first `run_static` from the chain. The skill
  MUST surface a warning via LLM narration whenever pre-step is
  skipped.
- `bin/doctor_static.py` is the exception: it IS the static phase
  and has no pre/post wrap.
- `prune-history`'s wrapper has a pre-step but NO post-step
  LLM-driven phase (post-step static still runs).
...
- `bin/seed.py` writes the resulting files to the repo at the
  paths dictated by the template's `specification-template/`
  structure.
...
- **`bin/propose_change.py` ONLY accepts `--findings-json <path>`**;
  ...
  via `livespec.validate.findings.validate` (factory shape with
  the loaded `critique_findings.schema.json`), writes the validated
...
- The wrapper validates the JSON against
  ...
  maps each finding to one `## Proposal` section.
...
- The LLM (per `critique/SKILL.md`) invokes the active template's
  ...
  the findings schema (skill-owned; under
  `.claude-plugin/scripts/livespec/schemas/critique_findings.schema.json`).
- The LLM validates the JSON against the schema using
  `fastjsonschema` (via the validate/ factory shape); re-invokes
  the prompt on malformed output (up to 3 retries) with error
  context; aborts on repeated failure.
- The LLM then invokes `bin/critique.py --findings-json <tempfile>
  <author>`, which delegates internally to `propose_change`'s
...
1. YAML front-matter (file-level):
   ...
   keys are a parse error. The parsed dict is then validated
   against `front_matter.schema.json` via the factory-shape
   validator pattern.
...
2. The plugin layout (`.claude-plugin/plugin.json` +
   ...
   (with vendored-lib facades), `parse/`, `validate/`, `schemas/`,
   `context.py`, `errors.py`, plus the `bin/*.py` shebang-wrapper
   executables and `bin/_bootstrap.py`, and the `_vendor/` tree with
   pinned versions of dry-python/returns + fastjsonschema +
   structlog — and the built-in `specification-templates/livespec/`
...
13. Every directory under `.claude-plugin/scripts/` (with
    ...
    `<repo-root>/tests/`, and under `<repo-root>/dev-tooling/`
...
5. **Process every entry in `deferred-items.md`** by filing
   ...
   way include template prompt authoring, AST-check semantics
   (`ast-check-semantics`), the returns pyright-plugin
