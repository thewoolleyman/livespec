...
- `CRITIQUE`  — the critique-file sequence number for this pass.
                ...
                So for producing `v(N+1)`, use `v(N-1)` zero-padded to
                two digits (e.g. producing v005 → `CRITIQUE = 04`).
...
`PROPOSAL.md` or the two spec files above — the current set is):
...
3. **Interview me through every item**, one question at a time.
   ...
   - If mid-interview I rewrite one of the companion files (e.g. the
     bash style doc, goals/non-goals), append new critique items for
     any gaps that rewrite opens up against PROPOSAL.md before
     finishing.
